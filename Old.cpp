...
void LocalBundleAdjustor::Initialize(IBA::Solver* solver, const int serial,
    const int verbose, const int debug, const int history)
{
...
#endif
    m_dsGT =
        solver->m_internal->m_DsGT.empty() ? NULL : &solver->m_internal->m_dsGT;
#ifdef CFG_HISTORY
...
void LocalBundleAdjustor::Reset()
{
    ...
    m_ic2LF.reserve(LBA_MAX_LOCAL_FRAMES);
    m_ic2LF.resize(0);
    m_LFs.reserve(LBA_MAX_LOCAL_FRAMES);
    m_LFs.resize(0);
    m_CsLF.Reserve(LBA_MAX_LOCAL_FRAMES);
    m_CsLF.Resize(0);
#ifdef CFG_GROUND_TRUTH
    m_CsLFGT.Reserve(LBA_MAX_LOCAL_FRAMES);
    m_CsLFGT.Resize(0);
#endif
    m_ucsLF.reserve(LBA_MAX_LOCAL_FRAMES);
    m_ucsLF.resize(0);
    m_ucmsLF.reserve(LBA_MAX_LOCAL_FRAMES);
    m_ucmsLF.resize(0);
#ifdef CFG_INCREMENTAL_PCG
    m_xcsLF.Reserve(LBA_MAX_LOCAL_FRAMES);
    m_xcsLF.Resize(0);
    m_xmsLF.Reserve(LBA_MAX_LOCAL_FRAMES);
    m_xmsLF.Resize(0);
#endif
    m_DsLF.Reserve(LBA_MAX_LOCAL_FRAMES);
    m_DsLF.Resize(0);
#ifdef CFG_GROUND_TRUTH
    m_DsLFGT.Reserve(LBA_MAX_LOCAL_FRAMES);
    m_DsLFGT.Resize(0);
#endif
    m_AdsLF.Reserve(LBA_MAX_LOCAL_FRAMES);
    m_AdsLF.Resize(0);
    m_AfpsLF.Reserve(LBA_MAX_LOCAL_FRAMES);
    m_AfpsLF.Resize(0);
    m_AfmsLF.Reserve(LBA_MAX_LOCAL_FRAMES);
    m_AfmsLF.Resize(0);
    m_SAcusLF.Reserve(LBA_MAX_LOCAL_FRAMES);
    m_SAcusLF.Resize(0);
    m_SMcusLF.Reserve(LBA_MAX_LOCAL_FRAMES);
    m_SMcusLF.Resize(0);
    m_SAcmsLF.Reserve(LBA_MAX_LOCAL_FRAMES);
    m_SAcmsLF.Resize(0);
...
void LocalBundleAdjustor::PushKeyFrame(
    const GlobalMap::InputKeyFrame& IKF, const bool serial)
{
...
void LocalBundleAdjustor::PushDeleteMapPoints(
    const int iFrm, const std::vector<int>& ids)
{
...
void LocalBundleAdjustor::PushUpdateCameras(const int iFrm,
    const std::vector<GlobalMap::InputCamera>& Cs, const bool serial)
{
...
//#if 1
  if (m_debug >= 2) {
...
#ifdef LBA_DEBUG_CHECK
    {
        ...
        if (nKFs != 0)
        {
            ...
            if (g_ix >= 0 && g_ix < int(g_KF->m_xs.size()))
            {
                ...
                if (g_ist >= 0 &&
                    g_ist < g_KF->m_ix2ST[g_ix + 1] - g_KF->m_ix2ST[g_ix])
                {
    ...
    {
        const int NzLF = CountMeasurementsFeatureLF(),
                  NzKF = CountMeasurementsFeatureKF();
...
#ifdef CFG_GROUND_TRUTH
        if (!m_CsLFGT.Empty() && !m_CsKFGT.Empty() && !m_DsLFGT.Empty() &&
            m_dsGT)
        {
            UT::PrintSeparator();
            PrintErrorStatistic(
                "*GT: ", m_CsLFGT, m_CsKFGT, *m_dsGT, m_DsLFGT, true);
        }
#endif
    }
#endif
...
//#if 1
  {
    ...
    if (m_verbose >= 2)
    {
        ...
        if (g_viewer)
        {
            ...
            g_viewer->ActivateFrame(
                g_viewer->GetKeyFrames() + g_viewer->GetLocalFrames() - 1);
        }
    }
#endif
#if 0
...
//#ifdef CFG_DEBUG
      ...
      m_SAcmsLF.MakeZero();
        ...
        if (m_verbose)
        {
            ...
            PrintErrorStatistic(UT::String("*%2d: ", m_iIter),
                m_CsLF,
                m_CsKF,
                m_ds,
                m_DsLF,
    ...
    UT::DebugStop();
        ...
        for (m_iIterDL = 0; m_iIterDL < nItersDL; ++m_iIterDL)
        {
            ...
            if (m_verbose)
            {
                ...
                PrintErrorStatistic(
                    str, m_CsLFBkp, m_CsKF, m_dsBkp, m_xsDL, m_verbose >= 2);
            }
#endif
...
//#ifdef CFG_DEBUG
    ...
    if (m_history >= 1 && (m_hists.empty() || T.m_iFrm > m_hists.back().m_iFrm))
    {
        ...
        if (m_history >= 2)
        {
            ...
            hist.m_ESb =
                ComputeErrorStatistic(m_CsLFBkp, m_CsKF, m_dsBkp, m_DsLF);
            hist.m_ESp = ComputeErrorStatistic(
                m_CsLFBkp, m_CsKF, m_dsBkp, m_xsDL, false);
#ifdef CFG_GROUND_TRUTH
            if (m_CsGT)
            {
                ...
                if (m_dsGT)
                {
                    ...
                    {
                        EmbeddedPointIteration(
                            m_CsLFGT, m_CsKFGT, m_ucsKFGT, m_udsGT, m_dsGT);
                    }
                    SolveBackSubstitutionGT(m_dsBkp, &m_xsGT);
                    hist.m_ESaGT = ComputeErrorStatistic(
                        m_CsLFGT, m_CsKFGT, *m_dsGT, m_DsLFGT);
                    hist.m_ESpGT = ComputeErrorStatistic(
                        m_CsLFBkp, m_CsKFBkp, m_dsBkp, m_xsGT, false);
                }
            }
#endif
            ...
            if (m_CsGT)
            {
                ...
                const float epsr = UT::Inverse(
                    BA_VARIANCE_MAX_ROTATION, BA_WEIGHT_FEATURE, eps);
                const float epsp = UT::Inverse(
                    BA_VARIANCE_MAX_POSITION, BA_WEIGHT_FEATURE, eps);
                const float epsv = UT::Inverse(
                    BA_VARIANCE_MAX_VELOCITY, BA_WEIGHT_FEATURE, eps);
                const float epsba = UT::Inverse(
                    BA_VARIANCE_MAX_BIAS_ACCELERATION, BA_WEIGHT_FEATURE, eps);
                const float epsbw = UT::Inverse(
                    BA_VARIANCE_MAX_BIAS_GYROSCOPE, BA_WEIGHT_FEATURE, eps);
                const float _eps[] = {epsp,
                    epsp,
                    epsp,
                    epsr,
                    epsr,
                ...
                if (m_ZpKF.Valid())
                {
                    if (m_ZpKF.GetPriorMeasurement(
                            BA_WEIGHT_FEATURE, &S, &x, &xTb, &m_work, _eps))
                    {
                        CameraPrior::Pose::Error e;
                        m_ZpKF.GetError(m_CsKFGT, &e, BA_ANGLE_EPSILON);
                        hist.m_PSKF.m_F =
                            m_ZpKF.GetCost(1.0f / BA_WEIGHT_FEATURE, e) - xTb;
                        ComputePriorStatisticPose(
                            m_ZpKF, m_CsKFGT, S, x, &hist.m_PSKF);
                    }
                    else
                    {
                ...
                if (m_Zp.Pose::Valid())
                {
                    if (m_Zp.GetPriorMotion(&m_ZpLF, &m_work, _eps) &&
                        m_ZpLF.GetPriorMeasurement(
                            BA_WEIGHT_FEATURE, &S, &x, &xTb, &m_work, _eps + 6))
                    {
                        CameraPrior::Motion::Error e;
                        m_ZpLF.GetError(C, &e);
                        hist.m_PSLF.m_F =
                            m_ZpLF.GetCost(1.0f / BA_WEIGHT_FEATURE, e) - xTb;
                        ComputePriorStatisticMotion(
                            m_ZpLF, C, S, x, &hist.m_PSLF);
                    }
                    else
                    {
...
#endif
                    if (m_Zp.GetPriorMeasurement(
                            BA_WEIGHT_FEATURE, &S, &x, &xTb, &m_work, _eps))
                    {
                        CameraPrior::Joint::Error e;
                        m_Zp.GetError(m_CsKFGT, C, &e, BA_ANGLE_EPSILON);
                        hist.m_PS.m_F =
                            m_Zp.GetCost(1.0f / BA_WEIGHT_FEATURE, e) - xTb;
                        ComputePriorStatisticJoint(
                            m_Zp, m_CsKFGT, C, S, x, &hist.m_PS);
...
#ifdef LBA_DEBUG_PRINT
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            if (i == 0)
            {
                ...
                const LocalFrame& LF = m_LFs[iLF];
                const int iKFNearest = LF.m_iKFNearest == -1
                                           ? LF.m_iKFsMatch.front()
                                           : LF.m_iKFNearest;
                UT::Print("[%d] <-- [%d]\n",
                    LF.m_T.m_iFrm,
                    m_KFs[iKFNearest].m_T.m_iFrm);
                Cc = m_CsLF[iLF];
                Ck.MakeIdentity();
...
//#ifdef CFG_DEBUG
            ...
            else
            {
                ...
                const xp128f sm =
                    xp128f::get(1.0f / (CountSchurComplements() - nLFs));
                for (int iLF = 0; iLF < nLFs; ++iLF)
                {
...
void LocalBundleAdjustor::SetCallback(
    const IBA::Solver::IbaCallback& iba_callback)
{
...
bool LocalBundleAdjustor::SaveCameras(
    const std::string fileName, const bool poseOnly)
{
    FILE* fp = fopen(fileName.c_str(), "w");
    if (!fp)
    {
...
#ifdef CFG_GROUND_TRUTH
        case 3:
            hist.m_ESaGT.Save(fp);
            break;
        case 4:
            hist.m_ESpGT.Save(fp);
            break;
#endif
        }
    }
...
bool LocalBundleAdjustor::SaveResiduals(
    const std::string fileName, const int type)
{
    FILE* fp = fopen(fileName.c_str(), "w");
    if (!fp)
    {
...
bool LocalBundleAdjustor::SaveMarginalizations(
    const std::string fileName, const int type)
{
    FILE* fp = fopen(fileName.c_str(), "w");
    if (!fp)
...
void LocalBundleAdjustor::ComputeErrorFeature(const FRM::Frame* F,
    const Rigid3D& C, const AlignedVector<Rigid3D>& CsKF,
    const std::vector<Depth::InverseGaussian>& ds, float* ex, const int iKF)
{
    Rigid3D Tr[2];
    FTR::Error e;
//#if 0
...
#ifdef CFG_STEREO
    *ex = SN == 0 ? 0.0f
                  : sqrtf((Se2 * m_K.m_K.fxy() + Se2r * m_K.m_Kr.fxy()) / SN);
#else
...
void LocalBundleAdjustor::ComputeErrorIMU(
    float* er, float* ep, float* ev, float* eba, float* ebw)
{
    *er = *ep = *ev = *eba = *ebw = 0.0f;
    ...
    for (int ic1 = 0, ic2 = 1; ic2 < nLFs; ic1 = ic2++)
    {
        ...
        const IMU::Delta::Error e = m_DsLF[iLF2].GetError(
            m_CsLF[iLF1], m_CsLF[iLF2], m_K.m_pu, BA_ANGLE_EPSILON);
...
float LocalBundleAdjustor::GetTotalDistance()
{
    ...
    if (m_CsGT)
    {
        ...
        {
            S += sqrtf((
                m_CsGT[m_hists[i1].m_iFrm].m_p - m_CsGT[m_hists[i2].m_iFrm].m_p)
                           .SquaredLength());
//#ifdef CFG_DEBUG
#if 0
      UT::Print("[%d, %d] %f\n", i1, i2, S);
...
#endif
    {
        ...
        {
            S += sqrtf(
                (m_hists[i1].m_C.m_p - m_hists[i2].m_C.m_p).SquaredLength());
        }
    }
#endif
    return S;
...
void LocalBundleAdjustor::SynchronizeData()
{
...
#endif
    const int iFrm =
        m_ic2LF.empty() ? MT_TASK_NONE : m_LFs[m_ic2LF.back()].m_T.m_iFrm;
...
#endif
    for (std::list<InputLocalFrame>::iterator ILF = m_ILFs2.begin();
         ILF != m_ILFs2.end();
         ++ILF)
    {
        ...
        if (m_LFs.empty())
        {
            ...
            if (m_CsGT)
            {
                const LA::AlignedVector3f g =
                    m_CsGT[ILF->m_T.m_iFrm].m_T.GetGravity();
                C.MakeIdentity(&g);
            }
#endif
        }
    ...
    const bool newKF = !m_IKFs2.empty(), delKF = !m_IDKFs2.empty(),
               updCams = !m_IUCs2.empty();
    ...
    if (newKF)
    {
        ...
        for (std::list<InputType>::const_iterator IT = m_ITs2.begin();
             IT != m_ITs2.end();
             ++IT)
        {
            ...
            if (m_CsGT && m_dsGT)
            {
                ...
                for (int iX1 = 0, iX2 = 0; iX1 < NX; iX1 = iX2)
                {
                    ...
                    for (iX2 = iX1 + 1; iX2 < NX && IKF->m_Xs[iX2].m_iKF == iKF;
                         ++iX2)
                    {
                    }
                    const int Nx = iKF < m_CsKFGT.Size()
                                       ? m_iKF2d[iKF + 1] - m_iKF2d[iKF]
                                       : 0;
                    const Depth::InverseGaussian* ds =
                        m_dsGT->data() + iKF2d[iKF] + Nx - iX1;
                ...
                DebugSetFeatureMeasurements(
                    C, m_CsKFGT, *m_dsGT, iKF2d, &(*IKF));
            }
#endif
            if (IKF->m_C.Invalid())
            {
            ...
            for (int iX1 = 0, iX2 = 0; iX1 < NX; iX1 = iX2)
            {
                ...
                for (int iX = iX1; iX < iX2; ++iX)
                {
                    ...
                    if (X.m_x.m_xr.Valid())
                    {
                        ...
                        m_zds.push_back(
                            Depth::Measurement(m_K.m_br, Rx, X.m_x.m_xr, W));
                    }
#endif
                    ...
                    for (int i = 0; i < Nz; ++i)
                    {
                        ...
                        if (R.Valid())
                        {
                            ...
                            if (z.m_z.Valid())
                            {
                                z.m_W.GetScaled(w, W);
                                m_zds.push_back(
                                    Depth::Measurement(t[0], Rx, z.m_z, W));
                            }
                            if (z.m_zr.Valid())
                            {
                                z.m_Wr.GetScaled(w, W);
                                m_zds.push_back(
                                    Depth::Measurement(t[1], Rx, z.m_zr, W));
                            }
#else
                            z.m_W.GetScaled(w, W);
                            m_zds.push_back(
                                Depth::Measurement(*t, Rx, z.m_z, W));
#endif
                        }
                    }
//#ifdef CFG_DEBUG
          ...
          if (KF.m_T.m_iFrm == 1041 && ix == 431) {
            ...
            X.m_d.Print();
                        ...
                        m_K.m_K.fx() * eAvg > DEPTH_TRI_MAX_ERROR)
                    {
                        // X.m_d.Invalidate();
                        // X.m_d = m_KFs[iKF].m_d;
                        // if (!Depth::Triangulate(w, Nzd, m_zds.data(), &X.m_d,
                        // &m_work)) {
                        //  X.m_d.Invalidate();
                        //}
                        if (Nzd == 0)
    ...
    while (!m_ITs2.empty())
    {
        ...
        if (IT == IT_LOCAL_FRAME)
        {
...
#ifdef LBA_DEBUG_GROUND_TRUTH_MEASUREMENT
            const std::vector<Depth::InverseGaussian>& dsGT =
                m_solver->m_internal->m_dsGT;
            if (m_CsGT && !dsGT.empty())
            {
                DebugSetFeatureMeasurements(m_CsGT[ILF.m_T.m_iFrm].m_T,
                    m_CsKFGT,
                    dsGT,
                    m_solver->m_internal->m_iKF2d,
                    &ILF);
            }
#endif
            if (ILF.m_d.Invalid())
            {
                ...
                const int Nz = static_cast<int>(ILF.m_zs.size()),
                          NzC = SIMD_FLOAT_CEIL(Nz);
                m_work.Resize(NzC * 3);
                LA::AlignedVectorXf us(m_work.Data(), Nz, false);
                us.Resize(0);
                LA::AlignedVectorXf ws(us.Data() + NzC, Nz, false);
                ws.Resize(0);
                LA::AlignedVectorXf wus(ws.Data() + NzC, Nz, false);
                for (std::list<GlobalMap::InputKeyFrame>::iterator IKF =
                         m_IKFs2.begin();
                     IKF != m_IKFs2.end();
                     ++IKF)
                {
                    ...
                    const int NX = static_cast<int>(IKF->m_Xs.size()),
                              N = NX + Nz, NC = SIMD_FLOAT_CEIL(N);
                    m_work.Resize(NC * 3);
                    us.Bind(m_work.Data(), N);
                    us.Resize(0);
                    ws.Bind(us.Data() + NC, N);
                    ws.Resize(0);
            ...
            if (ILF.m_iKFNearest == -1 && Nk != 0)
            {
                if (LBA_MARGINALIZATION_REFERENCE_NEAREST)
                {
                    ...
                    for (int iKF = 0; iKF < _nKFs; ++iKF)
                    {
                        ...
                        const float imgMotion =
                            ComputeImageMotion(z, C, _C, &first);
        ...
        else if (IT == IT_KEY_FRAME || IT == IT_KEY_FRAME_SERIAL)
        {
            ...
            for (int iX = 0; iX < NX; ++iX)
            {
                ...
                {
                    X.m_d.Initialize(
                        X.m_iKF == nKFs ? IKF.m_d.u() : m_KFs[X.m_iKF].m_d.u());
                }
            }
            _PushKeyFrame(IKF);
            m_IKFs2.pop_front();
...
//#if 1
  m_IT->RunView();
#endif
    if (!m_iFrmsKF.empty() && m_GM->LBA_Synchronize(m_iFrmsKF.back(),
                                  m_CsKF,
                                  m_CsKFBkp,
                                  m_marksTmp1
#ifdef CFG_HANDLE_SCALE_JUMP
                                  ...
                                  ))
    {
        ...
        if (ss.Size() >
            static_cast<int>(nKFs * BA_UPDATE_FRAME_DEPTH_RATIO + 0.5f))
        {
...
void LocalBundleAdjustor::UpdateData()
{
    ...
    {
        const int Ncu =
            UT::VectorCountFlag<ubyte>(m_UcsLF, LM_FLAG_FRAME_UPDATE_CAMERA_LF);
        const int Nc = static_cast<int>(m_LFs.size());
        const int Nku =
            UT::VectorCountFlag<ubyte>(m_UcsKF, LM_FLAG_FRAME_UPDATE_CAMERA_KF);
        const int Nk = static_cast<int>(m_KFs.size());
        const int Ndu =
            UT::VectorCountFlag<ubyte>(m_Uds, LM_FLAG_TRACK_UPDATE_DEPTH);
        const int Nd = static_cast<int>(m_ds.size());
        UT::PrintSeparator();
        ...
        UT::Print("  CameraLF = %d / %d = %.2f%%\n",
            Ncu,
            Nc,
            UT::Percentage(Ncu, Nc));
        UT::Print("  CameraKF = %d / %d = %.2f%%\n",
            Nku,
            Nk,
            UT::Percentage(Nku, Nk));
        UT::Print("  Depth    = %d / %d = %.2f%%\n",
            Ndu,
            Nd,
            UT::Percentage(Ndu, Nd));
    }
#endif
...
bool LocalBundleAdjustor::BufferDataEmpty() { return m_ITs1.empty(); }

#ifdef LBA_DEBUG_PRINT_MARGINALIZATION
static inline void PrintReduction(
    const IMU::Delta::Error& e1, const IMU::Delta::Error* e2)
{
    ...
    UT::PrintReduction(
        sqrtf(e1.m_er.SquaredLength()) * UT_FACTOR_RAD_TO_DEG, er, 3, "  r = ");
    ...
    UT::PrintReduction(sqrtf(e1.m_ebw.SquaredLength()) * UT_FACTOR_RAD_TO_DEG,
        ebw,
        3,
        " bw = ");
}
    ...
    const FTR::Error* e2)
{
    ...
    if (e1.Valid())
    {
        ...
        UT::PrintReduction(sqrtf(e1.SquaredLength() * K.m_K.fxy()),
            e,
            3,
            "",
            false,
    ...
    if (e1r.Valid())
    {
        ...
        UT::PrintReduction(sqrtf(e1r.SquaredLength() * K.m_Kr.fxy()),
            e,
            3,
    ...
    UT::PrintReduction(sqrtf(e1.m_ex.SquaredLength() * K.m_K.fxy()),
        e,
        3,
        UT::String("  %d ", ix));
#endif
}
static inline void PrintDifference(
    const LA::Vector2f* xg, const int i1 = 0, const int i2 = 1)
{
    UT::Print(" eg = %f\n",
        sqrtf((xg[i1] - xg[i2]).SquaredLength()) * UT_FACTOR_RAD_TO_DEG);
}
static inline void PrintDifference(
    const CameraPrior::Element::EC* xc, const int i1 = 0, const int i2 = 1)
{
    UT::Print(" ep = %f\n", sqrtf((xc[i1].m_ep - xc[i2].m_ep).SquaredLength()));
    UT::Print(" er = %f\n",
        sqrtf((xc[i1].m_er - xc[i2].m_er).SquaredLength()) *
            UT_FACTOR_RAD_TO_DEG);
}
static inline void PrintDifference(const CameraPrior::Element::EC* xc1,
    const CameraPrior::Element::EC* xc2, const int i1 = 0, const int i2 = 1)
{
    UT::Print(" ep = %f %f\n",
        sqrtf((xc1[i1].m_ep - xc1[i2].m_ep).SquaredLength()),
        sqrtf((xc2[i1].m_ep - xc2[i2].m_ep).SquaredLength()));
    UT::Print(" er = %f %f\n",
        sqrtf((xc1[i1].m_er - xc1[i2].m_er).SquaredLength() *
              UT_FACTOR_RAD_TO_DEG),
        sqrtf((xc2[i1].m_er - xc2[i2].m_er).SquaredLength()) *
            UT_FACTOR_RAD_TO_DEG);
}
static inline void PrintDifference(
    const std::vector<CameraPrior::Element::EC>* xks, const int i1 = 0,
    const int i2 = 1)
{
    ...
    {
        UT::Print(
            " %f", sqrtf((xks[i1][i].m_ep - xks[i2][i].m_ep).SquaredLength()));
    }
    UT::Print("\n");
    UT::Print(" er =");
    for (int i = 0; i < N; ++i)
    {
        UT::Print(" %f",
            sqrtf((xks[i1][i].m_er - xks[i2][i].m_er).SquaredLength()) *
                UT_FACTOR_RAD_TO_DEG);
    }
    UT::Print("\n");
}
static inline void PrintDifference(const CameraPrior::Element::EM* xm1,
    const CameraPrior::Element::EM* xm2, const int i1 = 0, const int i2 = 1)
{
    UT::Print(" ev = %f %f\n",
    ...
    UT::Print("ebw = %f %f\n",
        sqrtf((xm1[i1].m_ebw - xm1[i2].m_ebw).SquaredLength()) *
            UT_FACTOR_RAD_TO_DEG,
        sqrtf((xm2[i1].m_ebw - xm2[i2].m_ebw).SquaredLength()) *
            UT_FACTOR_RAD_TO_DEG);
}
static inline void PrintDifference(const LA::Vector2f* xg,
    const std::vector<CameraPrior::Element::EC>* xks,
    const CameraPrior::Element::EC* xc1, const CameraPrior::Element::EM* xm1,
    const CameraPrior::Element::EC* xc2, const CameraPrior::Element::EM* xm2)
{
...
//#ifdef CFG_DEBUG
  ...
  m_Zp.PrintDiagonal();
    ...
    const float eps = 0.0f;
    const float epsr =
        UT::Inverse(BA_VARIANCE_MAX_ROTATION, BA_WEIGHT_FEATURE, eps);
    const float epsp =
        UT::Inverse(BA_VARIANCE_MAX_POSITION, BA_WEIGHT_FEATURE, eps);
    const float epsv =
...
//#ifdef CFG_DEBUG
  ...
  UT_ASSERT(m_KFs[iKFr].m_T.m_iFrm <= LF1.m_T.m_iFrm);
    ...
    UT::Print("LF [%d]\n", LF1.m_T.m_iFrm);
    UT::Print("KF [%d] %d",
        m_Zp.m_iKFr == -1 ? -1 : m_KFs[m_Zp.m_iKFr].m_T.m_iFrm,
        m_Zp.m_iKFr);
    if (iKFr != m_Zp.m_iKFr)
    {
...
//#ifdef LBA_DEBUG_GROUND_TRUTH_MEASUREMENT
        ...
        const float s2g =
            v ? BA_VARIANCE_PRIOR_GRAVITY_NEW : BA_VARIANCE_PRIOR_GRAVITY_FIRST;
        // m_ZpLF.m_Amm *= sp;
        // m_ZpLF.m_bm *= sp;
        m_Zp.Initialize(BA_WEIGHT_PRIOR_CAMERA_INITIAL /* * sp*/,
            iKFr,
            Tr,
            s2g,
            m_ZpLF,
        ...
        if (m_history >= 2)
        {
            m_MH.Initialize(BA_WEIGHT_PRIOR_CAMERA_INITIAL,
                s2g,
...
#endif
        if (!m_Zp.PropagateKF(Tr, C2, A, &m_work, _eps) &&
            LBA_MARGINALIZATION_CHECK_INVERTIBLE)
        {
            m_ZpLF.Initialize(BA_WEIGHT_PRIOR_CAMERA_INITIAL,
                BA_VARIANCE_PRIOR_VELOCITY_RESET,
...
#endif
            m_Zp.Initialize(BA_WEIGHT_PRIOR_CAMERA_INITIAL,
                iKFr,
                Tr,
            ...
            if (m_CsGT)
            {
                m_Zp.Pose::DebugSetMeasurement(
                    m_CsKFGT[iKFr], false, &m_CsLFGT[iLF2].m_T);
            }
#endif
#ifdef CFG_HISTORY
            if (m_history >= 2)
...
//#ifdef CFG_DEBUG
    ...
    const bool scc2 = m_Zp.Invertible(&m_work, _eps);
        ...
        {
            int ip = 0;
            xg[2].Set(m_MH.m_x.Data() + ip);
            ip += 2;
            xm1[2].Set(m_MH.m_x.Data() + ip);
            ip += 9;
            xc2[2].Set(m_MH.m_x.Data() + ip);
            ip += 6;
    ...
    else
    {
        ...
        const float s2d = UT::Inverse(
            BA_VARIANCE_PRIOR_DEPTH_NEW, BA_WEIGHT_PRIOR_CAMERA_INITIAL);
        const float epsd =
            UT::Inverse(BA_VARIANCE_MAX_DEPTH, BA_WEIGHT_FEATURE, eps);
        const float epsc[12] = {epsp,
            epsp,
            epsp,
            epsr,
            epsr,
            ...
            epsr,
            epsr};
        // const float s2pMax = BA_VARIANCE_PRIOR_POSITION_INSERT_MAX /
        // BA_WEIGHT_FEATURE; const float s2rMax =
        // BA_VARIANCE_PRIOR_ROTATION_INSERT_MAX / BA_WEIGHT_FEATURE; const
        // float r2Max = ME_VARIANCE_HUBER;
        ...
        for (int iZ = 0; iZ < NZ; ++iZ)
        {
            ...
            // const Depth::InverseGaussian *ds = m_dsGT->data() +
            // m_solver->m_internal->m_iKF2d[iKF];
            const KeyFrame& KF = m_KFs[iKF];
            if (iKF == m_Zp.m_iKFr)
            {
                *Tk1 = Tr1;
                ...
                for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz)
                {
                    ...
                    const int ix = z.m_ix;
                    FTR::GetFactor<LBA_ME_FUNCTION>(BA_WEIGHT_FEATURE,
                        Tk1,
                        KF.m_xs[ix],
                        ds[ix],
                        Tr1,
                    ...
                    FTR::Factor::FixSource::Marginalize(
                        mdd, A1.m_adczA, &mdcz, &M);
                    SAczz -= M.m_Mczz;
                }
            ...
            {
                const std::vector<int>::const_iterator it = std::lower_bound(
                    m_Zp.m_iKFs.begin(), m_Zp.m_iKFs.end(), iKF);
...
//#ifdef CFG_DEBUG
            ...
            A2.m_adx.m_add.m_a = addBkp;
                    ...
                    FTR::Factor::Full::Source::Marginalize(
                        mdd, A2.m_adx, &M1, &M2);
                    FTR::Factor::Full::Marginalize(
                        mdd, M1, A1, &M3, &M4, &adcz);
                    SAcxx -= M2.m_Mcxx;
                    SAcxz -= M4.m_Mcxz;
                    SAczz -= M4.m_Mczz;
                }
...
//#ifdef CFG_DEBUG
          ...
          UT::DebugStop();
                    ...
                    m_Zp.Insert(BA_WEIGHT_PRIOR_CAMERA_INITIAL,
                        ik,
                        iKF,
                        Trk,
                        BA_VARIANCE_PRIOR_POSITION_NEW,
...
//#ifdef CFG_DEBUG
        ...
        if (m_history >= 2)
        {
            ...
            const int Npgm = Npg + (m_MH.m_FdKF.Valid() ? Npm : 0),
                      Npcm = Npc + Npm;
#ifdef CFG_GROUND_TRUTH
            if (m_CsGT)
            {
...
//#ifdef CFG_DEBUG
      ...
      const bool scc3 = A.SolveLDL(b);
            ...
            if (m_Zp.GetPriorMeasurement(
                    BA_WEIGHT_FEATURE, &S, &x, NULL, &m_work, _eps))
            {
...
//#ifdef LBA_DEBUG_PRINT_MARGINALIZATION
      ...
      const Rigid3D &Tr = m_CsKFGT[iKFr];
            ...
            if (m_CsGT)
            {
                const Rigid3D &TrGT = m_CsKFGT[iKFr],
                              &T1GT = m_CsLFGT[iLF1].m_T;
        ...
        if (m_CsGT)
        {
            ...
            for (int i = 0; i < Nk; ++i)
            {
                m_Zp.Pose::GetError(m_CsKFGT[iKFr],
                    m_CsKFGT[m_Zp.m_iKFs[i]],
                    i,
                    &xks[0][i],
                    BA_ANGLE_EPSILON);
            }
...
//#ifdef CFG_DEBUG
    ...
    const bool scc1 = m_Zp.Invertible(&m_work, _eps);
#endif
        if (!m_Zp.PropagateLF(_Tr, C2, A, &m_work, _eps) &&
            LBA_MARGINALIZATION_CHECK_INVERTIBLE)
        {
...
//#if 0
    ...
    UT_ASSERT(m_Zp.m_iKFs[Nk] == INT_MAX);
#endif
    m_Zp.m_Acc[Nk][Nk].IncreaseDiagonal(
        UT::Inverse(
            BA_VARIANCE_PRIOR_POSITION_NEW, BA_WEIGHT_PRIOR_CAMERA_INITIAL),
        UT::Inverse(
            BA_VARIANCE_PRIOR_ROTATION_NEW, BA_WEIGHT_PRIOR_CAMERA_INITIAL));
...
#endif
            if (icMin == -1 || (KF.m_ix2ST[ix + 1] - iSTMin > 1 &&
                                   KF.m_STs[iSTMin + 1].m_icMin == icMin))
            {
                popST = true;
                ...
                mddST.MakeMinus();
                const bool nonZero = !(
                    KF.m_usST[iSTMin] & LBA_FLAG_TRACK_UPDATE_INFORMATION_ZERO);
                if (icMin != -1)
                {
...
#ifdef CFG_DEBUG
                        UT_ASSERT(_LF.m_STs[_iz].m_ist1 == 0 &&
                                  _LF.m_STs[_iz].m_ist2 > 1);
#endif
...
//#ifdef CFG_DEBUG
    ...
    for (int _ic = 1; _ic < icMax; ++_ic)
    {
        ...
        for (int iI = 0; iI < NI; ++iI)
        {
            ...
            for (int i = i1; i < i2; ++i)
            {
                const int iz2 = LF1.m_Zm.m_izms[i].m_iz2,
                          ix = LF2.m_zs[iz2].m_ix, iST = KF.m_ix2ST[ix];
                if (LF2.m_STs[iz2].m_ist1 != 0 ||
                    !(KF.m_usST[iST] & LBA_FLAG_TRACK_POP) ||
                    (KF.m_usST[iST] & LBA_FLAG_TRACK_UPDATE_INFORMATION_ZERO))
                {
                    continue;
                }
                // LF1.m_Zm.m_SmddsST[i] = -KF.m_MxsST[iST].m_mdd.m_a +
                // LF1.m_ZmLF.m_SmddsST[i];
                LF1.m_Zm.m_SmddsST[i] =
                    KF.m_MxsST[iST].m_mdd.m_a + LF1.m_Zm.m_SmddsST[i];
    ...
    for (int _ic = STL; _ic < nLFs; ++_ic)
    {
        ...
        for (int _iZ = 0; _iZ < _NZ; ++_iZ)
        {
            ...
            {
                if (KF.m_usST[KF.m_ix2ST[_LF.m_zs[_iz].m_ix]] &
                    LBA_FLAG_TRACK_POP)
                {
                    _LF.m_STs[_iz].Step();
                }
    ...
    for (int iZ = 0; iZ < NZ; ++iZ)
    {
        ...
        for (int ix = 0, iST = 0; ix < Nx; ++ix)
        {
            ...
            const bool popST =
                iST1 < iST2 && (KF.m_usST[iST1] & LBA_FLAG_TRACK_POP);
...
//#if 1
  UT::Print("+ [%d]\n", LF.m_T.m_iFrm);
    ...
    while (!LF.m_Zs.empty() &&
           LF.m_Zs.back().m_iKF >= static_cast<int>(m_KFs.size()))
    {
    ...
    const int nLFs2 = int(m_LFs.size()),
              STL = std::min(nLFs2, LBA_MAX_SLIDING_TRACK_LENGTH);
    ...
    m_ucsLF[iLF] = LBA_FLAG_FRAME_UPDATE_CAMERA;
    m_ucmsLF[iLF] = LBA_FLAG_CAMERA_MOTION_UPDATE_ROTATION |
                    LBA_FLAG_CAMERA_MOTION_UPDATE_POSITION |
                    LBA_FLAG_CAMERA_MOTION_UPDATE_VELOCITY |
                    LBA_FLAG_CAMERA_MOTION_UPDATE_BIAS_ACCELERATION |
                    LBA_FLAG_CAMERA_MOTION_UPDATE_BIAS_GYROSCOPE;
  ...
  if (nLFs2 > 1) {
...
#endif
    const ubyte udFlagST =
        LBA_FLAG_TRACK_PUSH | LBA_FLAG_TRACK_UPDATE_INFORMATION_ZERO;
    const int NZ = static_cast<int>(LF.m_Zs.size());
    for (int iZ = 0; iZ < NZ; ++iZ)
    {
        const FRM::Measurement& Z = LF.m_Zs[iZ];
        ...
        for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz)
        {
            const int ix = LF.m_zs[iz].m_ix, Nst = KF.CountSlidingTracks(ix),
                      iSTMax = KF.m_ix2ST[ix + 1] - 1;
        ...
        m_ix2STTmp.swap(KF.m_ix2ST);
        KF.m_ix2ST.resize(Nx + 1);
        m_STsTmp.swap(KF.m_STs);
        KF.m_STs.resize(0);
        m_usSTTmp.swap(KF.m_usST);
        KF.m_usST.resize(0);
        m_AxsTmp.Swap(KF.m_AxsST);
        KF.m_AxsST.Resize(0);
        m_MxsTmp.Swap(KF.m_MxsST);
        KF.m_MxsST.Resize(0);
        for (int ix = 0; ix < Nx; ++ix)
        {
            const int iST1 = m_ix2STTmp[ix], iST2 = m_ix2STTmp[ix + 1],
            ...
            KF.m_STs.insert(KF.m_STs.end(),
                m_STsTmp.begin() + iST1,
                m_STsTmp.begin() + iST2);
            KF.m_usST.insert(KF.m_usST.end(),
                m_usSTTmp.begin() + iST1,
                m_usSTTmp.begin() + iST2);
            ...
            KF.m_AxsST.Resize(NST2, true);
            KF.m_AxsST[NST1].MakeZero();
            KF.m_MxsST.Resize(NST2, true);
            KF.m_MxsST[NST1].MakeZero();
#ifdef CFG_DEBUG
    ...
    if (nLFs2 > 1)
    {
        ...
        if (nLFs2 > 2)
        {
            ...
            m_ucmsLF[_iLF] |= LBA_FLAG_CAMERA_MOTION_UPDATE_ROTATION |
                              LBA_FLAG_CAMERA_MOTION_UPDATE_POSITION |
                              LBA_FLAG_CAMERA_MOTION_UPDATE_VELOCITY |
                              LBA_FLAG_CAMERA_MOTION_UPDATE_BIAS_ACCELERATION |
                              LBA_FLAG_CAMERA_MOTION_UPDATE_BIAS_GYROSCOPE;
...
//#ifdef CFG_DEBUG
...
#endif
#ifdef LBA_DEBUG_GROUND_TRUTH_MEASUREMENT
            D.DebugSetMeasurement(
                _C, m_CsLFGT[iLF], m_K.m_pu, BA_ANGLE_EPSILON);
            m_DsLFGT[iLF].DebugSetMeasurement(
                _C, m_CsLFGT[iLF], m_K.m_pu, BA_ANGLE_EPSILON);
#endif
            if (nLFs2 > 2)
            {
                ...
                IMU::PreIntegrate(_LF.m_us,
                    ...
                    BA_ANGLE_EPSILON);
#ifdef LBA_DEBUG_GROUND_TRUTH_MEASUREMENT
                m_DsLF[_iLF].DebugSetMeasurement(
                    m_CsLFGT[__iLF], _C, m_K.m_pu, BA_ANGLE_EPSILON);
                _D.DebugSetMeasurement(
                    m_CsLFGT[__iLF], _C, m_K.m_pu, BA_ANGLE_EPSILON);
#endif
            }
        }
    ...
    if (LF.m_T.m_iFrm == 0)
    {
        const LA::Vector3f s2r =
            LA::Vector3f::Get(BA_VARIANCE_FIX_ORIGIN_ROTATION_X,
...
void LocalBundleAdjustor::_PushKeyFrame(const GlobalMap::InputKeyFrame& IKF)
{
    ...
    const ubyte udFlag1 =
        LBA_FLAG_TRACK_UPDATE_DEPTH | LBA_FLAG_TRACK_UPDATE_INFORMATION_ZERO;
    const int NX = static_cast<int>(IKF.m_Xs.size());
    for (int iX1 = 0, iX2 = 0; iX1 < NX; iX1 = iX2)
    {
        ...
        if (m_history >= 3)
        {
            ...
            m_udsGT.insert(
                m_udsGT.begin() + id, Nx, LBA_FLAG_TRACK_UPDATE_DEPTH);
        }
#endif
...
#ifdef CFG_HANDLE_SCALE_JUMP
        m_dsKF[_iKF] = AverageDepths(
            m_ds.data() + m_iKF2d[_iKF], m_iKF2d[_iKF + 1] - m_iKF2d[_iKF]);
#endif
        ...
        for (int ik2 = 0; ik2 < Nk; ++ik2)
        {
            ...
            for (int ik1 = 0; ik1 < ik2; ++ik1)
            {
                ...
                const std::vector<int>::iterator _jk2 = std::lower_bound(
                    KF2.m_iKFsMatch.begin() + jk2, KF2.m_iKFsMatch.end(), iKF1);
                ...
                int& jk1 = iks[ik1];
                const std::vector<int>::iterator _jk1 = std::lower_bound(
                    KF1.m_iKFsMatch.begin() + jk1, KF1.m_iKFsMatch.end(), iKF2);
    ...
    for (int ic = 0; ic < nLFs; ++ic)
    {
        ...
        {
            const int Nx = static_cast<int>(KF.m_xs.size()),
                      Nz = static_cast<int>(LF.m_zs.size());
//#ifdef CFG_DEBUG
#if 0
      UT_ASSERT(static_cast<int>(KF.m_zs.size()) >= Nz);
            ...
            KF.m_AxsST.Resize(Nx);
            KF.m_AxsST.MakeZero();
            KF.m_MxsST.Resize(Nx);
            KF.m_MxsST.MakeZero();
#ifdef CFG_DEBUG
            for (int ix = 0; ix < Nx; ++ix)
            {
    ...
    const ubyte udFlag3 =
        (ud ? LBA_FLAG_TRACK_UPDATE_DEPTH : LBA_FLAG_TRACK_DEFAULT)
#ifdef LBA_FLAG_TRACK_MEASURE_KF
    ...
    m_GM->LBA_PushKeyFrame(GlobalMap::Camera(IKF.m_C.m_T,
        KF.m_T.m_iFrm,
        GM_FLAG_FRAME_DEFAULT
#ifdef CFG_HANDLE_SCALE_JUMP
        ,
        m_dsKF[iKF]
#endif
    ...
    const int N = static_cast<int>(
        std::lower_bound(m_usKF.Data(), m_usKF.End(), KF.m_T.m_t) -
        m_usKF.Data());
    m_usKF.Erase(N, m_usKFLast);
    m_GBA->PushKeyFrame(IKF,
        m_usKFLast,
        m_dsBkp
#ifdef CFG_HANDLE_SCALE_JUMP
        ,
    ...
    m_esKF.resize(nKFs2);
    ComputeErrorFeature(
        &KF, m_CsKF[nKFs1], m_CsKF, m_ds, &m_esKF[nKFs1].first, nKFs1);
#endif
#ifdef LBA_DEBUG_VIEW
    if (g_viewer)
    {
...
void LocalBundleAdjustor::DeleteKeyFrame(const int iKF)
{
    ...
    const int id1 = m_iKF2d[iKF], id2 = id1 + Nd;
    {
        ...
        const ubyte udFlag = LBA_FLAG_TRACK_UPDATE_INFORMATION |
        ...
        for (int iZ = 0; iZ < NZ; ++iZ)
        {
            ...
            for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz)
            {
                ...
                daddST = AST.m_Sadd;
                AST = _KF.m_Axps[ix];
                const int iST1 = _KF.m_ix2ST[ix], iST2 = _KF.m_ix2ST[ix + 1],
                          Nst = iST2 - iST1;
                if (Nst > 1)
                {
    ...
    for (int iLF1 = 0; iLF1 < nLFs; ++iLF1)
    {
        ...
        for (int ik = 0; ik < Nk; ++ik)
        {
            ...
            if (z1)
            {
                ...
                for (int i = i2 - 1; i >= i1 && izms[i].m_iz1 >= iZ1->m_iz2;
                     --i)
                {
                    izms[i].m_iz1 -= Nz1;
                }
            ...
            if (z2)
            {
                ...
                for (int i = i2 - 1; i >= i1 && izms[i].m_iz2 >= iZ2->m_iz2;
                     --i)
                {
                    izms[i].m_iz2 -= Nz2;
                }
    ...
    if (m_Zp.Pose::Invalid())
    {
        ...
        else
        {
            ...
            {
                m_Zp.Pose::DebugSetMeasurement(
                    m_CsKFGT[iKFr], false, &m_CsLFGT[iLF].m_T);
            }
#endif
        }
    }
...
//#ifdef CFG_DEBUG
  ...
  if (m_debug) {
    ...
    for (int i1 = 0, i2 = 0, iKF = 0; i1 < N; i1 = i2)
    {
        iKF = static_cast<int>(
                  std::upper_bound(
                      m_iKF2d.begin() + iKF, m_iKF2d.end(), ids[i1]) -
                  m_iKF2d.begin()) -
              1;
...
#ifdef CFG_DEBUG
        UT_ASSERT((mcs[iKF] != 0) ==
                  UT::VectorExistFlag<ubyte>(mds.data() + id, Nx, 1));
#endif
        if (mcs[iKF])
        {
...
void LocalBundleAdjustor::UpdateCameras(const std::vector<ubyte>& ucs,
    const AlignedVector<Rigid3D>& CsKF1, const AlignedVector<Rigid3D>& CsKF2)
{
    Rigid3D TI;
    int iKFNearest = -1;
    const ubyte ucmFlag = LBA_FLAG_CAMERA_MOTION_UPDATE_ROTATION |
    ...
    for (int ic = 0; ic < nLFs; ++ic)
    {
        ...
        if (/*_iKFNearest == -1 || */ !(
            ucs[_iKFNearest] & GM_FLAG_FRAME_UPDATE_CAMERA))
        {
            continue;
        }
...
void LocalBundleAdjustor::SearchMatchingKeyFrames(FRM::Frame& F)
{
    ...
    const int nKFs2 =
        !F.m_Zs.empty() && F.m_Zs.back().m_iKF < nKFs1 ? nKFs1 : nKFs1 + 1;
    ...
    {
        const FRM::Measurement& Z = F.m_Zs[iZ];
        const GlobalMap::KeyFrame& KF =
            *(Z.m_iKF == nKFs1 ? (GlobalMap::KeyFrame*)&m_IKFs2.front()
        ...
        for (int i = 0; i < nKFsMatch; ++i)
        {
            ...
            const int _iz1 = _Z.m_iz1, _iz2 = _Z.m_iz2;
            int _iz;
            for (_iz = _iz1; _iz < _iz2 && !m_marksTmp1[_KF.m_zs[_iz].m_ix];
                 ++_iz)
                ;
            if (_iz < _iz2)
            {
...
void LocalBundleAdjustor::PushFeatureMeasurementMatchesFirst(
...
void LocalBundleAdjustor::PushFeatureMeasurementMatchesNext(
    const FRM::Frame& F1, const FRM::Frame& F2, const std::vector<int>& iKF2X,
    const std::vector<int>& iX2z2, MeasurementMatchLF& Zm)
{
    ubyte firstKF = 1;
...
void LocalBundleAdjustor::MarkFeatureMeasurements(
    const LocalFrame& LF, const int iKF, std::vector<int>& ix2z)
{
...
void LocalBundleAdjustor::MarkFeatureMeasurementsUpdateDepth(
    const FRM::Frame& F, std::vector<ubyte>& ucsKF, std::vector<ubyte>& uds)
{
    const int NZ = static_cast<int>(F.m_Zs.size());
    for (int iZ = 0; iZ < NZ; ++iZ)
    {
...
void LocalBundleAdjustor::DebugSetFeatureMeasurements(const Rigid3D& C,
    const AlignedVector<Rigid3D>& CsKF, const Depth::InverseGaussian& d,
    GlobalMap::Point* X)
{
    Rigid3D Tr[2];
    const int Nz = static_cast<int>(X->m_zs.size());
...
float LocalBundleAdjustor::ComputeImageMotion(const float z1, const Rigid3D& C1,
    const Rigid3D& C2, ubyte* first /* = NULL */)
{
    Rigid3D C12;
    ...
    if (fabs(C12.r20()) < fabs(C12.r21()))
    {
        ...
        SIMD::Cross012(
            C12.r_10_11_12_x(), C12.r_20_21_22_x(), C12.r_00_01_02_x());
    }
    else
    {
        ...
        SIMD::Cross012(
            C12.r_20_21_22_x(), C12.r_00_01_02_x(), C12.r_10_11_12_x());
    }
