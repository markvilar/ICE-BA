...
// CMakeLists. For Windows, we simply assume IBA runs with CVD and manually
// define IBA_WITH_CVD
#ifdef WIN32
#ifndef IBA_WITH_CVD
#define IBA_WITH_CVD
...
public:
    inline InputSequence(const Configurator& cfgor)
    {
        ...
        if (m_binary)
        {
            ...
            {
                m_filesImgRight = UT::FilesSearch(m_filesFtr,
                    UT::FileNameExtractDirectory(fileImgRight),
                    extImg);
            }
        }
        else
        {
            ...
            m_filesFtrRight =
                UT::FilesSearch(fileFtrRight, iStart, iStep, iEnd);
            if (m_filesFtrRight.size() != m_filesFtr.size())
            {
                m_filesFtrRight = UT::FilesSearch(m_filesFtr,
                    UT::FileNameExtractDirectory(fileFtrRight),
                    extFtr);
            }
            if (m_filesImgRight.size() != m_filesFtrRight.size())
            {
                m_filesImgRight = UT::FilesSearch(m_filesFtrRight,
                    UT::FileNameExtractDirectory(fileImgRight),
                    extImg);
            }
#ifdef CFG_DEBUG
        ...
        for (int iImg = 0; iImg < nImgs; ++iImg)
        {
            ...
            {
                t = UT::FileNameExtractSuffix<double>(m_filesFtr[iImg]) *
                    tFactor;
            }
        ...
        const std::string fileCalib =
            m_dir +
            cfgor.GetArgument("input_calibration_file", "calibration.txt");
        ...
        else
        {
            ...
            m_K.fishEye =
                cfgor.GetArgument("input_calibration_fish_eye", 0) != 0;
            if (!IBA::LoadCalibration(
                    fileCalib, m_K.Tu, &m_K.K, m_K.ba, m_K.bw /*, m_K.sa*/))
            {
                if ((m_K.K.fx = cfgor.GetArgument(
                         "input_camera_focal_x", 0.0f)) == 0.0f &&
                    (m_K.K.fx = cfgor.GetArgument(
                         "input_camera_fov_x", 0.0f)) != 0.0f)
                {
                    m_K.K.fx = Intrinsic::FovToFocal(m_K.w, m_K.K.fx);
                }
                if ((m_K.K.fy = cfgor.GetArgument(
                         "input_camera_focal_y", 0.0f)) == 0.0f &&
                    (m_K.K.fy = cfgor.GetArgument(
                         "input_camera_fov_y", 0.0f)) != 0.0f)
                {
                ...
                if ((m_K.K.cx = cfgor.GetArgument(
                         "input_camera_center_x", 0.0f)) == 0.0f)
                {
                    m_K.K.cx = (m_K.w - 1) * 0.5f;
                }
                if ((m_K.K.cy = cfgor.GetArgument(
                         "input_camera_center_y", 0.0f)) == 0.0f)
                {
                    m_K.K.cy = (m_K.h - 1) * 0.5f;
                }
                memset(m_K.K.ds, 0, sizeof(m_K.K.ds));
                const std::string r[3] = {
                    cfgor.GetArgument("input_imu_rotation_x"),
            ...
            const std::string fileCalibRight =
                m_dir + cfgor.GetArgument("input_calibration_file_right",
                            "calibration_right.txt");
            if (!IBA::LoadCalibration(fileCalibRight, m_K.Tr, &m_K.Kr))
            {
                m_K.Tr[0][0] = FLT_MAX;
                m_K.Tr[0][3] = cfgor.GetArgument(
                    "input_calibration_right_position_x", 0.0f);
                m_K.Tr[1][3] = cfgor.GetArgument(
                    "input_calibration_right_position_y", 0.0f);
                m_K.Tr[2][3] = cfgor.GetArgument(
                    "input_calibration_right_position_z", 0.0f);
            }
#endif
        }
        ...
        if (cfgor.GetArgument("tune_imu_calibration", 0))
        {
            ...
            const float bax = cfgor.GetArgument(
                "tune_imu_calibration_bias_acceleration_x", 0.0f);
            const float bay = cfgor.GetArgument(
                "tune_imu_calibration_bias_acceleration_y", 0.0f);
            const float baz = cfgor.GetArgument(
                "tune_imu_calibration_bias_acceleration_z", 0.0f);
            const float bwx = cfgor.GetArgument(
                "tune_imu_calibration_bias_gyroscope_x", 0.0f);
            const float bwy = cfgor.GetArgument(
                "tune_imu_calibration_bias_gyroscope_y", 0.0f);
            const float bwz = cfgor.GetArgument(
                "tune_imu_calibration_bias_gyroscope_z", 0.0f);
            Rotation3D dR;
        ...
        const bool left =
            cfgor.GetArgument("input_ground_truth_left_hand", 0) != 0;
        ...
        const bool origin =
            cfgor.GetArgument("input_ground_truth_origin", 1) != 0;
        const bool gravity =
        ...
        const std::string fileSensor =
            m_dir + cfgor.GetArgument(
                        "input_ground_truth_sensor", "calibration_sensor.txt");
        ...
        if (UT::FileNameExtractExtension(fileGT) == "dat")
        {
            ...
            if (Ru)
            {
                CameraTrajectory::TransformBias(
                    *Ru, m_CTGT.m_Cs, m_CTGT.m_ba, m_CTGT.m_bw);
            }
            if (origin)
            {
            ...
            !m_XsGT.empty())
        {
            const bool keyframeOnly =
                cfgor.GetArgument(
                    "input_ground_truth_depth_keyframe_only", 0) != 0;
#ifdef CFG_GROUND_TRUTH
            IBA::Solver solver;
            IBA::CurrentFrame CF;
            IBA::KeyFrame KF;
            ...
            {
                LoadCurrentFrame(iFrm, &CF, &KF, &solver);
                solver.PushDepthMeasurementsGT(
                    CF, KF.iFrm == -1 ? NULL : &KF, keyframeOnly);
                UT::Print("\r%d / %d = %f%%",
                    iFrm + 1,
                    nFrms,
                    UT::Percentage(iFrm + 1, nFrms));
        ...
        const std::string fileZp =
            m_dir + cfgor.GetArgument("input_relative_constraint",
        ...
        IBA::KeyFrame* KF, IBA::Solver* solver) const
    {
        if (m_binary)
        {
            ...
            if (KF->iFrm == -1)
            {
                if (iFrm < KF_FIRST_LOCAL_FRAMES && KF_MIN_FRAME_STEP > 0 &&
                    iFrm - solver->GetKeyFrameIndex(
                               solver->GetKeyFrames() - 1) ==
                        KF_MIN_FRAME_STEP)
                {
                    KF->iFrm = CF->iFrm;
                    KF->C = CF->C.C;
        ...
        //////////////////////////////////////////////////////////////////////////
        if (Xs &&
            (Xs->empty() || (Xs->size() < KF_MIN_FEATURE_SROUCES &&
...
#ifdef CFG_STEREO
        solver->LoadFeatures(
            m_filesFtrRight[iFrm], iFrm, &CF->zs, Xs, iFrms, 1);
#endif
        const int i1 = m_ius[iFrm], i2 = m_ius[iFrm + 1], N = i2 - i1;
        const IMU::Measurement* us = m_us.Data() + i1;
        CF->us.resize(N);
        ...
        if (Xs || iFrm < KF_FIRST_LOCAL_FRAMES ||
            ...
            (!m_kfs.empty() &&
                (m_kfs[iFrm] == 2 ||
                    m_kfs[iFrm] == 1 && KF_MIN_FRAME_STEP == -1)))
        {
            KF->iFrm = CF->iFrm;
            KF->C = CF->C.C;
            KF->zs = CF->zs;
    ...
    inline bool LoadRelativeConstraint(
        const int iFrm, IBA::RelativeConstraint* Z) const
    {
    ...
    inline bool LoadDeleteKeyFrames(
        const int iFrm, std::vector<int>* iFrms) const
    {
        return IBA::LoadKeyFrames(m_filesDKF[iFrm], iFrms);
    }
    inline bool LoadDeleteMapPoints(
        const int iFrm, std::vector<int>* idxs) const
    {
    ...
    static inline void ConvertCameras(
        const AlignedVector<Camera>& Cs, std::vector<IBA::CameraIMUState>* Xs)
    {
        const int N = Cs.Size();
        Xs->resize(N);
        for (int i = 0; i < N; ++i)
    ...
    static inline void ConvertCameras(
        const std::vector<IBA::CameraIMUState>& Xs, AlignedVector<Camera>* Cs,
        LA::AlignedVector3f* ba = NULL, LA::AlignedVector3f* bw = NULL)
    {
