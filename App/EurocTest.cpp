...
using std::string;
using std::vector;
DEFINE_string(imgs_folder, "",
    "The folder containing l and r folders, and the calib.yaml");
...
DEFINE_int32(
    ft_len, 125, "The feature track length threshold when dropout kicks in");
DEFINE_double(
    ft_droprate, 0.05, "The drop out rate when a feature track exceeds ft_len");
DEFINE_bool(show_feat_only, false, "wether or not show detection results only");
DEFINE_int32(fast_thresh, 10,
    "FAST feature threshold (only meaningful if use_fast=true)");
DEFINE_double(min_feature_distance_over_baseline_ratio, 4,
    "Used for slave image feature detection");
DEFINE_double(max_feature_distance_over_baseline_ratio, 3000,
    "Used for slave image feature detection");
DEFINE_string(iba_param_path, "", "iba parameters path");
DEFINE_string(
    gba_camera_save_path, "", "Save the camera states to when finished");
...
size_t load_image_data(const string& image_folder,
    std::vector<string>& limg_name, std::vector<string>& rimg_name)
{
    ...
    while (getline(limg_file, line))
    {
        ...
        {
            bool is_exist =
                boost::filesystem::exists(r_img_prefix + time + ".png");
            if (i == 0 && is_exist)
            {
...
size_t load_imu_data(const string& imu_file_str,
    std::list<XP::ImuData>* imu_samples_ptr, uint64_t& offset_ts_ns)
{
...
void load_asl_calib(const std::string& asl_path, XP::DuoCalibParam& calib_param)
{
    ...
    std::vector<float> v_float =
        cam0_calib["intrinsics"].as<std::vector<float>>();

    calib_param.Camera.cv_camK_lr[0] << v_float[0], 0, v_float[2], 0,
        v_float[1], v_float[3], 0, 0, 1;

    calib_param.Camera.cameraK_lr[0] << v_float[0], 0, v_float[2], 0,
        v_float[1], v_float[3], 0, 0, 1;

    v_float = cam1_calib["intrinsics"].as<std::vector<float>>();

    calib_param.Camera.cv_camK_lr[1] << v_float[0], 0, v_float[2], 0,
        v_float[1], v_float[3], 0, 0, 1;

    calib_param.Camera.cameraK_lr[1] << v_float[0], 0, v_float[2], 0,
        v_float[1], v_float[3], 0, 0, 1;

    // Distortion coefficients.
    std::vector<double> v_double =
        cam0_calib["distortion_coefficients"].as<std::vector<double>>();
    calib_param.Camera.cv_dist_coeff_lr[0] =
        (cv::Mat_<float>(8, 1) << static_cast<float>(v_double[0]),
            static_cast<float>(v_double[1]),
            static_cast<float>(v_double[2]),
            static_cast<float>(v_double[3]),
            0.0,
            0.0,
            0.0,
            0.0);
    ...
    Eigen::Matrix4d b_t_c0 =
        Eigen::Map<Eigen::Matrix<double, 4, 4, Eigen::RowMajor>>(&v_double[0]);
...
float get_timestamp_from_img_name(const string& img_name, uint64_t offset_ns)
{
...
bool convert_to_asl_timestamp(
    const string& file_in, const string& file_out, uint64_t offset_ns)
{
...
template <typename T> void InitPOD(T& t) { memset(&t, 0, sizeof(t)); }

bool create_iba_frame(const vector<cv::KeyPoint>& kps_l,
    const vector<cv::KeyPoint>& kps_r, const vector<XP::ImuData>& imu_samples,
    const float rig_time, IBA::CurrentFrame* ptrCF, IBA::KeyFrame* ptrKF)
{
    CHECK(std::is_sorted(kps_l.begin(), kps_l.end(), cmp_by_class_id));
    CHECK(std::is_sorted(kps_r.begin(), kps_r.end(), cmp_by_class_id));
    CHECK(std::includes(kps_l.begin(),
        kps_l.end(),
        kps_r.begin(),
        kps_r.end(),
        cmp_by_class_id));
    ...
    CF.iFrm = iba_iFrm;
    // Needed to ensure the dumped frame deterministic even for unused field.
    InitPOD(CF.C);
    CF.C.C.R[0][0] = CF.C.v[0] = CF.C.ba[0] = CF.C.bw[0] = FLT_MAX;
    // MapPointMeasurement, process in ascending class id, left camera to
    // right. Note the right keypoints is a subset of the left ones.
    IBA::MapPointMeasurement mp_mea;
    InitPOD(mp_mea);
    mp_mea.x.S[0][0] = mp_mea.x.S[1][1] = 1.f;
    mp_mea.x.S[0][1] = mp_mea.x.S[1][0] = 0.f;
    for (; kp_it_l != kps_l.cend() && kp_it_l->class_id <= last_added_point_id;
         ++kp_it_l)
    {
    ...
    std::transform(imu_samples.begin(),
        imu_samples.end(),
        std::back_inserter(CF.us),
        XP::to_iba_imu);
    CF.t = rig_time;
    CF.d = kUnknownDepth;
    bool need_new_kf =
        std::distance(kp_it_l, kps_l.end()) >= 20 || CF.zs.size() < 20;
    ...
    else
    {
        ...
        for (; kp_it_l != kps_l.cend(); ++kp_it_l)
        {
            ...
            if (kp_it_r != kps_r.cend() &&
                kp_it_r->class_id == kp_it_l->class_id)
            {
                mp_mea.x.x[0] = kp_it_r->pt.x;
                mp_mea.x.x[1] = kp_it_r->pt.y;
                mp_mea.right = true;
            ...
            {
                LOG(WARNING) << "add new feature point " << kp_it_l->class_id
                             << " only found in left image";
            }
            KF.Xs.push_back(mp);
        }
...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < limg_name.size(); i++)
    {
        ...
        slave_img_file_paths.push_back(
            FLAGS_imgs_folder + "/mav0/cam1/data/" + rimg_name[i]);
        iba_dat_file_paths.push_back(
            FLAGS_imgs_folder + "/dat/" + limg_name[i] + ".dat");
    }
    ...
    while (FLAGS_start_idx < FLAGS_end_idx &&
    ...
    const Eigen::Matrix4f T_Cl_Cr =
        duo_calib_param.Camera.D_T_C_lr[0].inverse() *
    ...
    {
        IBA::SaveCalibration(FLAGS_imgs_folder + "/calibration.dat",
            to_iba_calibration(duo_calib_param));
    }
    ...
    {
        VLOG(0) << " start detection at ts = "
                << fs::path(img_file_paths[it_img]).stem().string();
        auto read_img_start = std::chrono::high_resolution_clock::now();
        cv::Mat img_in_raw;
        img_in_raw =
            cv::imread(img_file_paths[it_img], CV_LOAD_IMAGE_GRAYSCALE);
        ...
        const float time_stamp =
            get_timestamp_from_img_name(img_file_paths[it_img], offset_ts_ns);
        ...
        VLOG(1) << "img ts prev -> curr " << prev_time_stamp << " -> "
                << time_stamp;
        if (imu_meas.size() > 0)
        {
        ...
        if (!slave_img_file_paths.empty())
        {
            ...
            {
                cv::Mat slave_img_in;
                slave_img_in = cv::imread(
                    slave_img_file_paths[it_img], CV_LOAD_IMAGE_GRAYSCALE);
        ...
        {
            CHECK(it_img >= 1);
            VLOG(1) << "pre_image_key_points.size(): "
                    << pre_image_key_points.size();
            const int request_feat_num = FLAGS_max_num_per_grid *
                                         FLAGS_grid_row_num *
                                         FLAGS_grid_col_num;
            ...
            if (imu_meas.size() > 1)
            {
                ...
                XP::XpQuaternion I_new_q_I_old;
                // The rotation between the new {I} and old {I}
            ...
            VLOG(1) << "after OF key_pnts.size(): " << key_pnts.size()
                    << " requested # "
                    << FLAGS_max_num_per_grid * FLAGS_grid_row_num *
                           FLAGS_grid_col_num;
        }
        else
        {
        ...
        if (slave_img_smooth.rows > 0)
        {
            ...
            auto det_slave_img_start =
                std::chrono::high_resolution_clock::now();
            slave_img_feat_propagator.PropagateFeatures(slave_img_smooth,
                img_in_smooth,
                key_pnts,
                T_Cl_Cr,
                &key_pnts_slave,
                &orb_feat_slave,
                false);
            VLOG(1) << "detect slave key_pnts.size(): " << key_pnts_slave.size()
                    << " takes "
        ...
        std::sort(
            key_pnts_slave.begin(), key_pnts_slave.end(), cmp_by_class_id);

        // Push to IBA.
        IBA::CurrentFrame CF;
        IBA::KeyFrame KF;
        create_iba_frame(
            key_pnts, key_pnts_slave, imu_meas, time_stamp, &CF, &KF);
    ...
    convert_to_asl_timestamp(
        temp_file, FLAGS_gba_camera_save_path, offset_ts_ns);
    return 0;
}
