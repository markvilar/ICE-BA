...
public:
    inline Measurement() {}
    inline Measurement(
        const int iKF, const int ik, const int iz1, const int iz2)
        : m_iKF(iKF), m_ik(ik), m_iz1(iz1), m_iz2(iz2)
    {
    }
    inline bool operator==(const Measurement& Z) const
    {
        return m_iKF == Z.m_iKF && m_ik == Z.m_ik && m_iz1 == Z.m_iz1 &&
               m_iz2 == Z.m_iz2;
    }
...
public:
    ...
    inline void DeleteKeyFrame(const int iKF,
        const std::vector<Measurement>::iterator* iZ = NULL,
        const std::vector<int>::iterator* ik = NULL)
    {
        const std::vector<Measurement>::iterator _iZ =
            iZ ? *iZ : std::lower_bound(m_Zs.begin(), m_Zs.end(), iKF);
        const std::vector<int>::iterator _ik =
            ik ? *ik
        ...
        if (_iZ != m_Zs.end())
        {
            ...
            if (Z)
            {
                ...
                for (std::vector<Measurement>::iterator jZ = _iZ + 1;
                     jZ != m_Zs.end();
                     ++jZ)
                {
                    jZ->m_iz1 -= Nz;
                ...
                m_zs.erase(
                    m_zs.begin() + _iZ->m_iz1, m_zs.begin() + _iZ->m_iz2);
            }
            const bool k = _ik != m_iKFsMatch.end() && *_ik == iKF;
            for (std::vector<Measurement>::iterator jZ = Z ? _iZ + 1 : _iZ;
                 jZ != m_Zs.end();
                 ++jZ)
            {
                --jZ->m_iKF;
        ...
        const int iKF, const std::vector<int>::iterator* ik = NULL)
    {
        ...
        const std::vector<Measurement>::iterator iZ =
    ...
    inline void DeleteMatchKeyFrame(
        const int iKF, const std::vector<int>::iterator* ik = NULL)
    {
        const std::vector<int>::iterator _ik =
            ik ? *ik
    ...
    inline void DeleteFeatureMeasurementsPrepare(
        const std::vector<int>& izsDel, std::vector<int>* izs) const
    {
    ...
    inline void PushFrameMeasurement(
        const int iKF, const std::vector<FTR::Measurement>& zs)
    {
    ...
    inline void PushFeatureMeasurements(const int iKF,
        const std::vector<FTR::Measurement>& zs, int* iZ, int* iz)
    {
        ...
        else
        {
            ...
            *iZ = static_cast<int>(_iZ - m_Zs.begin());
            ...
            {
                const std::vector<int>::iterator ik = std::lower_bound(
                    m_iKFsMatch.begin(), m_iKFsMatch.begin() + _iZ->m_ik, iKF);
                const int _ik = static_cast<int>(ik - m_iKFsMatch.begin());
                if (ik == m_iKFsMatch.end() || *ik != iKF)
                {
                    m_iKFsMatch.insert(ik, iKF);
                    for (std::vector<Measurement>::iterator jZ = _iZ;
                         jZ != m_Zs.end();
                         ++jZ)
                    {
                        ++jZ->m_ik;
    ...
    {
#ifdef CFG_DEBUG
        UT_ASSERT(!m_Zs.empty() && !m_iKFsMatch.empty() &&
                  m_Zs.back().m_iKF == m_iKFsMatch.back() &&
    ...
    inline int SearchFeatureMeasurement(const int iKF, const int ix) const
    {
        ...
        const int iz = static_cast<int>(
            std::lower_bound(
                m_zs.begin() + Z.m_iz1, m_zs.begin() + Z.m_iz2, ix) -
            m_zs.begin());
    ...
    static inline void SearchFeatureMeasurementMatches(const Frame& F1,
        const Frame& F2, std::vector<FTR::Measurement::Match>& izms,
        const int iKF = -1)
    {
        izms.resize(0);
            ...
            for (int iz1 = iz11; iz1 < iz12; ++iz1)
            {
                ...
                if (iz2 != iz22 && iz2->m_ix == ix)
                {
                    izms.push_back(FTR::Measurement::Match(
                        iz1, int(iz2 - F2.m_zs.begin())));
                }
            }
        }
    }

    static inline bool HasFeatureMeasurementMatch(
...
public:
    ...
    inline void InsertFeatureMeasurement1(
        const int ik, const int iz1, const int Nz1)
    {
    ...
    inline void InsertFeatureMeasurement2(
        const int ik, const int iz2, const int Nz2)
    {
        ...
        AlignedVector<float>* work)
    {
        const std::vector<FTR::Measurement::Match>::iterator i =
            std::lower_bound(m_izms.begin() + m_ik2zm[ik],
                m_izms.begin() + m_ik2zm[ik + 1],
                izms.front().m_iz2);
        const int Nk = static_cast<int>(m_ik2zm.size()) - 1;
        const int Nzm = static_cast<int>(izms.size());
    ...
    inline void PushFeatureMeasurementMatches(
        const std::vector<FTR::Measurement::Match>& izms, ubyte* first = NULL)
    {
        const int Nzm1 = m_ik2zm.back();
        const int Nzm = static_cast<int>(izms.size());
    ...
    inline void DeleteFeatureMeasurementMatches(
        const int ik, const int i1, const int i2, const ubyte* ms = nullptr)
    {
        const int Nzm = i2 - i1;
        if (Nzm == 0)
        {
        ...
        for (int jk = ik + 1; jk <= m_SMczms.Size(); ++jk)
        {
            m_ik2zm[jk] -= Nzm;
        }
    ...
    inline void DeleteFeatureMeasurementMatches(const int ik, const int i1,
        const int i2, const std::vector<int>& izs1,
        const std::vector<int>& izs2, std::vector<ubyte>* ms = nullptr)
    {
    ...
    inline void AssertConsistency(const int ik, const Frame& F1,
        const Frame& F2, std::vector<FTR::Measurement::Match>& izmsTmp) const
    {
