...
ubyte LocalMap::IBA_Synchronize(const int iFrm, std::list<CameraLF>& CsLF,
    std::vector<CameraKF>& CsKF, std::vector<Depth::InverseGaussian>& ds,
    std::vector<ubyte>& uds)
{
    ...
    if (m_Uc)
    {
        ...
        {
            CsLF = m_CsLF;
            for (std::list<CameraLF>::iterator C = m_CsLF.begin();
                 C != m_CsLF.end();
                 ++C)
            {
...
void LocalMap::AssertConsistency()
{
    ...
    UT_ASSERT(
        m_iKF2d[0] == 0 && m_iKF2d[nKFs] == static_cast<int>(m_ds.size()));
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        const int id1 = m_iKF2d[iKF], id2 = m_iKF2d[iKF + 1];
        UT_ASSERT(
            (m_CsKF[iKF].m_uc & LM_FLAG_FRAME_UPDATE_DEPTH) != 0 ==
            UT::VectorExistFlag<ubyte>(
                m_uds.data() + id1, id2 - id1, LM_FLAG_TRACK_UPDATE_DEPTH));
    }
    MT_READ_LOCK_END(m_MT, MT_TASK_NONE, MT_TASK_NONE);
}
