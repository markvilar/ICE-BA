...
)
{
...
#ifdef CFG_STEREO
    const EigenVector3f e_br =
        br ? EigenVector3f(*br) : EigenVector3f(0.0f, 0.0f, 0.0f);
#endif
    ...
    const EigenMatrix3x3f e_JXr1 =
        EigenMatrix3x3f(-e_R2 * EigenSkewSymmetricMatrix(e_w1));
    const EigenMatrix3x3f e_JXp1 = e_R2;
    const EigenVector3f e_w2 = EigenVector3f(e_w1 + e_p1 - e_p2);
    const EigenMatrix3x3f e_JXr2 =
        EigenMatrix3x3f(e_R2 * EigenSkewSymmetricMatrix(e_w2));
    ...
    e_JxX_z2(1, 1) = 1.0f;
    e_JxX_z2(1, 2) = -e_x2.y();
    const EigenVector2f e_Jxd =
        EigenVector2f(e_JxX_z2 * e_JXd_d1 * (e_d2 * e_z1));
    ...
    const EigenRotation3D e_R2GT =
        EigenMatrix3x3f(e_R2 * EigenRotation3D(e_dr2));
    const EigenPoint3D e_p2GT = EigenVector3f(e_p2 + e_dp2);
    const EigenPoint3D e_X1GT = EigenVector3f(e_x1 * e_z1GT);
    const EigenPoint3D e_X2GT = EigenVector3f(
        e_R2GT * EigenVector3f(e_R1GT.GetTranspose() * e_X1GT + e_p1GT - e_p2GT)
#ifdef CFG_STEREO
        + e_br
#endif
    );
    ...
    const EigenVector2f e_ex2 =
        EigenVector2f(e_ex1 + e_Jxd * e_dd + e_Jxr1 * e_dr1 + e_Jxp1 * e_dp1 +
                      e_Jxr2 * e_dr2 + e_Jxp2 * e_dp2);
    ...
    const Point3D& br, const Depth::InverseGaussian& d, const Source& x)
{
    ...
    e_JxX_zr(1, 2) = -e_xr.y();
    const EigenVector2f e_Jxd =
        EigenVector2f(e_JxX_zr * e_JXd_d * (e_dr * e_z));
