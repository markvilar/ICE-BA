...
#ifdef CFG_STEREO
        return m_x == x.m_x && m_xr == x.m_xr && m_Wr == x.m_Wr;
#else
        return m_x == x.m_x;
#endif
    }
...
public:
    ...
    public:
        ...
        inline void Set(const int iz1, const int iz2)
        {
            m_iz1 = iz1;
            m_iz2 = iz2;
        }
...
public:
    ...
    {
#ifdef CFG_STEREO
        return m_z == x.m_x && m_zr == x.m_xr && m_Wr == x.m_Wr;
#else
        return m_z == x.m_x;
#endif
    }
    ...
    inline void Set(
        const int ix, const Point2D& z, const LA::SymmetricMatrix2x2f& W
#ifdef CFG_STEREO
        ,
...
public:
    ...
    inline ESError(const Intrinsic& K, const LA::Vector2f& ex)
        : LA::Vector2f(K.fx() * ex.x(), K.fy() * ex.y())
    {
    }
...
public:
    ...
    inline ESIndex(
        const int ixFrm, const int ix, const int izFrm = -1, const int iz = -1)
...
{
public:
    inline void Initialize(const bool r = true)
    {
        m_ESx.Initialize(r);
        m_ESd.Initialize(r);
    }
    ...
    inline void Accumulate(const Intrinsic& K, const LA::Vector2f& ex,
        const float F, const ESIndex idx, const bool r = true)
    {
        m_ESx.Accumulate(ESError(K, ex), F, idx, r);
    }

    inline void Accumulate(
        const float ed, const float F, const ESIndex idx, const bool r = true)
    {
        m_ESd.Accumulate(ed, F, idx, r);
    }

    inline float Total() const { return m_ESx.m_SF + m_ESd.m_SF; }

    inline void Print(
        const std::string str = "", const bool l = true, const int r = 1) const
    {
        if (m_ESx.Valid())
        {
        ...
        if (m_ESd.Valid())
        {
            m_ESd.Print(m_ESx.Valid() ? std::string(str.size(), ' ') + "   + "
                                      : str + "ex = ",
                true,
                l,
                true,
...
{
public:
    static inline DD Get(const float a, const float b)
    {
        DD _a;
        _a.Set(a, b);
        return _a;
    ...
    static inline DD Get(const Depth::Prior::Factor& A)
    {
        DD _a;
        _a.Set(A.m_a, A.m_b);
        return _a;
    ...
    inline void Set(const float a, const float b)
    {
        m_a = a;
        m_b = b;
    }
    ...
    inline void operator+=(const DD& a)
    {
        m_a = a.m_a + m_a;
        m_b = a.m_b + m_b;
    }
    ...
    inline DD operator-(const DD& b) const
    {
        DD _amb;
        amb(*this, b, _amb);
        return _amb;
    ...
    inline DD operator*(const float s) const
    {
        DD sa;
        sa.Set(s * m_a, s * m_b);
        return sa;
    ...
    inline void GetMinus(DD& a) const
    {
        a.m_a = -m_a;
        a.m_b = -m_b;
    }
    ...
    inline bool AssertEqual(const DD& a, const int verbose = 1,
        const std::string str = "", const float epsAbs = 0.0f,
        const float epsRel = 0.0f) const
    {
        if (UT::AssertEqual(
                m_a, a.m_a, verbose, str + ".m_a", epsAbs, epsRel) &&
...
{
public:
    static inline DC Get(const float* a)
    {
        DC _a;
        _a.Set(a);
        return _a;
    }
    inline bool AssertEqual(const DC& a, const int verbose = 1,
        const std::string str = "", const float epsAbs = 0.0f,
        const float epsRel = 0.0f) const
    {
...
public:
    ...
    {
        return m_adc == a.m_adc && m_add == a.m_add;
    }
    ...
    inline bool AssertEqual(const DDC& a, const int verbose = 1,
        const std::string str = "", const float epsAbs = 0.0f,
        const float epsRel = 0.0f) const
    {
        return m_adc.AssertEqual(
                   a.m_adc, verbose, str + ".m_adc", epsAbs, epsRel) &&
               m_add.AssertEqual(
                   a.m_add, verbose, str + ".m_add", epsAbs, epsRel);
    }
    inline bool AssertZero(
        const int verbose = 1, const std::string str = "") const
    {
    ...
    static inline void aTb(
        const float* a, const DDC& b, Camera::Factor::Unitary::CC& aTb)
    {
...
public:
    ...
    public:
        ...
        inline void Accumulate(const ErrorJacobian::D& Je, const float w,
        ...
        inline void Set(const ErrorJacobian::D& Je, const float w,
            const LA::SymmetricMatrix2x2f& W)
        {
            LA::SymmetricMatrix2x2f::Ab(W, Je.m_Jd, m_WJ);
            m_WJ *= w;
...
public:
    ...
    inline A operator*(const float s) const
    {
        A _A;
        _A.m_Sadd = m_Sadd * s;
        return _A;
...
public:
    ...
    inline bool AssertEqual(const A1& A, const int verbose = 1,
        const std::string str = "", const float epsAbs = 0.0f,
        const float epsRel = 0.0f) const
    {
        return m_adcz.AssertEqual(
            A.m_adcz, verbose, str + ".m_adcz", epsAbs, epsRel);
    }
...
public:
    inline void Initialize() { m_A.MakeZero(); }
    inline void Accumulate(const ErrorJacobian::DCZ& Je, const float w,
        const LA::SymmetricMatrix2x2f& W)
    {
    ...
    inline void Set(const ErrorJacobian::DCZ& Je, const float w,
        const LA::SymmetricMatrix2x2f& W)
    {
        m_J.Set(Je.m_Jd, Je.m_Jcz);
        m_Je.Set(m_J, Je.m_e);
...
public:
    inline void Initialize() { m_A.MakeZero(); }
    inline void Accumulate(const ErrorJacobian::DCXZ& Je, const float w,
        const LA::SymmetricMatrix2x2f& W)
    {
    ...
    inline void Set(const ErrorJacobian::DCXZ& Je, const float w,
        const LA::SymmetricMatrix2x2f& W)
    {
        m_J.Set(Je.m_Jd, Je.m_Jcx, Je.m_Jcz);
        m_Je.Set(m_J, Je.m_e);
...
inline void Marginalize(const xp128f& mdd, const Source::M1& Mx, const A1& Az,
    M1* Mz1, M2* Mz2, LA::ProductVector6f* adcz)
{
#ifdef CFG_DEBUG
    ...
    const xp128f t = xp128f::get(Mz1->m_adcz.v4(),
        Mz1->m_adcz.v5(),
        Mx.m_mdx.m_add.m_a,
        Mx.m_mdx.m_add.m_b);
...
inline void GetError(const Rigid3D& T12, const Source& x1,
    const Depth::InverseGaussian& d1, const Point2D& z2, LA::Vector2f& e2)
{
#ifdef CFG_DEBUG
    UT_ASSERT(z2.Valid());
#endif
...
inline void GetError(const Rigid3D* T12, const Source& x1,
    const Depth::InverseGaussian& d1, const Measurement& z2, Error& e2)
{
#ifdef CFG_STEREO
    if (z2.m_z.Valid())
    {
...
inline void GetError(
    const ErrorJacobian::D& Je, const float xd, LA::Vector2f& e)
{
    e = Je.m_e;
    e += Je.m_Jd * xd;
}

inline void GetError(const ErrorJacobian::DCZ& Je,
...
inline void GetError(const ErrorJacobian::DCXZ& Je,
...
inline void GetError(const Factor::FixSource::L& L,
...
inline void GetErrorJacobian(
    const Rigid3D& T12, const Source& x1, const Depth::InverseGaussian& d1,
...
)
{
    ...
    if (vp)
    {
        ...
        Je2.m_Jcz.m_00_01_02_03() =
            _d2 * (_y2 * T2.r_20_21_22_x() - T2.r_10_11_12_x());
        Je2.m_Jcz.m_00_01_02_03().vstore_unalign(Je2.m_Jcz[1]);
        Je2.m_Jcz.m_00_01_02_03() =
            _d2 * (_x2 * T2.r_20_21_22_x() - T2.r_00_01_02_x());
#ifdef CFG_STEREO
        if (br)
        {
            const LA::AlignedVector3f bd2 = *br * _d2;
            const float x = Je2.m_e.x() - bd2.x(), y = Je2.m_e.y() - bd2.y(),
                        z = 1.0f - bd2.z();
...
)
{
    ...
    if (vp)
    {
        ...
        Je2.m_Jcx.m_00_01_02_03() =
            _d2 * (T2.r_00_01_02_x() - _x2 * T2.r_20_21_22_x());
        Je2.m_Jcz.m_00_01_02_03() =
            _d2 * (T2.r_10_11_12_x() - _y2 * T2.r_20_21_22_x());
    ...
    const float r2Max = FLT_MAX)
{
    ...
    if (z2.m_z.Valid())
    {
        ...
        const float r2 =
            LA::SymmetricMatrix2x2f::MahalanobisDistance(z2.m_W, L->m_Je.m_e);
        if (r2 > r2Max)
        {
            L->m_w = 0.0f;
        }
    ...
    if (z2.m_zr.Valid())
    {
        ...
        const float r2 =
            LA::SymmetricMatrix2x2f::MahalanobisDistance(z2.m_Wr, L->m_Jer.m_e);
        if (r2 > r2Max)
        {
            L->m_wr = 0.0f;
        }
...
inline void GetFactor(const float w, const Rigid3D* T12, const Source& x1,
    ...
    const float r2Max = FLT_MAX)
{
    GetFactor<ME_FUNCTION, Factor::Depth, Factor::Depth::U>(w,
        T12,
        x1,
        d1,
        T2,
...
inline void GetFactor(const float w, const Rigid3D* T12, const Source& x1,
    ...
    const float r2Max = FLT_MAX)
{
    GetFactor<ME_FUNCTION, Factor::FixSource::L, Factor::FixSource::U>(w,
        T12,
        x1,
        d1,
        T2,
...
inline void GetFactor(const float w, const Rigid3D* T12, const Source& x1,
    ...
    const float r2Max = FLT_MAX)
{
    GetFactor<ME_FUNCTION, Factor::Full::L, Factor::Full::U>(w,
        T12,
        x1,
        d1,
        T2,
...
inline float GetCost(
    const LINEARIZATION& L, const Measurement& z, const Error& e)
{
#ifdef CFG_STEREO
    float F = 0.0f;
    if (z.m_z.Valid())
    ...
    {
        const float r2 =
            LA::SymmetricMatrix2x2f::MahalanobisDistance(z.m_Wr, e.m_er);
        F += L.m_wr * r2;
    }
#else
...
inline float GetCost(
    const Factor::Depth& A, const Measurement& z, const float xd, Error& e)
{
    GetError(A, xd, e);
...
inline void GetReduction(const Factor::Depth& A, const Rigid3D* T12,
    const Source& x1, const Depth::InverseGaussian& d1, const Measurement& z2,
...
inline void GetReduction(const Factor::FixSource::L& L, const Rigid3D* T12,
    const Source& x1, const Depth::InverseGaussian& d1, const Measurement& z2,
    const LA::ProductVector6f* xcz, const float* xd, Reduction& Ra,
    Reduction& Rp)
{
    GetError(T12, x1, d1, z2, Ra.m_e);
...
inline void GetReduction(const Factor::Full::L& L, const Rigid3D* T12,
    const Source& x1, const Depth::InverseGaussian& d1, const Measurement& z2,
    const LA::ProductVector6f* xcx, const LA::ProductVector6f* xcz,
    const float* xd, Reduction& Ra, Reduction& Rp)
{
...
inline void DebugSetMeasurement(
    const Point3D& br, const Depth::InverseGaussian& d, Source& x)
{
...
inline void GetError(const Point3D& br, const Depth::InverseGaussian& d,
    const Source& x, LA::Vector2f& e)
{
...
inline void GetErrorJacobian(const Point3D& br, const Depth::InverseGaussian& d,
    const Source& x, ErrorJacobian::D& Je)
{
...
template <int ME_FUNCTION>
inline void GetFactor(const float w, const Point3D& br,
    const Depth::InverseGaussian& d, const Source& x, Factor::Stereo* A,
    Factor::Stereo::U* U)
{
    GetErrorJacobian(br, d, x, A->m_Je);
    const float r2 =
        LA::SymmetricMatrix2x2f::MahalanobisDistance(x.m_Wr, A->m_Je.m_e);
...
inline float GetCost(
    const Factor::Stereo& A, const Source& x, const LA::Vector2f& e)
{
    return A.m_w * LA::SymmetricMatrix2x2f::MahalanobisDistance(x.m_Wr, e);
}

inline float GetCost(
    const Factor::Stereo& A, const Source& x, const float xd, LA::Vector2f& e)
{
    GetError(A, xd, e);
...
inline void GetReduction(const Factor::Stereo& A, const Point3D& br,
    const Depth::InverseGaussian& d, const Source& x, const float xd,
...
public:
    ...
    inline void AssertEqual(const ErrorJacobian::D& Je, const int verbose = 1,
        const std::string str = "", const float epsAbs = 0.0f,
        const float epsRel = 0.0f) const
    {
        m_Jd.AssertEqual(Je.m_Jd, verbose, str + ".m_Jd", epsAbs, epsRel);
    ...
    inline void AssertEqual(const ErrorJacobian::DCZ& Je, const int verbose = 1,
        const std::string str = "", const float epsAbs = 0.0f,
        const float epsRel = 0.0f) const
    {
        m_Jd.AssertEqual(Je.m_Jd, verbose, str + ".m_Jd", epsAbs, epsRel);
...
public:
    ...
    public:
        ...
        inline DC(const Eigen::Matrix<float, 1, 6>& a)
            : Eigen::Matrix<float, 1, 6>(a)
        {
        }
        inline DC(const Factor::DC& a)
            : Eigen::Matrix<float, 1, 6>(EigenVector6f(a).transpose())
        {
        }

        inline void operator=(const Eigen::Matrix<float, 1, 6>& a)
        {
            *((Eigen::Matrix<float, 1, 6>*)this) = a;
        }

        inline void operator=(const Factor::DC& a)
        {
            *this = EigenVector6f(a).transpose();
        }

        inline void operator+=(const Factor::DC& a)
        {
            *((Eigen::Matrix<float, 1, 6>*)this) += DC(a);
        }

        inline void operator+=(const DC& a)
        {
            *((Eigen::Matrix<float, 1, 6>*)this) += a;
        }

        EigenVector6f GetTranspose() const { return EigenVector6f(*this); }
        LA::Vector6f GetVector6f() const
        {
            return GetTranspose().GetVector6f();
        }
    };
    ...
    public:
        ...
        inline bool AssertEqual(const Factor::DDC& a, const int verbose = 1,
            const std::string str = "", const float epsAbs = 0.0f,
            const float epsRel = 0.0f) const
        {
            const auto x = m_add.AssertEqual(
                a.m_add, verbose, str + ".m_add", epsAbs, epsRel) const auto y =
                Factor::DC::Get(m_adc.GetVector6f())
                    .AssertEqual(
                        a.m_adc, verbose, str + ".m_adc", epsAbs, epsRel);
            return a && b;
        }
    ...
    public:
        ...
        inline void AssertEqual(const Factor::Stereo& A, const int verbose = 1,
            const std::string str = "", const float epsAbs = 0.0f,
            const float epsRel = 0.0f) const
        {
            UT::AssertEqual(m_F, A.m_F, verbose, str + ".m_F", epsAbs, epsRel);
            UT::AssertEqual(
                m_add, A.m_add.m_a, verbose, str + ".m_add", epsAbs, epsRel);
            UT::AssertEqual(
                m_bd, A.m_add.m_b, verbose, str + ".m_bd", epsAbs, epsRel);
        }
...
public:
    inline EigenFactor() {}
    inline EigenFactor(
        const float F, const Eigen::Matrix<float, 13, 14, Eigen::RowMajor>& A)
    {
        m_F = F;
        m_add = A(0, 0);
    ...
    inline void Set(const Factor::FixSource::L& L,
        const Factor::FixSource::A1& A1, const Factor::FixSource::A2& A2)
    {
        m_F = L.m_F;
        m_add = A2.m_add.m_a;
        m_adcx.setZero();
    ...
    inline void AssertEqual(const Factor::Depth& A, const int verbose = 1,
        const std::string str = "", const float epsAbs = 0.0f,
        const float epsRel = 0.0f) const
    {
        UT::AssertEqual(m_F, A.m_F, verbose, str + ".m_F");
        Factor::DD::Get(m_add, m_bd)
            .AssertEqual(A.m_add, verbose, str + ".m_add", epsAbs, epsRel);
        Factor::DC::Get(m_adcx.GetVector6f())
            .AssertZero(verbose, str + ".m_adcx");
        Factor::DC::Get(m_adcz.GetVector6f())
            .AssertZero(verbose, str + ".m_adcz");
        Camera::Factor::Unitary::CC::Get(
            m_Acxx.GetSymmetricMatrix6x6f(), m_bcx.GetVector6f())
            .AssertZero(verbose, str + ".m_Acxx");
        Camera::Factor::Binary::CC(m_Acxz.GetAlignedMatrix6x6f())
            .AssertZero(verbose, str + ".m_Acxz");
        Camera::Factor::Unitary::CC::Get(
            m_Aczz.GetSymmetricMatrix6x6f(), m_bcz.GetVector6f())
            .AssertZero(verbose, str + ".m_Aczz");
    }

    inline void AssertEqual(const Factor::FixSource::L& L,
        const Factor::FixSource::A1& A1, const Factor::FixSource::A2& A2,
        const int verbose = 1, const std::string str = "",
        const float epsAbs = 0.0f, const float epsRel = 0.0f) const
    {
        ...
        Factor::DD::Get(m_add, m_bd)
            .AssertEqual(A2.m_add, verbose, str + ".m_add", epsAbs, epsRel);
        Factor::DC::Get(m_adcx.GetVector6f())
            .AssertZero(verbose, str + ".m_adcx");
        Factor::DC::Get(m_adcz.GetVector6f())
            .AssertEqual(A1.m_adcz, verbose, str + ".m_adcz", epsAbs, epsRel);
        Camera::Factor::Unitary::CC::Get(
            m_Acxx.GetSymmetricMatrix6x6f(), m_bcx.GetVector6f())
            .AssertZero(verbose, str + ".m_Acxx");
        Camera::Factor::Binary::CC(m_Acxz.GetAlignedMatrix6x6f())
        ...
        Camera::Factor::Unitary::CC::Get(Aczz, bcz).AssertEqual(
            A2.m_Aczz, verbose, str + ".m_Aczz", epsAbs, epsRel);
    }

    inline void AssertEqual(const Factor::Full::L& L,
        const Factor::Full::A1& A1, const Factor::Full::A2& A2,
        const int verbose = 1, const std::string str = "",
        const float epsAbs = 0.0f, const float epsRel = 0.0f) const
    {
        UT::AssertEqual(m_F, L.m_F, verbose, str + ".m_F");
        Factor::DD::Get(m_add, m_bd)
            .AssertEqual(
                A2.m_adx.m_add, verbose, str + ".m_add", epsAbs, epsRel);
        Factor::DC::Get(m_adcx.GetVector6f())
            .AssertEqual(
                A2.m_adx.m_adc, verbose, str + ".m_adcx", epsAbs, epsRel);
        Factor::DC::Get(m_adcz.GetVector6f())
            .AssertEqual(A1.m_adcz, verbose, str + ".m_adcz", epsAbs, epsRel);
        const LA::SymmetricMatrix6x6f Acxx = m_Acxx.GetSymmetricMatrix6x6f();
        const LA::Vector6f bcx = m_bcx.GetVector6f();
        Camera::Factor::Unitary::CC::Get(Acxx, bcx).AssertEqual(
            A2.m_Acxx, verbose, str + ".m_Acxx", epsAbs, epsRel);
        const LA::AlignedMatrix6x6f Acxz = m_Acxz.GetAlignedMatrix6x6f();
        Camera::Factor::Binary::CC(Acxz).AssertEqual(
            A2.m_Acxz, verbose, str + ".m_Acxz", epsAbs, epsRel);
        const LA::SymmetricMatrix6x6f Aczz = m_Aczz.GetSymmetricMatrix6x6f();
        const LA::Vector6f bcz = m_bcz.GetVector6f();
        Camera::Factor::Unitary::CC::Get(Aczz, bcz).AssertEqual(
            A2.m_Aczz, verbose, str + ".m_Aczz", epsAbs, epsRel);
...
EigenErrorJacobian EigenGetErrorJacobian(
    const Rigid3D& C1, const Source& x1, const Depth::InverseGaussian& d1,
...
template <int ME_FUNCTION>
inline EigenFactor EigenGetFactor(const float w, const Rigid3D& C1,
    const Source& x1, const Depth::InverseGaussian& d1, const Rigid3D& C2,
...
)
{
    ...
    if (z2.m_z.Valid())
    {
        const EigenErrorJacobian e_Je =
            EigenGetErrorJacobian(C1, x1, d1, C2, z2.m_z, cx, cz);
        ...
        const EigenMatrix2x13f e_J =
            EigenMatrix2x13f(e_Je.m_Jd, e_Je.m_Jcx, e_Je.m_Jcz);
    ...
    {
        const EigenErrorJacobian e_Je =
            EigenGetErrorJacobian(C1, x1, d1, C2, z2.m_zr, cx, cz, &br);
    ...
    const EigenMatrix2x13f e_WJ = EigenMatrix2x13f(_w * e_W * e_J);
    const float F = _w * r2;
    const EigenMatrix13x14f e_A =
        EigenMatrix13x14f(e_WJ.transpose() * EigenMatrix2x14f(e_J, e_Je.m_e));
#endif
...
inline float EigenGetCost(const float w, const Rigid3D& C1, const Source& x1,
    const Depth::InverseGaussian& d1, const Rigid3D& C2, const Measurement& z2,
    const EigenVector6f* e_xcx, const EigenVector6f* e_xcz, const float xd
#ifdef CFG_STEREO
    ,
    const Point3D& br
#endif
)
{
    ...
    if (z2.m_z.Valid())
    {
        const EigenErrorJacobian e_Je = EigenGetErrorJacobian(
            C1, x1, d1, C2, z2.m_z, e_xcx != NULL, e_xcz != NULL);
        const EigenMatrix2x2f e_W = EigenMatrix2x2f(z2.m_W);
        const float r2 = (e_W * e_Je.m_e).dot(e_Je.m_e);
        const float _w = w * ME::Weight<ME_FUNCTION>(r2);
    ...
    {
        const EigenErrorJacobian e_Je = EigenGetErrorJacobian(
            C1, x1, d1, C2, z2.m_zr, e_xcx != NULL, e_xcz != NULL, &br);
        const EigenMatrix2x2f e_W = EigenMatrix2x2f(z2.m_Wr);
        const float r2 = (e_W * e_Je.m_e).dot(e_Je.m_e);
        const float _w = w * ME::Weight<ME_FUNCTION>(r2);
...
#ifdef CFG_STEREO
EigenErrorJacobian::Stereo EigenGetErrorJacobian(
    const Point3D& br, const Depth::InverseGaussian& d, const Source& x);
...
template <int ME_FUNCTION>
inline float EigenGetCost(const float w, const Point3D& br,
    const Depth::InverseGaussian& d, const Source& x, const float xd)
{
    const EigenErrorJacobian::Stereo e_Je = EigenGetErrorJacobian(br, d, x);
    const EigenMatrix2x2f e_W = EigenMatrix2x2f(x.m_Wr);
