...
struct CurrentFrame
{
    ...
    std::vector<MapPointMeasurement>
        zs; // feature measurements of current frame
...
{
    std::vector<int>
        iFrms; // frame indexes of those sliding window frames whose
    // camera/IMU state has been updated since last call
    std::vector<CameraIMUState>
        CsLF; // camera/IMU states corresponding to iFrms
    std::vector<int>
