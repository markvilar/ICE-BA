...
void Solver::Create(const Calibration& K, const int serial, const int verbose,
    ...
    const std::string dir, const std::vector<CameraIMUState>* XsGT,
...
#ifndef WIN32
    UT_ASSERT(
        (reinterpret_cast<std::uintptr_t>(m_internal) & (SIMD_ALIGN - 1)) == 0);
#endif
#endif
...
void Solver::PushCurrentFrame(
    const CurrentFrame& CF, const KeyFrame* KF, const bool serial)
{
//#ifdef CFG_DEBUG
#if 0
...
void Solver::EstimateMotionGT(std::vector<CameraIMUState>* XsGT)
{
    ...
    for (int iIter = 0; iIter < BA_MAX_ITERATIONS; ++iIter)
    {
        ...
        for (int ic = Nc - 1, imp = Nmr - 1, r = ic & 1; ic >= 0;
             --ic, r = 1 - r)
        {
            const int _ic = ic + 1;
...
void Solver::PushDepthMeasurementsGT(
    const CurrentFrame& CF, const KeyFrame* KF, const bool keyframeOnly)
{
    ...
    if (KF)
    {
        ...
        std::vector<std::vector<::Depth::Measurement>>& zsGT =
            m_internal->m_zsGT;
        ...
        for (int i1 = 0, i2 = 0; i1 < N; i1 = i2)
        {
            ...
            zsGT.insert(
                zsGT.begin() + id, Nx, std::vector<::Depth::Measurement>());
            ...
            for (int i = 0; i < Nx; ++i)
            {
                ...
                if (x.m_xr.Valid())
                {
                    ...
                    _zs.push_back(
                        ::Depth::Measurement(tsGT[0], Rx, x.m_xr, x.m_Wr));
                    ::Depth::Triangulate(1.0f,
                        static_cast<int>(_zs.size()),
                        _zs.data(),
                        &d,
                        &work);
                }
...
//#ifdef CFG_DEBUG
          ...
          tsGT[it + 1].AssertEqual(tr);
                    ...
                    {
                        _zs.push_back(
                            ::Depth::Measurement(tsGT[it], Rx, z.m_z, z.m_W));
                    }
                    if (z.m_zr.Valid())
                    {
                        _zs.push_back(::Depth::Measurement(
                            tsGT[it + 1], Rx, z.m_zr, z.m_Wr));
                    }
#else
                    _zs.push_back(
                        ::Depth::Measurement(tsGT[it], Rx, z.m_z, z.m_W));
                    ...
                    //          e * sqrtf(m_internal->m_K.m_K.fxy()),
                    //          iFrmsKF[z.m_iKF]);
                }
            }
        }
...
void Internal::PushDepthMeasurementsGT(const FRM::Frame& F)
{
    ...
    for (int iZ = 0; iZ < NZ; ++iZ)
    {
        ...
        for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz)
        {
            ...
            {
                _zs.push_back(
                    ::Depth::Measurement(m_TsGT[it], Rx, z.m_z, z.m_W));
            }
            if (z.m_zr.Valid())
            {
                _zs.push_back(
                    ::Depth::Measurement(m_TsGT[it + 1], Rx, z.m_zr, z.m_Wr));
            }
#else
...
void Solver::TriangulateDepthsGT(std::vector<Depth>* dsGT)
{
    ...
    for (int idx = 0; idx < N; ++idx)
    {
        ...
        else
        {
            ...
            const std::vector<::Depth::Measurement>& zs =
                m_internal->m_zsGT[id];
            const int Nz = static_cast<int>(zs.size());
            if (::Depth::Triangulate(
                    1.0f, Nz, zs.data(), &_d, &m_internal->m_work, true, &e))
            {
                es.Push(e);
                ConvertDepth(_d, &d);
            }
...
void Solver::UpdateCameras(const std::vector<int>& iFrms,
    const std::vector<CameraPose>& Cs, const bool serial)
{
    Rigid3D C;
...
bool Solver::PropagateState(const std::vector<IMUMeasurement>& us,
    const float t1, const float t2, const CameraIMUState& X1,
    CameraIMUState* X2, CameraPoseCovariance* S)
{
...
bool Solver::SaveFeatures(const std::string fileName,
    const std::vector<MapPointMeasurement>& zs, const std::vector<MapPoint>* Xs)
{
...
bool Solver::LoadFeatures(const std::string fileName, const int iFrm,
    std::vector<MapPointMeasurement>* zs, std::vector<MapPoint>* Xs,
...
)
{
            ...
            if (i == 7)
            {
                ...
                if (z.idx > idxMax || idx2iX[z.idx] == -1)
                {
                    ...
                    if (Xs)
                    {
                        ...
                        {
                            const int iX =
                                z.idx < static_cast<int>(idx2iXTmp.size())
                                    ? idx2iXTmp[z.idx]
                                    : -1;
                            if (iX != -1)
                            {
...
#endif
                                z.iFrm = iFrm;
                                X.zs.insert(std::lower_bound(
                                                X.zs.begin(), X.zs.end(), z),
                                    z);
                            }
                        }
                        else
    ...
    if (Xs)
    {
        ...
        while (fgets(line, UT_STRING_WIDTH_MAX, fp))
        {
            const int len =
                int(std::remove(
                        line, std::remove(line, line + strlen(line), 10), ' ') -
                    line);
            if (len == 0 || line[0] == '%' || line[len - 1] == ':')
            {
                continue;
            ...
            for (int ix = 0; ix < Nx; ++ix)
            {
                ...
                if (right)
                {
                    iX = idx < static_cast<int>(idx2iXTmp.size())
                             ? idx2iXTmp[idx]
                             : -1;
                    if (iX == -1)
                    {
                ...
                for (int i = 0; i < Nz; ++i)
                {
                    ...
                    X.zs.insert(
                        std::lower_bound(X.zs.begin(), X.zs.end(), z), z);
                }
            }
            break;
        }
...
void Solver::LoadB(FILE* fp)
{
    ...
    //  m_internal->m_LBA.m_dsGT = m_internal->m_GBA.m_dsGT =
    //  &m_internal->m_dsGT;
    //}
#if 0
...
void Solver::ComputeErrorLBA(Error* e)
{
    ...
    m_internal->m_LBA.ComputeErrorIMU(
        &e->eur, &e->eup, &e->euv, &e->euba, &e->eubw);
...
void Solver::ComputeErrorGBA(Error* e)
{
    ...
    m_internal->m_GBA.ComputeErrorIMU(
        &e->eur, &e->eup, &e->euv, &e->euba, &e->eubw);
...
static inline std::string ConvertFileName(
    const std::string fileName, const std::string dir, const bool append = true)
{
    if (fileName == "")
    {
        return "";
    ...
    if (UT::FileNameRemoveDirectoryExtension(fileName) == "*")
    {
        ...
        _fileName = UT::FileNameReplaceDirectory(_fileName,
            UT::FileNameExtractDirectory(_fileName),
...
bool Solver::SaveCamerasLBA(
    const std::string fileName, const bool append, const bool poseOnly)
{
...
bool Solver::SaveCostsLBA(
    const std::string fileName, const bool append, const int type)
{
    const std::string _fileName =
        ConvertFileName(fileName, m_internal->m_dir, append);
...
bool Solver::SaveMarginalizations(
    const std::string fileName, const bool append, const int type)
{
    const std::string _fileName =
        ConvertFileName(fileName, m_internal->m_dir, append);
...
{
    return m_internal->SavePoints(
        m_internal->m_LBA.m_CsKF, m_internal->m_LBA.m_ds, fileName, append);
}

bool Solver::SavePointsGBA(const std::string fileName, const bool append)
{
    return m_internal->SavePoints(
        m_internal->m_GBA.m_Cs, m_internal->m_GBA.m_ds, fileName, append);
}

bool Internal::SavePoints(const AlignedVector<Rigid3D>& CsKF,
    const std::vector<::Depth::InverseGaussian>& ds, const std::string fileName,
    const bool append)
{
...
static inline void Rectify(const ::Intrinsic& K, const Point2D& x,
    ...
    ::Intrinsic::UndistortionMap* UM = NULL, const float eps = 0.0f
#ifdef CFG_STEREO
    ,
    Rotation3D* Rr = NULL
#endif
...
const GlobalMap::InputKeyFrame& Internal::PushKeyFrame(
    const KeyFrame& KF, const Camera* C)
{
//#ifdef CFG_DEBUG
#if 0
  UT::Print("%d\n", KF.iFrm);
    ...
    else
    {
        ...
        {
            iKF0 = static_cast<int>(
                       std::upper_bound(m_CsKF.begin(), m_CsKF.end(), _C) -
                       m_CsKF.begin()) -
                   1;
        }
...
//#ifdef CFG_DEBUG
    ...
    if (ix.m_idx == 2001) {
      ...
      UT::DebugStop();
        ...
        {
            iKF1 = static_cast<int>(
                std::lower_bound(
                    m_CsKF.begin() + iKF1, m_CsKF.begin() + iKF2, ix.m_iFrm) -
                m_CsKF.begin());
        ...
        for (int i = 0; i < Nz1; ++i)
        {
            ...
            if (_z.right)
            {
                ...
                Rectify(
                    m_K.m_Kr, _z.x, &z.m_zr, &z.m_Wr, &m_UMr, eps, &m_K.m_Rr);
...
#endif
            {
...
#endif
                {
                    ...
                    z.m_zr.Invalidate();
#endif
                    z.m_iKF = static_cast<int>(
                        std::lower_bound(m_CsKF.begin() + z.m_iKF,
                            m_CsKF.begin() + iKF2,
                            _z.iFrm) -
                        m_CsKF.begin());
                    if (z.m_iKF != nKFs && m_CsKF[z.m_iKF].m_iFrm == _z.iFrm)
      ...
      else if (ext == "yaml") */
    {
        ...
        while (fgets(line, UT_STRING_WIDTH_MAX, fp))
        {
            ...
            case 3:
            {
                ...
                // if (j == 2 && sa) {
                //  sscanf(line, "accel_scale: [%f, %f, %f]", &sa[0], &sa[1],
                //  &sa[2]);
                //}
                ++j;
                break;
...
bool SaveGroundTruth(
    const std::string fileName, const std::vector<CameraIMUState>& XsGT)
{
    return UT::VectorSaveB(XsGT, fileName.c_str());
}

bool LoadGroundTruth(
    const std::string fileName, std::vector<CameraIMUState>* XsGT)
{
...
static inline bool LoadIndexes(
    const std::string fileName, std::vector<int>* idxs)
{
    idxs->resize(0);
    FILE* fp = fopen(fileName.c_str(), "r");
    if (!fp)
...
static inline bool SaveIndexes(
    const std::string fileName, const std::vector<int>& idxs)
{
    FILE* fp = fopen(fileName.c_str(), "w");
    if (!fp)
    {
...
bool SaveCurrentFrame(
    const std::string fileName, const CurrentFrame& CF, const KeyFrame& KF)
{
    FILE* fp = fopen(fileName.c_str(), "wb");
    if (!fp)
    {
...
bool LoadCurrentFrame(
    const std::string fileName, CurrentFrame* CF, KeyFrame* KF)
{
    FILE* fp = fopen(fileName.c_str(), "rb");
    if (!fp)
    {
...
bool LoadRelativeConstraints(const std::string fileName,
    const std::vector<float>& ts, std::vector<RelativeConstraint>* Zs,
    const float dtMax)
{
    Zs->resize(0);
    FILE* fp = fopen(fileName.c_str(), "r");
...
static inline bool AssertError(
    const int iFrm, const float e, const float eMax, const std::string str)
{
    if (e <= eMax)
    {
        return true;
