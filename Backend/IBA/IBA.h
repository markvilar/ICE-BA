...
public:
    ...
    void PushDepthMeasurementsGT(const CurrentFrame& CF,
        const KeyFrame* KF = NULL, const bool keyframeOnly = false);
    void TriangulateDepthsGT(std::vector<Depth>* dsGT);
#endif

    /*
     * @brief Set callback function that will be triggered after LBA/GBA
     * finishes
     */
    ...
    /*
     * @brief Call for each current frame to synchronize the optimization
     * results
    ...
    void UpdateCameras(const std::vector<int>& iFrms,
        const std::vector<CameraPose>& Cs, const bool serial = false);

    bool PropagateState(const std::vector<IMUMeasurement>& us, const float t1,
        const float t2, const CameraIMUState& X1, CameraIMUState* X2,
    ...
    bool LoadFeatures(const std::string fileName, const int iFrm,
        std::vector<MapPointMeasurement>* zs, std::vector<MapPoint>* Xs = NULL,
...
extern bool LoadCalibration(const std::string fileName, float T[3][4],
    Intrinsic* K, float* ba = NULL, float* bw = NULL /*, float *sa = NULL*/);
extern void PrintCalibration(const Calibration& K);
extern bool SaveGroundTruth(
    const std::string fileName, const std::vector<CameraIMUState>& XsGT);
extern bool LoadGroundTruth(
    const std::string fileName, std::vector<CameraIMUState>* XsGT);
extern bool SaveGroundTruth(
    const std::string fileName, const std::vector<Depth>& dsGT);
extern bool LoadGroundTruth(
    const std::string fileName, std::vector<Depth>* dsGT);
extern bool SaveKeyFrames(
    const std::string fileName, const std::vector<int>& iFrms);
...
extern bool SaveFeatureTracks(
    const std::string fileName, const std::vector<FeatureTrack>& xs);
extern bool LoadFeatureTracks(
    const std::string fileName, std::vector<FeatureTrack>* xs);
extern bool SaveCurrentFrame(
...
extern void PrintCameraPose(
    const int iFrm, const CameraPose& C, const bool n = false);
...
extern bool SaveCameraPoses(const std::string fileName,
    const std::vector<int>& iFrms, const std::vector<CameraPose>& Cs);
