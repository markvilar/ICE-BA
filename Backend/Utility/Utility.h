...
FILE* FileOpen(
    const std::string fileName, const char* mode, const char* format, ...);
...
std::vector<std::string> FilesSearch(const std::string fileNameFirst,
    const int iStart = 0, const int iStep = 1, const int iEnd = INT_MAX,
    const bool verbose = true);
...
std::string FileNameRemovePrefix(
    const std::string fileName, const std::string prefix);
std::string FileNameAppendSuffix(
...
std::string StringReplace(
    const std::string str, const std::string strSrc, const std::string strDst);
...
std::vector<std::string> Strings(const std::string str0, const std::string str1,
    const std::string str2, const std::string str3, const std::string str4,
    const std::string str5, const std::string str6, const std::string str7);
...
void SaveHistogram(const std::string fileName, const std::vector<float>& vs,
    const int Nb, const float vMin = FLT_MAX, const float vMax = FLT_MAX);

template <typename TYPE>
inline bool Equal(
    const TYPE v1, const TYPE v2, const TYPE epsAbs = 0, const TYPE epsRel = 0)
{
    ...
    return e <= epsAbs || er <= epsRel || e <= EpsilonMin<TYPE>() ||
    ...
    const TYPE epsRel = 0)
{
    ...
    const TYPE _epsRel =
        epsRel < 0 ? epsRel : std::max(epsRel, EpsilonRatioMin<TYPE>());
...
inline void VectorsSaveB(const std::list<std::vector<TYPE>>& Vs, FILE* fp)
{
    const int N = static_cast<int>(Vs.size());
    SaveB<int>(N, fp);
    for (typename std::list<std::vector<TYPE>>::const_iterator i = Vs.begin();
         i != Vs.end();
         ++i)
    {
...
inline void VectorsLoadB(std::list<std::vector<TYPE>>& Vs, FILE* fp)
{
    const int N = LoadB<int>(fp);
    Vs.resize(N);
    for (typename std::list<std::vector<TYPE>>::iterator i = Vs.begin();
         i != Vs.end();
         ++i)
    {
...
inline void ListSaveB(const std::list<TYPE>& L, FILE* fp)
{
    ...
    SaveB<int>(N, fp);
    for (typename std::list<TYPE>::const_iterator i = L.begin(); i != L.end();
         ++i)
    {
        SaveB<TYPE>(*i, fp);
    }
...
{
    xp128f tmp;
    tmp.vset_all_lane(static_cast<float>(i11),
        static_cast<float>(i12),
        static_cast<float>(i21),
...
{
    xp128f tmp;
    tmp.vset_all_lane(i11, i12, i21, i22);
    ip = (tmp * w).vsum_all();
}
...
public:
    ...
    inline void Print(const std::string str = "", const bool F = true,
        const bool l = true, const bool i = true, const int r = 0,
        const bool n = true) const
    {
        ...
        if (r >= 1)
        {
            ...
            UT::Print(
                "%d%%)", static_cast<int>(UT::Percentage(m_SNr, m_SN) + 0.5f));
        }
        if (n)
        {
            UT::Print("\n");
...
template <class TYPE>
inline bool AssertReduction(const TYPE& v1, const TYPE& v2,
...
inline bool AssertReduction<float>(const float& v1, const float& v2,
    ...
    const float vMax, const float eps)
{
