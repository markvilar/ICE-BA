...
public:
    ...
    inline std::string GetArgument(
        const std::string directive, const std::string default_val = "") const
    {
        const DirectiveArgumentMap::const_iterator i =
    ...
    inline int GetArgument(
        const std::string directive, const int default_val) const
    {
        const DirectiveArgumentMap::const_iterator i =
            m_directiveArgumentMap.find(directive);
        if (i == m_directiveArgumentMap.end())
    ...
    inline float GetArgument(
        const std::string directive, const float default_val) const
    {
        const DirectiveArgumentMap::const_iterator i =
            m_directiveArgumentMap.find(directive);
        if (i == m_directiveArgumentMap.end())
    ...
    inline double GetArgument(
        const std::string directive, const double default_val) const
    {
        const DirectiveArgumentMap::const_iterator i =
            m_directiveArgumentMap.find(directive);
        if (i == m_directiveArgumentMap.end())
    ...
    inline float GetArgumentInverse(
        const std::string directive, const float default_val) const
    {
        const float argument = GetArgument(directive, UT::Inverse(default_val));
        return UT::Inverse(argument);
    }
    inline float GetArgumentScaled(const std::string directive,
        const float default_val, const float scalar) const
    {
        const float argument = GetArgument(directive, default_val / scalar);
        return argument * scalar;
    }
    inline int GetArgumentScaled(const std::string directive,
        const int default_val, const float scalar) const
    {
        const float argument = GetArgument(directive, default_val / scalar);
        return int(argument * scalar + 0.5f);
    }
    inline float GetArgumentSquared(
        const std::string directive, const float default_val) const
    {
        const float argument = GetArgument(directive, sqrtf(default_val));
        return argument * argument;
    }
    inline float GetArgumentScaledSquared(const std::string directive,
        const float default_val, const float scalar) const
    {
        const float argument =
            GetArgumentScaled(directive, sqrtf(default_val), scalar);
        return argument * argument;
    }
    inline float GetArgumentRadian(
        const std::string directive, const float default_val) const
    {
        return GetArgumentScaled(directive, default_val, UT_FACTOR_DEG_TO_RAD);
    }
    inline float GetArgumentRadianSquared(
        const std::string directive, const float default_val) const
    {
        return GetArgumentScaledSquared(
            directive, default_val, UT_FACTOR_DEG_TO_RAD);
    }

    inline void SetArgument(
        const std::string directive, const std::string argument)
    {
        DirectiveArgumentMap::iterator i =
            m_directiveArgumentMap.find(directive);
        if (i == m_directiveArgumentMap.end())
        {
    ...
    void Print() const
    {
        ...
        for (DirectiveArgumentMap::const_iterator i =
                 m_directiveArgumentMap.begin();
             i != m_directiveArgumentMap.end();
             i++)
        {
