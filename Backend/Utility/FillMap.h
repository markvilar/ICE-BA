...
public:
    inline void Initialize(const int w, const int h)
    {
        Resize(w, h);
        MakeZero();
    }
    ...
    inline void Fill(const int x, const int y, const int dist = 0)
    {
        ...
        const auto x1 = std::min(std::max(x - dist, 0), m_w);
        const auto x2 = std::min(x + dist, m_w);
        const auto y1 = std::min(std::max(y - dist, 0), m_h);
        const auto y2 = std::min(y + dist, m_h);
    ...
    inline bool Filled(const int x, const int y) const
    {
        return m_rows[y][x] != 0;
    }
    ...
    inline void GetDilated(FillMap& FM, const int dist = 1) const
    {
        ...
        for (int y = 0; y < m_h; ++y)
        {
            ...
            for (int x = 0; x < m_w; ++x)
            {
                const auto x1 = std::max(x - dist, 0);
                const auto x2 = std::min(x + dist, m_w);
                ubyte v = 0;
                for (int _y = y1; _y < y2; ++_y)
                {
