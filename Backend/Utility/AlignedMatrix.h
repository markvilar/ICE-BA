...
public:
        ...
        const bool retain = false)
    {
        ...
        const int N =
            symmetric ? SIMD::Ceil<TYPE>((Nc * (Nc + 1)) >> 1) : Nr * NcC;
        if (N > m_capacity)
        {
            TYPE* dataBkp = m_data;
    ...
    inline void Bind(
        void* M, const int Nr, const int Nc, const bool symmetric = false)
    {
        if (m_data && m_own)
        {
            SIMD::Free<TYPE>(m_data);
    ...
    inline int BindSize(
        const int Nr, const int Nc, const bool symmetric = false) const
    {
#ifdef CFG_DEBUG
        if (symmetric)
        {
...
#endif
        return sizeof(TYPE) * (symmetric
                                      ? SIMD::Ceil<TYPE>((Nc * (Nc + 1)) >> 1)
    ...
    inline AlignedMatrixX<TYPE> GetBlock(
        const int i, const int j, const int Nr, const int Nc)
    {
#ifdef CFG_DEBUG
    ...
    inline void Erase(const int i, const int N = 1)
    {
        ...
        if (m_symmetric)
        {
            ...
            for (int i1 = i, i2 = i1 + N; i2 < m_Nr;
                 ++i1, ++i2, size -= sizeof(TYPE))
            {
...
//#ifdef CFG_DEBUG
        ...
        for (int j1 = i, j2 = i2; j2 < m_Nr; ++j1, ++j2) {
          ...
          memcpy(m_rows[j2] + i2, MTmp[j1] + i, size2);
            ...
            if (m_symmetric)
            {
                ...
                for (int _i = m_Nr - 1, _j = _i - N; _i >= i2;
                     --_i, --_j, size += sizeof(TYPE))
                {
    ...
    inline void GetRow(const int i, TYPE* r) const
    {
        memcpy(r, m_rows[i], sizeof(TYPE) * m_Nc);
    }
