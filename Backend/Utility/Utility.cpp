...
bool FileDelete(
    const std::string fileName, const bool check, const bool verbose)
{
#ifdef CFG_DEBUG
...
std::vector<std::string> FilesSearch(const std::string fileNameFirst,
    const int iStart, const int iStep, const int iEnd, const bool verbose)
{
...
// files.
void FilesStartSaving(
    const std::string fileNameFirst, const bool check, const bool verbose)
{
    const std::string dir = FileNameExtractDirectory(fileNameFirst);
    if (FileExists(dir))
    {
