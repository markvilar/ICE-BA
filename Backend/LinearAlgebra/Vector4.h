...
public:
    inline const xp128f& v0123() const { return m_data; }
    inline xp128f& v0123() { return m_data; }
    ...
    inline const xp128f& xyzw() const { return v0123(); }
    inline xp128f& xyzw() { return v0123(); }
    ...
    inline void MakeMinus() { v0123().vmake_minus(); }
    inline AlignedVector4f GetMinus() const
    {
        AlignedVector4f v;
        xp128f zero;
        zero.vdup_all_lane(0.f);
        v.v0123() = zero - v0123();
        return v;
    }
    ...
    inline void Scale(const float s) { v0123() *= s; }
    inline void Scale(const xp128f& s) { v0123() *= s; }
        ...
        const float epsRel = 0.0f) const
    {
        if (UT::VectorAssertEqual(
                &v0(), &v.v0(), 4, verbose, str, epsAbs, epsRel))
        {
            return true;
        }
        else if (verbose)
    ...
    inline void SetInfinite() { v0123().vdup_all_lane(FLT_MAX); }
...
public:
    ...
    static inline EigenVector4f GetRandom(const float vMax)
    {
        return EigenVector4f(LA::AlignedVector4f::GetRandom(vMax));
    }
    inline bool AssertEqual(const LA::AlignedVector4f& v, const int verbose = 1,
        const std::string str = "", const float epsAbs = 0.0f,
        const float epsRel = 0.0f) const
    {
        return GetAlignedVector4f().AssertEqual(
    ...
    {
        return AssertEqual(
            e_v.GetAlignedVector4f(), verbose, str, epsAbs, epsRel);
    }
};
#endif
#endif
