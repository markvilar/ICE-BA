...
public:
    ...
    inline bool AssertEqual(const AlignedMatrix2x8f& M, const int verbose = 1,
        const std::string str = "", const float epsAbs = 0.0f,
        const float epsRel = 0.0f) const
    {
        if (UT::VectorAssertEqual(
                &m00(), &M.m00(), 16, verbose, str, epsAbs, epsRel))
        {
            return true;
        }
        else if (verbose)
...
public:
    ...
    inline bool AssertEqual(const LA::AlignedMatrix2x8f& M,
        ...
        const float epsAbs = 0.0f, const float epsRel = 0.0f) const
    {
        return GetAlignedMatrix2x8f().AssertEqual(
            M, verbose, str, epsAbs, epsRel);
    }
    inline bool AssertEqual(const EigenMatrix2x8f& e_M, const int verbose = 1,
        const std::string str = "", const float epsAbs = 0.0f,
        const float epsRel = 0.0f) const
    {
        return AssertEqual(
            e_M.GetAlignedMatrix2x8f(), verbose, str, epsAbs, epsRel);
    }
};
#endif
