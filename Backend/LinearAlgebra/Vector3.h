...
public:
    ...
    inline const xp128f& v012r() const { return m_data; }
    inline xp128f& v012r() { return m_data; }
    inline const xp128f& xyzr() const { return m_data; }
    inline xp128f& xyzr() { return m_data; }
    ...
    inline const float& r() const { return m_data[3]; }
    inline float& r() { return m_data[3]; }
    ...
    inline void operator=(const xp128f& v) { v012r() = v; }
    ...
    inline void operator+=(const AlignedVector3f& v) { v012r() += v.v012r(); }
    inline void operator-=(const AlignedVector3f& v) { v012r() -= v.v012r(); }
    ...
    inline void Set(const float* v) { memcpy(&m_data[0], v, 12); }
    ...
    inline void Normalize() { v012r() *= (1.0f / sqrtf(SquaredLength())); }
    ...
    inline void Scale(const float s) { v012r() *= s; }
    ...
    inline void GetScaled(
        const LA::AlignedVector3f& s, AlignedVector3f& v) const
    {
    ...
    inline void MakeSquareRoot() { v012r().vset_sqrt_from_vec(v012r()); }
    ...
    inline void Interpolate(
        const float w1, const AlignedVector3f& v1, const AlignedVector3f& v2)
    {
        v012r() = v1.v012r() * w1 + v2.v012r() * (1.f - w1);
    }
    inline void Interpolate(const AlignedVector3f& v1,
        const AlignedVector3f& v2, const float t1, const float t2,
        const float t)
    {
    ...
    inline void Print(
        const std::string str, const bool e, const bool l, const bool n) const
    {
        ...
        const float epsRel = 0.0f) const
    {
        if (UT::VectorAssertEqual(
                &v0(), &v.v0(), 3, verbose, str, epsAbs, epsRel))
        {
            return true;
        }
        else if (verbose)
    ...
    inline void SetInfinite() { v012r().vdup_all_lane(FLT_MAX); }
    ...
    static inline void apb(const AlignedVector3f& a, const AlignedVector3f& b,
        AlignedVector3f& apb)
    {
        apb.v012r() = a.v012r() + b.v012r();
    }
    static inline void amb(const AlignedVector3f& a, const AlignedVector3f& b,
        AlignedVector3f& amb)
    {
...
public:
    ...
    inline AlignedVector3f GetAlignedVector3f() const
    {
        AlignedVector3f v;
        Get(v);
        return v;
...
template <> inline void Vector3f::Set(const float* v)
{
    memcpy(this, v, sizeof(Vector3f));
}
template <> inline void Vector3d::Set(const double* v)
{
    memcpy(this, v, sizeof(Vector3d));
}
...
public:
    ...
    inline bool AssertEqual(const LA::AlignedVector3f& v, const int verbose = 1,
        ...
        const float epsRel = 0.0f) const
    {
        return GetAlignedVector3f().AssertEqual(
    ...
    {
        return AssertEqual(
            e_v.GetAlignedVector3f(), verbose, str, epsAbs, epsRel);
    }
    static inline EigenVector3f Zero()
    {
        return EigenVector3f(Eigen::Vector3f::Zero());
    }
};
