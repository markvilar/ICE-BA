...
public:
    inline const xp128f& m_00_01_10_11() const { return m_data; }
    inline xp128f& m_00_01_10_11() { return m_data; }

    inline const float& m00() const { return m_data[0]; }
    inline float& m00() { return m_data[0]; }
    inline const float& m01() const { return m_data[1]; }
    inline float& m01() { return m_data[1]; }
    inline const float& m10() const { return m_data[2]; }
    inline float& m10() { return m_data[2]; }
    inline const float& m11() const { return m_data[3]; }
    inline float& m11() { return m_data[3]; }
    ...
    inline float& operator()(int row, int col) { return m_data[row * 2 + col]; }
    ...
    inline void Scale(const xp128f& s) { m_00_01_10_11() *= s; }
    ...
    inline bool AssertEqual(const AlignedMatrix2x2f& M, const int verbose = 1,
        const std::string str = "", const float epsAbs = 0.0f,
        const float epsRel = 0.0f) const
    {
        if (UT::VectorAssertEqual(
                &m00(), &M.m00(), 4, verbose, str, epsAbs, epsRel))
        {
            return true;
        }
        else if (verbose)
    ...
    static inline void Ab(
        const AlignedMatrix2x2f& A, const Vector2f& b, Vector2f& Ab)
    {
        const xp128f t =
            A.m_00_01_10_11() * xp128f::get(b.v0(), b.v1(), b.v0(), b.v1());
        Ab.v0() = t[0] + t[1];
        Ab.v1() = t[2] + t[3];
    }
    static inline void AddAbTo(
        const AlignedMatrix2x2f& A, const float* b, Vector2f& Ab)
    {
        const xp128f t =
            A.m_00_01_10_11() * xp128f::get(b[0], b[1], b[0], b[1]);
        Ab.v0() += t[0] + t[1];
        Ab.v1() += t[2] + t[3];
    }

    static inline void ABT(const AlignedMatrix2x2f& A,
        const AlignedMatrix2x2f& B, AlignedMatrix2x2f& ABT)
    {
...
public:
    ...
    inline SymmetricMatrix2x2<TYPE> operator-(
        const SymmetricMatrix2x2<TYPE>& B) const
    {
    ...
    inline void Set(const TYPE M[2][2])
    {
        m00() = M[0][0];
        m01() = M[0][1];
        m11() = M[1][1];
    ...
    inline void Get(TYPE M[2][2]) const
    {
        M[0][0] = m00();
        M[0][1] = M[1][0] = m01();
        M[1][1] = m11();
    ...
    inline bool GetInverseLDL(
        SymmetricMatrix2x2<TYPE>& MI, const TYPE* eps) const
    {
        TYPE M[2][2];
        Get(M);
        TYPE* _M[2] = {M[0], M[1]};
    ...
    inline void Random(const TYPE mMin, const TYPE mMax)
    {
    ...
    static inline SymmetricMatrix2x2<TYPE> GetRandom(
        const TYPE mMin, const TYPE mMax)
    {
        SymmetricMatrix2x2<TYPE> v;
        v.Random(mMin, mMax);
        return v;
    ...
    static inline TYPE MahalanobisDistance(
        const SymmetricMatrix2x2<TYPE>& W, const TYPE d)
    {
        return (W.m00() + W.m01() + W.m10() + W.m11()) * d * d;
    }
    static inline TYPE MahalanobisDistance(
        const SymmetricMatrix2x2<TYPE>& W, const Vector2<TYPE>& d)
    {
    ...
    static inline void aaT(
        const Vector2<TYPE>& a, SymmetricMatrix2x2<TYPE>& aaT)
    {
        aaT.m00() = a.v0() * a.v0();
        aaT.m01() = a.v0() * a.v1();
        aaT.m11() = a.v1() * a.v1();
    ...
    static inline void AddAbTo(const SymmetricMatrix2x2<TYPE>& A,
        const Vector2<TYPE>& b, Vector2<TYPE>& Ab)
    {
        Ab.v0() += A.m00() * b.v0() + A.m01() * b.v1();
        Ab.v1() += A.m01() * b.v0() + A.m11() * b.v1();
    }
    static inline void AB(const AlignedMatrix2x2f& A,
        SymmetricMatrix2x2<TYPE>& B, AlignedMatrix2x2f& AB)
    {
    ...
    static inline void AAT(
        const AlignedMatrix2x2f& A, SymmetricMatrix2x2<TYPE>& AAT)
    {
    ...
    static inline void ABT(const AlignedMatrix2x2f& A,
        const AlignedMatrix2x2f& B, SymmetricMatrix2x2<TYPE>& ABT)
    {
        const xp128f t = A.m_00_01_10_11() * B.m_00_01_10_11();
        ABT.m00() = t[0] + t[1];
        ABT.m11() = t[2] + t[3];
...
public:
    ...
    inline bool AssertEqual(const LA::AlignedMatrix2x2f& M,
        ...
        const float epsAbs = 0.0f, const float epsRel = 0.0f) const
    {
        return GetAlignedMatrix2x2f().AssertEqual(
            M, verbose, str, epsAbs, epsRel);
    }
    inline bool AssertEqual(const EigenMatrix2x2f& e_M, const int verbose = 1,
        const std::string str = "", const float epsAbs = 0.0f,
        const float epsRel = 0.0f) const
    {
        return AssertEqual(
            e_M.GetAlignedMatrix2x2f(), verbose, str, epsAbs, epsRel);
    }
    inline bool AssertZero(const int verbose = 1, const std::string str = "",
        const float epsAbs = 0.0f, const float epsRel = 0.0f) const
    {
