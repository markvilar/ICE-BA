...
public:
    ...
    static inline void amb(const AlignedVector8f& a, const AlignedVector8f& b,
        AlignedVector8f& amb)
    {
    ...
    static inline AlignedVector8f GetRandom(const float vMax)
    {
        AlignedVector8f v;
        v.Random(vMax);
        return v;
    ...
    inline bool AssertEqual(const LA::AlignedVector8f& v, const int verbose = 1,
        const std::string str = "", const float epsAbs = 0.0f,
        const float epsRel = 0.0f) const
    {
        if (UT::VectorAssertEqual(
                &v0(), &v.v0(), 8, verbose, str, epsAbs, epsRel))
        {
            return true;
        }
        else if (verbose)
...
public:
    ...
    inline bool AssertEqual(const LA::AlignedVector8f& v, const int verbose = 1,
        ...
        const float epsRel = 0.0f) const
    {
        return GetAlignedVector8f().AssertEqual(
            v, verbose, str, epsAbs, epsRel);
    }
    inline bool AssertEqual(const EigenVector8f& e_v, const int verbose = 1,
        const std::string str = "", const float epsAbs = 0.0f,
        const float epsRel = 0.0f) const
    {
        return AssertEqual(
            e_v.GetAlignedVector8f(), verbose, str, epsAbs, epsRel);
    }
};
#endif
#endif
