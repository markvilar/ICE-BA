...
public:
    ...
    inline const xp128f& v456x() const { return m_data4[1]; }
    ...
    {
        memcpy(this, v, sizeof(AlignedVector7f));
    }
    inline void Get(float* v) const
    {
        memcpy(v, this, sizeof(AlignedVector7f));
    }
    ...
    static inline void apb(const AlignedVector7f& a, const AlignedVector7f& b,
        AlignedVector7f& apb)
    {
    ...
    static inline AlignedVector7f GetRandom(const float vMax)
    {
        AlignedVector7f v;
        v.Random(vMax);
        return v;
    ...
    inline bool AssertEqual(const AlignedVector7f& v, const int verbose = 1,
        const std::string str = "", const float epsAbs = 0.0f,
        const float epsRel = 0.0f) const
    {
        if (UT::VectorAssertEqual(
                &v0(), &v.v0(), 7, verbose, str, epsAbs, epsRel))
        {
            return true;
        }
        else if (verbose)
...
public:
    ...
    inline EigenVector7f(const Eigen::Matrix<float, 7, 1>& e_v)
        : Eigen::Matrix<float, 7, 1>(e_v)
    {
    }
    inline EigenVector7f(const LA::AlignedVector7f& v)
    ...
    inline float SquaredLength() const
    {
        return GetAlignedVector7f().SquaredLength();
    }
    inline void Print(const bool e = false) const
    {
        GetAlignedVector7f().Print(e);
    }
    static inline EigenVector7f GetRandom(const float vMax)
    {
        return EigenVector7f(LA::AlignedVector7f::GetRandom(vMax));
    }
    inline bool AssertEqual(const LA::AlignedVector7f& v, const int verbose = 1,
        const std::string str = "", const float epsAbs = 0.0f,
        const float epsRel = 0.0f) const
    {
        return GetAlignedVector7f().AssertEqual(
            v, verbose, str, epsAbs, epsRel);
    }
    inline bool AssertEqual(const EigenVector7f& e_v, const int verbose = 1,
        const std::string str = "", const float epsAbs = 0.0f,
        const float epsRel = 0.0f) const
    {
        return AssertEqual(
            e_v.GetAlignedVector7f(), verbose, str, epsAbs, epsRel);
    }
};
#endif
#endif
