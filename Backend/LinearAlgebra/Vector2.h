...
public:
    ...
    inline void GetScaled(const TYPE s, Vector2<TYPE>& v) const
    {
        v.v0() = s * v0();
        v.v1() = s * v1();
    }
    inline void GetScaled(const Vector2<TYPE>& s, Vector2<TYPE>& v) const
    {
        v.v0() = s.v0() * v0();
        v.v1() = s.v1() * v1();
    }
    ...
    inline TYPE Dot(const TYPE v0, const TYPE v1) const
    {
        ...
        const float epsRel = 0.0f) const
    {
        if (UT::VectorAssertEqual<TYPE>(
                &v0(), &v.v0(), 2, verbose, str, epsAbs, epsRel))
        {
            return true;
        }
        else if (verbose)
    ...
    {
        UT::Random<TYPE>(m_data, 2, -vMax, vMax);
    }
    inline void Random(const TYPE vMin, const TYPE vMax)
    {
        UT::Random<TYPE>(m_data, 2, vMin, vMax);
    }
    static inline Vector2<TYPE> GetRandom(const TYPE vMax)
    {
        Vector2<TYPE> v;
        v.Random(vMax);
        return v;
    }
    static inline Vector2<TYPE> GetRandom(const TYPE vMin, const TYPE vMax)
    {
        Vector2<TYPE> v;
        v.Random(vMin, vMax);
        return v;
    ...
    static inline void apb(
        const Vector2<TYPE>& a, const Vector2<TYPE>& b, Vector2<TYPE>& apb)
    {
        apb.v0() = a.v0() + b.v0();
        apb.v1() = a.v1() + b.v1();
    }
    static inline void amb(
        const Vector2<TYPE>& a, const Vector2<TYPE>& b, Vector2<TYPE>& amb)
    {
        amb.v0() = a.v0() - b.v0();
        amb.v1() = a.v1() - b.v1();
    }
...
template <> inline void Vector2f::Set(const float* v)
{
    memcpy(this, v, sizeof(Vector2f));
}
template <> inline void Vector2d::Set(const double* v)
{
    memcpy(this, v, sizeof(Vector2d));
}
...
template <> inline void Vector2f::Get(float* v) const
{
    memcpy(v, this, sizeof(Vector2f));
}
template <> inline void Vector2d::Get(double* v) const
{
    memcpy(v, this, sizeof(Vector2d));
}
