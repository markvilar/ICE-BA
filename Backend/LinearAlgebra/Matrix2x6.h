...
public:
        ...
        const float epsRel = 0.0f) const
    {
        if (UT::VectorAssertEqual(
                &m_data[0][0], &M[0][0], 12, verbose, str, epsAbs, epsRel))
        {
            return true;
        }
        else if (verbose)
        ...
        const float epsAbs = 0.0f, const float epsRel = 0.0f) const
    {
        if (UT::VectorAssertZero(
                &m_data[0][0], 12, verbose, str, epsAbs, epsRel))
        {
            return true;
        }
        else if (verbose)
    ...
    static inline AlignedMatrix2x6f GetRandom(
    ...
    static inline void ApB(const AlignedMatrix2x6f& A,
        const AlignedMatrix2x6f& B, AlignedMatrix2x6f& ApB)
    {
    ...
    static inline void AmB(const AlignedMatrix2x6f& A,
        const AlignedMatrix2x6f& B, AlignedMatrix2x6f& AmB)
    {
    ...
    static inline void Ab(
        const AlignedMatrix2x6f& A, const AlignedVector6f& b, Vector2f& Ab)
    {
    ...
    static inline void Ab(
        const AlignedMatrix2x6f& A, const ProductVector6f& b, Vector2f& Ab)
    {
    ...
    static inline void ABT(const AlignedMatrix2x6f& A,
        const AlignedMatrix2x6f& B, AlignedMatrix2x2f& ABT)
    {
    ...
    static inline void ABT(const AlignedMatrix2x6f& A,
        const AlignedMatrix2x6f& B, SymmetricMatrix2x2f& ABT)
    {
        const xp128f t = A.m_04_05_10_11() * B.m_04_05_10_11();
        ABT.m00() =
            (A.m_00_01_02_03() * B.m_00_01_02_03()).vsum_all() + t[0] + t[1];
    ...
    static inline void ATb(
        const AlignedMatrix2x6f& A, const Vector2f& b, AlignedVector6f& ATb)
    {
...
public:
    ...
    inline EigenMatrix2x6f(
        const EigenMatrix2x3f& e_M0, const EigenMatrix2x3f& e_M1)
    {
        block<2, 3>(0, 0) = e_M0;
        block<2, 3>(0, 3) = e_M1;
    }
    inline EigenMatrix2x6f(
        const LA::AlignedMatrix2x3f& M0, const LA::AlignedMatrix2x3f& M1)
    {
    ...
    inline bool AssertEqual(const LA::AlignedMatrix2x6f& M,
        ...
        const float epsAbs = 0.0f, const float epsRel = 0.0f) const
    {
        return GetAlignedMatrix2x6f().AssertEqual(
    ...
    {
        return AssertEqual(
            e_M.GetAlignedMatrix2x6f(), verbose, str, epsAbs, epsRel);
    }
    inline bool AssertZero(const int verbose = 1, const std::string str = "",
        const float epsAbs = 0.0f, const float epsRel = 0.0f) const
    {
