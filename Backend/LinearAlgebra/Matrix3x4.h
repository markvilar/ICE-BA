...
public:
    ...
    inline bool AssertEqual(const AlignedMatrix3x4f& M, const int verbose = 1,
        const std::string str = "", const float epsAbs = 0.0f,
        const float epsRel = 0.0f) const
    {
        if (UT::VectorAssertEqual(
                &m00(), &M.m00(), 12, verbose, str, epsAbs, epsRel))
        {
            return true;
        }
        else if (verbose)
    ...
    // TODO(haomin) : add comments to this function. This function doesn't have
    // a unit test yet.
    static inline void ATBToSymmetric(const AlignedMatrix3x3f& A,
        const AlignedMatrix3x4f& B, AlignedMatrix3x4f& ATB)
    {
...
public:
    ...
    inline EigenMatrix3x4f(
        const EigenMatrix3x3f& e_M0, const EigenVector3f& e_M1)
    {
        block<3, 3>(0, 0) = e_M0;
        block<3, 1>(0, 3) = e_M1;
    }
    ...
    inline bool AssertEqual(const LA::AlignedMatrix3x4f& M,
        ...
        const float epsAbs = 0.0f, const float epsRel = 0.0f) const
    {
        return GetAlignedMatrix3x4f().AssertEqual(
            M, verbose, str, epsAbs, epsRel);
    }
    inline bool AssertEqual(const EigenMatrix3x4f& e_M, const int verbose = 1,
        const std::string str = "", const float epsAbs = 0.0f,
        const float epsRel = 0.0f) const
    {
        return AssertEqual(
            e_M.GetAlignedMatrix3x4f(), verbose, str, epsAbs, epsRel);
    }
};
#endif
