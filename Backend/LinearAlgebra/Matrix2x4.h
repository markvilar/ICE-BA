...
public:
    ...
    inline void Set(const float m00, const float m01, const float m02,
        const float m03, const float m10, const float m11, const float m12,
        const float m13)
    {
    ...
    inline bool AssertEqual(const AlignedMatrix2x4f& M, const int verbose = 1,
        const std::string str = "", const float epsAbs = 0.0f,
        const float epsRel = 0.0f) const
    {
        if (UT::VectorAssertEqual(
                &m00(), &M.m00(), 8, verbose, str, epsAbs, epsRel))
        {
            return true;
        }
        else if (verbose)
...
public:
    ...
    inline bool AssertEqual(const LA::AlignedMatrix2x4f& M,
        ...
        const float epsAbs = 0.0f, const float epsRel = 0.0f) const
    {
        return GetAlignedMatrix2x4f().AssertEqual(
            M, verbose, str, epsAbs, epsRel);
    }
    inline bool AssertEqual(const EigenMatrix2x4f& e_M, const int verbose = 1,
        const std::string str = "", const float epsAbs = 0.0f,
        const float epsRel = 0.0f) const
    {
        return AssertEqual(
            e_M.GetAlignedMatrix2x4f(), verbose, str, epsAbs, epsRel);
    }
};
#endif
