...
protected:
    ...
    void DrawStringAt(
        const int x, const int y, void* font, const char* format, ...);
    ...
    inline void DrawTexture(const TextureGL<CHANNELS>& tex)
    {
        tex.Bind();
        glBegin(GL_QUADS);
        glTexCoord2i(0, 0);
        glVertex2i(0, 0);
        glTexCoord2i(0, tex.GetHeight());
        glVertex2i(0, m_pWnd->size().y);
        glTexCoord2i(tex.GetWidth(), tex.GetHeight());
        glVertex2i(m_pWnd->size().x, m_pWnd->size().y);
        glTexCoord2i(tex.GetWidth(), 0);
        glVertex2i(m_pWnd->size().x, 0);
        glEnd();
    }
    template <int CHANNELS>
    inline void DrawTexture(const TextureGL<CHANNELS>& tex, const Point2D& x,
        const LA::Vector2f size)
    {
        tex.Bind();
        glBegin(GL_QUADS);
        glTexCoord2i(0, 0);
        glVertex2f(x.x() - size.x(), x.y() - size.y());
        glTexCoord2i(0, tex.GetHeight());
        glVertex2f(x.x() - size.x(), x.y() + size.y());
        glTexCoord2i(tex.GetWidth(), tex.GetHeight());
        glVertex2i(x.x() + size.x(), x.y() + size.y());
        glTexCoord2i(tex.GetWidth(), 0);
        glVertex2i(x.x() + size.x(), x.y() - size.y());
        glEnd();
    }
    template <int CHANNELS>
    inline void DrawTexture(const TextureGL<CHANNELS>& tex, const Point3D* Xs)
    {
        tex.Bind();
        glBegin(GL_QUADS);
        glTexCoord2i(0, 0);
    ...
    static void DrawQuad(
        const float x, const float y, const LA::Vector2f& size);
    ...
    static void DrawCovariance(
        const Point2D& x, const Point2DCovariance& S, const float X2);
