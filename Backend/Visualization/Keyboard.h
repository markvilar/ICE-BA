...
public:
    ...
    public:
        ...
        inline const int& operator=(const int value)
        {
            m_value = value;
            return m_value;
        }
        inline bool operator==(const Switch& key) const
        {
    ...
    public:
        inline Scalar() {}
        inline Scalar(const float value, const std::string info,
            const bool verbose = true, const float incr = 0.0f)
        {
            Set(value, info, verbose, incr);
        }
        inline operator const float&() const { return m_value; }
        inline const float& operator=(const float value)
        {
            m_value = value;
            return m_value;
        }
        inline void Set(const float value, const std::string info,
            const bool verbose = true, const float incr = 0.0f)
        {
            m_value = value;
            m_info = info;
        ...
        {
            Keyboard::Print("[%s] = %f", m_info.c_str(), m_value);
        }
        inline void SaveB(FILE* fp) const
        {
            UT::SaveB(m_value, fp);
    ...
    public:
        inline KeyPair() {}
        inline KeyPair(const int key1, const int key2)
            : m_key1(key1), m_key2(key2)
        {
        }
        inline bool operator<(const KeyPair& keyPair) const
        {
            return m_key1 < keyPair.m_key1 ||
                   m_key1 == keyPair.m_key1 && m_key2 < keyPair.m_key2;
        }
        inline bool operator==(const KeyPair& keyPair) const
        {
            return m_key1 == keyPair.m_key1 && m_key2 == keyPair.m_key2;
        }
...
public:
...
#ifdef CFG_DEBUG
        UT_ASSERT(
            Get(keyIncr, m_mapScalarIncr) == Get(keyDecr, m_mapScalarDecr));
#endif
    ...
    inline bool Press(const int key)
    {
        ...
        const std::map<int, KeyPair>::iterator itScalarIncr =
            m_mapScalarIncr.find(key);
        if (itScalarIncr != m_mapScalarIncr.end())
        {
            m_mapScalar.find(itScalarIncr->second)->second.Increase();
            return true;
        }
        const std::map<int, KeyPair>::iterator itScalarDecr =
            m_mapScalarDecr.find(key);
...
protected:
    ...
    static inline void Set(
        const KEY key, const VALUE& value, std::map<KEY, VALUE>& map)
    {
    ...
    static inline void Set(
        const KEY key, const VALUE_TYPE& value, std::map<KEY, VALUE>& map)
    {
#ifdef CFG_DEBUG
    ...
    template <class KEY, class VALUE>
    static inline const VALUE& Get(
        const KEY key, const std::map<KEY, VALUE>& map)
    {
#ifdef CFG_DEBUG
        UT_ASSERT(map.find(key) != map.end());
#endif
...
template <>
inline void Keyboard::Set<int, bool>(const int key, const bool& value)
{
    Set(key, Binary(value), m_mapBinary);
}
template <>
inline void Keyboard::Set<int, Keyboard::Switch>(
    const int key, const Keyboard::Switch& value)
{
    Set(key, value, m_mapSwitch);
}
template <> inline void Keyboard::Set<int, int>(const int key, const int& value)
{
    Set(key, value, m_mapSwitch);
}

template <> inline bool Keyboard::Get<bool>(const int key)
{
    return Get(key, m_mapBinary).m_value;
}
template <> inline int Keyboard::Get<int>(const int key)
{
    return Get(key, m_mapSwitch).m_value;
}
