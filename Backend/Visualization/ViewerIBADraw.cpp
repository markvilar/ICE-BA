...
//#if 0
    ...
    if (!m_pWnd)
    {
        if (m_keyPause)
        {
            ...
            const int inp =
                UT::Input<int>("[%d] [quit %d] [pause %d] [step %d] [save %d]",
                    GetLocalFrame(m_iLF)->m_T.m_iFrm,
                    VW_KEY_QUIT,
                    VW_KEY_XD_PAUSE,
                    VW_KEY_XD_STEP,
                    VW_KEY_XD_SAVE);
...
void ViewerIBA::DrawString()
{
    ...
    const GlobalBundleAdjustor::KeyFrame* KF =
        m_iFrmActive < nKFs ? &m_GBA->m_KFs[m_iKFActive] : NULL;
    const LocalFrame* LF =
        KF == NULL ? (LocalFrame*)GetLocalFrame(m_iLFActive) : NULL;
    if (KF)
    {
    ...
    if (KF && m_keyDrawCamTypeKF == DRAW_CAM_KF_LBA ||
        LF && m_keyDrawCamTypeLF == DRAW_CAM_LF_LBA)
    {
        Viewer::DrawStringTopLeft("LBA [%d] t %.3f", T.m_iFrm, T.m_t);
        Viewer::DrawStringCurrent("  fps %.2f  sync %.0f  norm %.0f  schur "
                                  "%.0f  cam %.0f  dep %.0f  dl %.0f",
            m_LBA->m_ts[LocalBundleAdjustor::TM_TOTAL].GetFPS(),
            m_LBA->m_ts[LocalBundleAdjustor::TM_SYNCHRONIZE]
                .GetAverageMilliseconds(),
            m_LBA->m_ts[LocalBundleAdjustor::TM_FACTOR]
                .GetAverageMilliseconds(),
            m_LBA->m_ts[LocalBundleAdjustor::TM_SCHUR_COMPLEMENT]
                .GetAverageMilliseconds(),
            m_LBA->m_ts[LocalBundleAdjustor::TM_CAMERA]
                .GetAverageMilliseconds(),
            m_LBA->m_ts[LocalBundleAdjustor::TM_DEPTH].GetAverageMilliseconds(),
            m_LBA->m_ts[LocalBundleAdjustor::TM_UPDATE]
                .GetAverageMilliseconds());
    }
    else if (KF && m_keyDrawCamTypeKF == DRAW_CAM_KF_GBA)
    {
        ...
        Viewer::DrawStringCurrent("  fps %.2f  sync %.0f  norm %.0f  schur "
            ...
            m_GBA->m_ts[GlobalBundleAdjustor::TM_FACTOR]
                .GetAverageMilliseconds(),
            m_GBA->m_ts[GlobalBundleAdjustor::TM_SCHUR_COMPLEMENT]
                .GetAverageMilliseconds(),
            m_GBA->m_ts[GlobalBundleAdjustor::TM_CAMERA]
                .GetAverageMilliseconds(),
            m_GBA->m_ts[GlobalBundleAdjustor::TM_DEPTH]
                .GetAverageMilliseconds(),
            m_GBA->m_ts[GlobalBundleAdjustor::TM_UPDATE]
                .GetAverageMilliseconds());
    }
#ifdef CFG_GROUND_TRUTH
    else if (KF && m_keyDrawCamTypeKF == DRAW_CAM_KF_GT ||
             LF && m_keyDrawCamTypeLF == DRAW_CAM_LF_GT)
    {
...
#endif
                          T.m_fileName;
        Viewer::DrawStringTopLeft(
            1, GLUT_BITMAP_HELVETICA_18, "%s", fileName.c_str());
    }
    else if (m_keyDrawViewType == DRAW_VIEW_3D)
    {
        ...
        const Camera C =
            KF ? GetCameraGBALM(m_iKFActive) : GetCameraLF(m_iLFActive);
        if (KF && C.Invalid())
        {
    ...
    else if (m_keyDrawViewType == DRAW_VIEW_PROFILE && DrawProfileTypeValid())
    {
                     ...
                     DRAW_PRF_IMU_DELTA_BIAS_GYROSCOPE_COVARIANCE)
        {
            ...
            const IMU::Delta D =
                KF ? GetIMUDeltaGBALM(m_iKFActive) : GetIMUDeltaLF(m_iLFActive);
            ...
            case DRAW_PRF_IMU_DELTA_ROTATION_MEASUREMENT:
                str = "zdr";
                v = (D.GetRotationMeasurement(C1, BA_ANGLE_EPSILON))
            ...
            const int t =
                m_keyDrawPrfType < DRAW_PRF_IMU_DELTA_BIAS_ACCELERATION_STATE
                 ...
                 m_keyDrawPrfType <= DRAW_PRF_CAMERA_PRIOR_POSITION_COVARIANCE)
        {
            ...
            const int t =
                (m_keyDrawPrfType - DRAW_PRF_CAMERA_PRIOR_ROTATION_STATE) % 4;
            ...
            if (KF)
            {
                ...
                for (int iZ = 0, rowStart = 0; iZ < NZ; ++iZ)
                {
                    ...
                    else if (t == 0 || Zp.GetPriorMeasurement(BA_WEIGHT_FEATURE,
                                           &S,
                                           &x,
                                           NULL,
                                           &m_work,
                    ...
                    {
                        Viewer::DrawStringTopLeft(
                            ++rowStart, GLUT_BITMAP_HELVETICA_10, "INVALID");
                    }
                }
            }
            ...
            {
                if (t == 0 ||
                    m_LBA->m_Zp.GetPriorMeasurement(
                ...
                {
                    Viewer::DrawStringTopLeft(
                        1, GLUT_BITMAP_HELVETICA_10, "INVALID");
                }
            }
        }
                     ...
                     DRAW_PRF_CAMERA_PRIOR_BIAS_GYROSCOPE_COVARIANCE)
        {
            ...
            const int t =
                (m_keyDrawPrfType - DRAW_PRF_CAMERA_PRIOR_VELOCITY_STATE) % 4;
            const bool r =
            ...
            const float _eps[] = {epsp,
                ...
                epsbw,
                epsbw};
            if (t == 0 || m_LBA->m_Zp.GetPriorMeasurement(
                              BA_WEIGHT_FEATURE, &S, &x, NULL, &m_work, _eps))
            {
                ...
                case DRAW_PRF_CAMERA_PRIOR_VELOCITY_MEASUREMENT:
                    str = "zpv";
                    v = m_LBA->m_Zp.GetVelocityMeasurement(x.Data() + ip);
                    break;
                case DRAW_PRF_CAMERA_PRIOR_VELOCITY_ERROR:
                    str = "epv";
                    v = m_LBA->m_Zp.GetVelocityError(C, x.Data() + ip);
                    break;
                ...
                case DRAW_PRF_CAMERA_PRIOR_BIAS_GYROSCOPE_MEASUREMENT:
                    str = "zpbw";
                    v = m_LBA->m_Zp.GetBiasGyroscopeMeasurement(
                        x.Data() + ip + 6);
                    break;
                case DRAW_PRF_CAMERA_PRIOR_BIAS_GYROSCOPE_ERROR:
                    str = "epbw";
                    v = m_LBA->m_Zp.GetBiasGyroscopeError(C, x.Data() + ip + 6);
                    break;
                case DRAW_PRF_CAMERA_PRIOR_BIAS_GYROSCOPE_COVARIANCE:
                    str = "Spbw";
                    S.GetBlock(ip + 6, ip + 6, _S);
                    break;
                }
        ...
        {
            const UT::ES<FTR::ESError, FTR::ESIndex> ES =
                ComputeReprojectionError(
                    KF ? m_iKFActive : -1, LF ? m_iLFActive : -1);
            Viewer::DrawStringTopLeft(1,
                GLUT_BITMAP_HELVETICA_10,
                "reprojection error = %f <= %f [%d] %d\n",
                ES.Mean(),
                ES.Maximal(),
                ES.m_idxMax.m_ixFrm,
                ES.m_idxMax.m_ix);
        ...
        else if (m_keyDrawPrfType >= DRAW_PRF_STATE_ROTATION_ABSOLUTE)
        {
            ...
            {
                Rotation3D R = KF ? GetCameraKF(m_iKFActive)
                                  : GetCameraLF(m_iLFActive).m_T;
                str = "r";
                ...
                else
                {
                    if (m_keyDrawPrfType >= DRAW_PRF_STATE_ROTATION_RELATIVE)
                    {
                        ...
                        str = UT::String("[%d] --> [%d] ",
                                  GetKeyFrame(iKFNearest)->m_T.m_iFrm,
                                  T.m_iFrm) +
                              str;
                    }
#ifdef CFG_GROUND_TRUTH
                    ...
                    {
                        R = R /
                            (KF ? GetCameraKF(m_iKFActive, DRAW_CAM_KF_GT)
            ...
            {
                v = KF ? GetCameraKF(m_iKFActive).GetPosition()
                       : GetCameraLF(m_iLFActive).m_p;
                str = "p";
                const int iKFNearest = KF ? KF->m_iKFNearest : LF->m_iKFNearest;
                if (iKFNearest == -1)
                {
                ...
                else
                {
                    ...
                    {
                        v += GetCameraKF(iKFNearest, DRAW_CAM_KF_GT)
                                 .GetPosition();
                    }
                    if (m_keyDrawPrfType ==
                            DRAW_PRF_STATE_POSITION_RELATIVE_ERROR ||
                    ...
                    {
                        v -= KF ? GetCameraKF(m_iKFActive, DRAW_CAM_KF_GT)
                                      .GetPosition()
            ...
            else
            {
                ...
                {
                    const Camera CGT =
                        KF ? GetCameraGBALM(m_iKFActive, DRAW_CAM_KF_GT)
    ...
    if (KF)
    {
        Viewer::DrawStringBottomLeft("KF = %d / %d  x = %d / %d  z = %d",
            m_iKFActive,
            nKFs,
            KF->m_xs.size(),
            m_iKF2d.back(),
    ...
    {
        Viewer::DrawStringBottomLeft("KF = %d  LF = %d / %d  z = %d",
            nKFs,
            (m_iLFActive + nLFs - (m_iLF + 1)) % nLFs,
            nLFs,
            LF->m_zs.size());
    }
}
...
void ViewerIBA::DrawString(const std::string str, const int t, const bool r,
    LA::AlignedVector3f& v, LA::AlignedMatrix3x3f& S, int* rowStart)
{
    ...
    if (t < 3)
    {
        ...
        Viewer::DrawStringTopLeft(row + 1,
            GLUT_BITMAP_HELVETICA_10,
            "%s = %f %f %f",
            str.c_str(),
            v.x(),
        ...
        if (t == 2)
        {
            Viewer::DrawStringCurrent(
                GLUT_BITMAP_HELVETICA_10, " = %f", sqrtf(v.SquaredLength()));
        }
        if (rowStart)
        {
            ++*rowStart;
...
void ViewerIBA::DrawString(const CameraPrior::Pose& Zp,
    const LA::AlignedMatrixXf& S, const LA::AlignedVectorXf& x, int* rowStart)
{
#ifdef CFG_DEBUG
    UT_ASSERT(Zp.Valid());
    ...
    for (int i = -1, ip = 0; i < Nk; ip = ip + (i == -1 ? 2 : 6), ++i)
    {
        if (i == -1)
        {
            ...
            case DRAW_PRF_CAMERA_PRIOR_ROTATION_MEASUREMENT:
                str = "zr";
                {
                    const Rotation3D eR = Zp.GetReferenceRotationMeasurement(
                        x.Data(), BA_ANGLE_EPSILON);
                    v = eR.GetRodrigues(BA_ANGLE_EPSILON);
                    break;
                }
            case DRAW_PRF_CAMERA_PRIOR_ROTATION_ERROR:
                str = "er";
                v = Zp.GetReferenceRotationError(
                    C1, x.Data(), BA_ANGLE_EPSILON);
                break;
        ...
        else
        {
            ...
            const Rigid3D C2 =
                iKF2 == INT_MAX ? GetCameraLF(iLF).m_T : GetCameraKF(iKF2);
            switch (m_keyDrawPrfType)
            {
            case DRAW_PRF_CAMERA_PRIOR_ROTATION_STATE:
                str = "xpr";
            ...
            case DRAW_PRF_CAMERA_PRIOR_ROTATION_MEASUREMENT:
                str = "zpr";
                {
                    const Rotation3D R = Zp.GetRotationMeasurement(
                        i, x.Data() + ip + 3, BA_ANGLE_EPSILON);
            ...
            case DRAW_PRF_CAMERA_PRIOR_ROTATION_ERROR:
                str = "epr";
                {
                    v = Zp.GetRotationError(
                        C1, C2, i, x.Data() + ip + 3, BA_ANGLE_EPSILON);
                    break;
                }
            case DRAW_PRF_CAMERA_PRIOR_ROTATION_COVARIANCE:
                str = "Spr";
            ...
            str = UT::String("[%d] --> [%d] ",
                      iFrm1,
                      (iKF2 == INT_MAX ? GetLocalFrame(iLF) : GetKeyFrame(iKF2))
                          ->m_T.m_iFrm) +
                  str;
        }
...
void ViewerIBA::DrawTimeLine()
{
    ...
    const int wh = VW_STRING_BORDER_X, h = VW_STRING_BORDER_Y + VW_STRING_GAP_Y,
              w = wh + wh;
    const float ratio = static_cast<float>(W - w) / (nFrms - 1);
    const Point2D sizeQuad(ratio * 0.5f, h);
    float x = static_cast<float>(wh);
    float y =
        H - 1 - (VW_STRING_BORDER_Y + VW_STRING_GAP_Y + VW_STRING_HEIGHT + h);
    glLineWidth(1.0f);
    glEnable(GL_BLEND);
    ...
    if (m_iFtrActive.Valid())
    {
        ...
        for (int iKF = 0; iKF < nKFs; ++iKF)
        {
            if (iKF != m_iFtrActive.m_ix.m_iKF &&
                GetKeyFrame(iKF)->SearchFeatureMeasurement(
                    m_iFtrActive.m_ix.m_iKF, m_iFtrActive.m_ix.m_ix) == -1)
            {
                continue;
            }
            x = wh + iKF * ratio;
        ...
        for (int iFrm = nKFs, iLF = nLFs == 0 ? 0 : (m_iLF + 1) % nLFs;
             iFrm < nFrms;
             ++iFrm, iLF = (iLF + 1) % nLFs)
        {
            if (GetLocalFrame(iLF)->SearchFeatureMeasurement(
                    m_iFtrActive.m_ix.m_iKF, m_iFtrActive.m_ix.m_ix) == -1)
            {
                continue;
            }
        ...
        if (m_iFtrActive.m_iz.Valid())
        {
            ...
            {
                x = wh + (nKFs + (m_iFtrActive.m_iz.m_iLF + nLFs - m_iLF - 1) %
                                     nLFs) *
                             ratio;
            }
            Viewer::DrawBox(x, y, sizeQuad);
    ...
    else
    {
        ...
        if (activeKF)
        {
            ...
            for (int iZ = 0; iZ < NZ; ++iZ)
            {
                ...
                if (Zp.m_iKFr == m_iKFActive)
                {
                    Zp.GetPriorMeasurement(
                        BA_WEIGHT_FEATURE, &S, NULL, NULL, &m_work, _eps);
...
void ViewerIBA::DrawTimeLine(const CameraPrior::Pose& Zp,
    const LA::AlignedMatrixXf& S, std::vector<float>* alphas,
...
void ViewerIBA::Draw2DFeatures()
{
    ...
    for (int iZ = 0; iZ < NZ; ++iZ)
    {
        ...
        const bool _activeKF =
            !activeKF && iZ == NZ - 1 && GetKeyFrame(Z.m_iKF)->m_T == LF.m_T;
...
void ViewerIBA::Draw2DFeature(const FeatureIndex::Source& ix, const int iz)
{
    ...
    const bool activeKF = m_iFrmActive < nKFs,
               activeCF = !activeKF && m_iLFActive == m_iLF;
    const FRM::Frame* F =
...
#endif
    const bool activePch = /*!activeCF && */ m_iFtrActive.m_ix == ix ||
                           activeCF && m_iFtrActive.m_ic.m_iz == iz;
    Point2D x;
    if (iz == -1)
    {
    ...
    if (m_keyDrawFtrType == DRAW_FTR_MATCH && m_iFrmActiveLast != -1)
    {
        ...
        const int _iLFActive =
            (m_iFrmActiveLast - nKFs + m_iLF + 1) % GetLocalFrames();
        if (_activeKF && m_iFrmActiveLast == ix.m_iKF)
        {
            const FTR::Source& __x =
                ((KeyFrame*)GetKeyFrame(ix.m_iKF))->m_xs[ix.m_ix];
#ifdef CFG_STEREO
        ...
        {
            const FRM::Frame& _F = *(_activeKF ? GetKeyFrame(m_iFrmActiveLast)
                                               : GetLocalFrame(_iLFActive));
            ...
            ))
    {
        ...
        Viewer::DrawCovariance(
            x, S, -logf(m_keyDrawCovProb) * m_keyDrawCovScale);
    }
    if (activePch)
    {
...
void ViewerIBA::Draw2DProjections()
{
    ...
    const FRM::Frame& F =
        *(activeKF ? GetKeyFrame(m_iKFActive) : GetLocalFrame(m_iLFActive));
    const Rigid3D C =
        activeKF ? GetCameraKF(m_iKFActive) : GetCameraLF(m_iLFActive).m_T;
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            const bool trk =
                ...
                activeKF && iKF == m_iKFActive && m_rightActive &&
                    KF.m_xs[ix].m_xr.Valid()
#endif
                ;
            if ((m_keyDrawPrjType == DRAW_PRJ_ALL ||
                    m_keyDrawPrjType == DRAW_PRJ_TRACKED) &&
                trk)
            {
                glColor3ub(255, 255, 0);
            ...
            if ((m_keyDrawPrjType == DRAW_PRJ_ALL ||
                    m_keyDrawPrjType == DRAW_PRJ_UNTRACKED) &&
                !trk && prj)
            {
                glColor3ub(0, 255, 255);
                Draw2DProjection(iKF, ix, iz);
...
void ViewerIBA::Draw2DProjection(const int iKF, const int ix, const int iz)
{
    ...
    const bool activeKF = m_iFrmActive < GetKeyFrames();
#ifdef CFG_STEREO
    const Rigid3D C = activeKF
                          ? GetCameraKF(m_iKFActive, -1, m_rightActive)
    ...
    const FRM::Frame& F =
        *(activeKF ? GetKeyFrame(m_iKFActive) : GetLocalFrame(m_iLFActive));
#ifdef CFG_STEREO
    const bool vxr = activeKF && iKF == m_iKFActive && m_rightActive &&
                     KF.m_xs[ix].m_xr.Valid();
#endif
    const bool vz = iz != -1
#ifdef CFG_STEREO
    ...
    if ((m_keyDrawErrType2D == DRAW_ERR_ALL ||
            m_keyDrawErrType2D == DRAW_ERR_MEAN ||
            m_iFtrActive.m_ix.Equal(iKF, ix)) &&
        vz)
    {
        int clr[4];
...
void ViewerIBA::Get2DProjectionRange(
    const Point2D& x, const LA::Vector2f& Jx, const float s2, Point2D* xs)
{
...
static inline void Rectify(const Intrinsic::RectificationMap& RM,
    const CVD::Image<TYPE_SRC>& Id, CVD::Image<TYPE_DST>& Ir)
{
    ...
    for (int yr = 0, i = 0; yr < h; ++yr)
        for (int xr = 0; xr < w; ++xr, ++i)
        {
            ...
            else
            {
                ...
                UT::ImageInterpolate(Id[iy1][ix1],
                    Id[iy2][ix1],
                    Id[iy1][ix2],
                    Id[iy2][ix2],
                    RM.m_ws[i],
...
void ViewerIBA::Draw3DCameras()
{
...
#ifdef CFG_STEREO
                    const Rigid3D C =
                        GetCameraKF(iKF, DRAW_CAM_KF_GT, m_rightActive);
#else
...
#ifdef CFG_STEREO
                    const Camera C =
                        GetCameraLF(iLF, DRAW_CAM_LF_GT, m_rightActive);
#else
    ...
    const Rigid3D C =
        activeKF ? GetCameraKF(m_iKFActive) : GetCameraLF(m_iLFActive).m_T;
#endif
    if (C.Valid() && (activeKF && m_keyDrawMotTypeKF != DRAW_MOT_KF_NONE ||
                         !activeKF && m_keyDrawMotTypeLF != DRAW_MOT_LF_NONE))
    {
        m_frustrumActive.SetPose(
            C, m_keyDrawAxis == DRAW_AXIS_WORLD_AND_CAMERA);
...
void ViewerIBA::Draw3DCameras(
    const std::vector<float>& alphas, const float alphaMin, const int iKFr)
{
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            if ((m_keyDrawStrType == DRAW_STR_CONVERGED && !converge ||
                    m_keyDrawStrType == DRAW_STR_NOT_CONVERGED && converge) &&
                !activeFtr)
            {
                continue;
            }
            ...
            if (m_keyDrawErrType3D == DRAW_ERR_ALL ||
                m_keyDrawErrType3D == DRAW_ERR_COVARIANCE || activeFtr)
            {
                if (converge)
                {
                    glBegin(GL_POINTS);
...
//#if 1
        ...
        if (iz != -1) {
          ...
          Viewer::DrawCovariance(x, Sx, X2);
            ...
            // glDepthFunc(GL_ALWAYS);
            if (m_keyDrawErrType3D == DRAW_ERR_ALL ||
                m_keyDrawErrType3D == DRAW_ERR_MEAN || activeFtr)
            {
                xc[0].Invalidate();
                if (m_keyDrawDepTypeCMP != m_keyDrawDepType)
                {
                    const Depth::InverseGaussian dc =
                        GetFeatureDepth(iKF, ix, m_keyDrawDepTypeCMP);
                    if (!dc.Invalid())
                    {
            ...
            {
                glColor3ub(0, 0, 255);
                // Viewer::DrawBox(x.x(), x.y(), 0.05f * _d.GetProjectedD(_Cr,
                // KF.m_xs[ix]));
...
void ViewerIBA::Draw3DDepthPlane()
{
    ...
    const Depth::InverseGaussian d =
        activeKF ? GetFrameDepthKF(m_iKFActive) : GetFrameDepthLF(m_iLFActive);
...
void ViewerIBA::Draw3DDepthPlane(const Rigid3D& C, const float d, Point3D* Xs)
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        {
        case 0:
            x.Set(-m_K.m_K.fxIcx(), -m_K.m_K.fyIcy());
            break;
        case 1:
            x.Set(-m_K.m_K.fxIcx(), m_K.m_K.fyIcy());
            break;
...
void ViewerIBA::Reset3DProjectionMatrix()
{
    ...
    glTranslatef((m_K.m_K.cx() * 2.0f) / m_K.m_K.w() - 1.0f,
        1.0f - (m_K.m_K.cy() * 2.0f) / m_K.m_K.h(),
        0.0f);
...
void ViewerIBA::Reset3DViewPoint(const bool active)
{
    ...
    m_frustrum.SetShape(z * VW_CAMERA_DEPTH_TO_SCENE_DEPTH_RATIO,
        xzRatio,
        yzRatio,
        m_keyDrawCamSize,
        m_keyDrawAxisLen);
    m_frustrumActive.SetShape(z * VW_CAMERA_DEPTH_TO_SCENE_DEPTH_RATIO,
        xzRatio,
        yzRatio,
        m_keyDrawCamSize * VW_CAMERA_SIZE_ACTIVE_RATIO,
        m_keyDrawAxisLen);
    // m_translation.MakeZero();
    m_arcball.Initialize(m_Cv.LA::AlignedMatrix3x3f::GetTranspose(),
        z,
        UnProjectWindowSize(z) * VW_ARCBALL_RADIUS_TO_WINDOW_SIZE_RATIO);
    m_arcball.GetRotationTranspose().Apply(
        m_Cv.GetTranslation() - m_arcball.GetCenter(), m_translation);
...
void ViewerIBA::Update3DViewPoint()
{
    ...
    m_Cv.SetTranslation(m_arcball.GetRotation().GetApplied(
                            m_translation - m_arcball.GetCenter()) +
                        m_arcball.GetCenter());
}
...
void ViewerIBA::UnProjectWindowPoint(
    const CVD::ImageRef& x, Point3D& X, const float z)
{
    double Xx, Xy, Xz;
    gluUnProject(x.x,
        m_pWnd->size().y - x.y,
...
void ViewerIBA::DrawProfile()
{
    ...
    const float hi = H * 0.2f, hhi = hi * 0.5f,
                whi = m_K.m_K.w() * hhi / m_K.m_K.h();
    const Point2D xi(dx + whi,
        H - VW_STRING_BORDER_Y - VW_STRING_HEIGHT - VW_STRING_GAP_Y - hhi);
    glEnable(GL_TEXTURE_RECTANGLE_ARB);
    Update2DImageTexture();
    ...
    // const float y1 = yi + hhi + VW_STRING_GAP_Y +
    // Viewer::GetStringHeight(GLUT_BITMAP_HELVETICA_10) + VW_STRING_GAP_Y; const
    // float y2 = H - VW_STRING_BORDER_Y - VW_STRING_HEIGHT - VW_STRING_GAP_Y;
    const float y1 = Viewer::GetStringYTopLeft(2, GLUT_BITMAP_HELVETICA_10) +
                     VW_STRING_GAP_Y;
    ...
    const bool r =
        (m_keyDrawPrfType >= DRAW_PRF_GYROSCOPE &&
            m_keyDrawPrfType <= DRAW_PRF_GYROSCOPE_DEVICE) ||
    ...
    switch (m_keyDrawPrfType)
    {
    case DRAW_PRF_ACCELERATION:
        str = "a";
        break;
    case DRAW_PRF_ACCELERATION_DEVICE:
        str = "a'";
        break;
    case DRAW_PRF_GYROSCOPE:
        str = "w";
        break;
    case DRAW_PRF_GYROSCOPE_DEVICE:
        str = "w'";
        break;
    case DRAW_PRF_IMU_DELTA_ROTATION_STATE:
        str = "xdr";
    ...
    case DRAW_PRF_CAMERA_PRIOR_BIAS_GYROSCOPE_COVARIANCE:
        str = "Spbw";
        break;
    case DRAW_PRF_REPROJECTION_ERROR:
        str = "reprojection error";
        break;
    case DRAW_PRF_STATE_ROTATION_ABSOLUTE:
    case DRAW_PRF_STATE_ROTATION_RELATIVE:
        str = "r";
        break;
    case DRAW_PRF_STATE_POSITION_ABSOLUTE:
    case DRAW_PRF_STATE_POSITION_RELATIVE:
        str = "p";
        break;
    case DRAW_PRF_STATE_VELOCITY:
        str = "v";
        break;
    case DRAW_PRF_STATE_BIAS_ACCELERATION:
        str = "ba";
        break;
    case DRAW_PRF_STATE_BIAS_GYROSCOPE:
        str = "bw";
        break;
#ifdef CFG_GROUND_TRUTH
    case DRAW_PRF_STATE_ROTATION_ABSOLUTE_ERROR:
    case DRAW_PRF_STATE_ROTATION_RELATIVE_ERROR:
        str = "r - r'";
        break;
    case DRAW_PRF_STATE_POSITION_ABSOLUTE_ERROR:
    case DRAW_PRF_STATE_POSITION_RELATIVE_ERROR:
        str = "p - p'";
        break;
    case DRAW_PRF_STATE_VELOCITY_ERROR:
        str = "v - v'";
        break;
    case DRAW_PRF_STATE_BIAS_ACCELERATION_ERROR:
        str = "ba - ba'";
        break;
    case DRAW_PRF_STATE_BIAS_GYROSCOPE_ERROR:
        str = "bw - bw'";
        break;
#endif
    }
    ...
    const int ws =
        Viewer::GetStringWidth(GLUT_BITMAP_HELVETICA_10, str.c_str());
    x -= ws * 0.5f;
    if (x < dx)
    {
        x = dx;
    ...
    Viewer::DrawStringAt(x,
        yb - B - Viewer::GetStringHeight(GLUT_BITMAP_HELVETICA_10),
        GLUT_BITMAP_HELVETICA_10,
        "%s",
        str.c_str());
    glLineWidth(1.0f);
        ...
        m_keyDrawPrfType <= DRAW_PRF_GYROSCOPE_DEVICE)
    {
        ...
        const bool d = m_keyDrawPrfType == DRAW_PRF_ACCELERATION_DEVICE ||
                       m_keyDrawPrfType == DRAW_PRF_GYROSCOPE_DEVICE;
        x = dx + ratio;
        for (int iFrm1 = 0, iFrm2 = 1; iFrm2 < nFrms;
             iFrm1 = iFrm2++, x = ratio + x)
        {
            if (iFrm1 == nKFs)
            {
                continue;
            ...
            const int iLF1 =
                iKF1 == -1 ? (iFrm1 - nKFs + m_iLF + 1) % nLFs : -1;
            const int iKF2 = iFrm2 < nKFs ? iFrm2 : -1;
            const int iLF2 =
                iKF2 == -1 ? (iFrm2 - nKFs + m_iLF + 1) % nLFs : -1;
            const Camera C1 =
                iKF1 != -1 ? GetCameraGBALM(iKF1) : GetCameraLF(iLF1);
            if (d && C1.Invalid())
            {
                continue;
            }
            ...
            const LA::AlignedVector3f b = a ? C1.m_ba : C1.m_bw;
            const float t1 =
                (iKF1 != -1 ? GetKeyFrame(iKF1) : GetLocalFrame(iLF1))->m_T.m_t;
            const GlobalBundleAdjustor::KeyFrame* KF2 =
                iKF2 != -1 ? &m_GBA->m_KFs[iKF2] : NULL;
            const LocalFrame* LF2 =
                !KF2 ? (LocalFrame*)GetLocalFrame(iLF2) : NULL;
            const float _ratio =
                ratio / ((KF2 ? KF2->m_T.m_t : LF2->m_T.m_t) - t1);
            const AlignedVector<IMU::Measurement>& us =
                KF2 ? KF2->m_us : LF2->m_us;
            const int Nu = us.Size();
            for (int i = 0; i < Nu; ++i)
            {
                const IMU::Measurement& u = us[i];
             ...
             m_keyDrawPrfType <= DRAW_PRF_IMU_DELTA_BIAS_GYROSCOPE_COVARIANCE)
    {
             ...
             iFrm1 = iFrm2++, x = ratio + x)
        {
            ...
            const Camera C1 =
                iKF1 != -1 ? GetCameraGBALM(iKF1) : GetCameraLF(iLF1);
            const Camera C2 =
                iKF2 != -1 ? GetCameraGBALM(iKF2) : GetCameraLF(iLF2);
            const IMU::Delta D =
                iKF2 != -1 ? GetIMUDeltaGBALM(iKF2) : GetIMUDeltaLF(iLF2);
    ...
    else if (m_keyDrawPrfType == DRAW_PRF_REPROJECTION_ERROR)
    {
        ...
        for (int iFrm = 0; iFrm < nFrms; ++iFrm, x = ratio + x)
        {
            ...
            const UT::ES<FTR::ESError, FTR::ESIndex> ES =
                ComputeReprojectionError(iKF, iLF);
            m_work.Push(x);
            m_work.Push(ES.Mean());
    ...
    else if (m_keyDrawPrfType >= DRAW_PRF_STATE_ROTATION_ABSOLUTE)
    {
        ...
        for (int iFrm = 0; iFrm < nFrms; ++iFrm, x = ratio + x)
        {
            const int iKF = iFrm < nKFs ? iFrm : -1;
            const int iLF = iKF == -1 ? (iFrm - nKFs + m_iLF + 1) % nLFs : -1;
            const int iKFNearest =
                (iKF != -1 ? GetKeyFrame(iKF) : GetLocalFrame(iLF))
                    ->m_iKFNearest;
            if (m_keyDrawPrfType < DRAW_PRF_STATE_POSITION_ABSOLUTE)
            {
                if (m_keyDrawPrfType >= DRAW_PRF_STATE_ROTATION_RELATIVE &&
                ...
                else
                {
                    ...
                    {
                        R = R / (iKF != -1
                                        ? GetCameraKF(iKF, DRAW_CAM_KF_GT)
            ...
            else if (m_keyDrawPrfType < DRAW_PRF_STATE_VELOCITY)
            {
                ...
                {
                    v = iKF != -1 ? GetCameraKF(iKF).GetPosition()
                                  : GetCameraLF(iLF).m_p;
                    if (m_keyDrawPrfType >= DRAW_PRF_STATE_POSITION_RELATIVE)
                    {
                        v -= GetCameraKF(iKFNearest).GetPosition();
                            ...
                            DRAW_PRF_STATE_POSITION_ABSOLUTE_ERROR)
                    {
                        v -=
                            iKF != -1
                                ? GetCameraKF(iKF, DRAW_CAM_KF_GT).GetPosition()
            ...
            {
                const Camera C =
                    iKF != -1 ? GetCameraGBALM(iKF) : GetCameraLF(iLF);
                if (C.Invalid())
                {
                    continue;
                }
                ...
                if (((m_keyDrawPrfType - DRAW_PRF_STATE_BIAS_ACCELERATION) & 1))
                {
                    const Camera CGT = iKF != -1
                                           ? GetCameraGBALM(iKF, DRAW_CAM_KF_GT)
    ...
    const int n = m_keyDrawPrfType == DRAW_PRF_REPROJECTION_ERROR ? 3 : 4,
              N = m_work.Size() / n;
    int N1 = 0;
                 ...
                 DRAW_PRF_CAMERA_PRIOR_BIAS_GYROSCOPE_COVARIANCE)
    {
        ...
        const int t =
            (m_keyDrawPrfType - DRAW_PRF_CAMERA_PRIOR_VELOCITY_STATE) % 4;
        if (t == 0 || m_LBA->m_Zp.GetPriorMeasurement(
                          BA_WEIGHT_FEATURE, &S, &_x, NULL, &m_work, _eps))
        {
            const Camera C = GetCameraLF(m_LBA->m_ic2LF.front());
            ...
            x = dx + nKFs * ratio;
            glColor3ub(255, 0, 0);
            Viewer::DrawCross(Point2D(x, v.x() * sv + yb), _size);
            glColor3ub(0, 255, 0);
            Viewer::DrawCross(Point2D(x, v.y() * sv + yb), _size);
            glColor3ub(0, 0, 255);
            Viewer::DrawCross(Point2D(x, v.z() * sv + yb), _size);
        }
    }
}

void ViewerIBA::DrawProfile(const CameraPrior::Pose& Zp, const float dx,
    const float ratio, const float yb, const float b)
{
#ifdef CFG_DEBUG
    UT_ASSERT(Zp.Valid());
#endif
...
void ViewerIBA::DrawProfile(const CameraPrior::Pose& Zp, const float dx,
    const float ratio, const float yb, const float sv, const LA::Vector2f& size,
    ...
    for (int i = -1, ip = 0; i < Nk; ip = ip + (i == -1 ? 2 : 6), ++i)
    {
        if (i == -1)
        {
            ...
            case DRAW_PRF_CAMERA_PRIOR_ROTATION_STATE:
                ...
                break;
            case DRAW_PRF_CAMERA_PRIOR_ROTATION_MEASUREMENT:
                v = Zp.GetReferenceRotationMeasurement(
                          x.Data(), BA_ANGLE_EPSILON)
                        .GetRodrigues(BA_ANGLE_EPSILON);
                break;
            case DRAW_PRF_CAMERA_PRIOR_ROTATION_ERROR:
                v = Zp.GetReferenceRotationError(
                    C1, x.Data() + ip, BA_ANGLE_EPSILON);
                break;
            case DRAW_PRF_CAMERA_PRIOR_ROTATION_COVARIANCE:
            {
                LA::Vector2f vr;
        ...
        glColor3ub(255, 0, 0);
        Viewer::DrawCross(Point2D(_x, v.x() * sv + yb), size);
        glColor3ub(0, 255, 0);
        Viewer::DrawCross(Point2D(_x, v.y() * sv + yb), size);
        glColor3ub(0, 0, 255);
        Viewer::DrawCross(Point2D(_x, v.z() * sv + yb), size);
    }
}
...
bool ViewerIBA::DrawProfileTypeValid(const int iFrm)
{
    ...
    {
        return iKF != -1
                   ? GetCameraKF(iKF).Valid()
                   : GetCameraLF(iLF).Valid()
#ifdef CFG_GROUND_TRUTH
                         && ((m_keyDrawPrfType -
...
void ViewerIBA::DrawProfileTypeStep(
    const int* types, const int N, const bool next)
{
    const int typeBkp = m_keyDrawPrfType;
    int i =
        static_cast<int>(std::upper_bound(types, types + N, typeBkp) - types) -
        1;
...
int ViewerIBA::GetDrawProfileScaleType()
{
    ...
    {
        return DRAW_PRF_STATE_VELOCITY +
               (m_keyDrawPrfType - DRAW_PRF_STATE_VELOCITY) / 2 * 2;
    }
    else
    {
...
UT::ES<FTR::ESError, FTR::ESIndex> ViewerIBA::ComputeReprojectionError(
    const int iKF, const int iLF)
{
#ifdef CFG_DEBUG
    ...
    for (int iZ = 0; iZ < NZ; ++iZ)
    {
        ...
        *Tr = C / _C;
        ...
        for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz)
        {
            ...
            const FTR::ESIndex idx(
                KF.m_T.m_iFrm, z.m_ix /*, F.m_T.m_iFrm, iz*/);
#ifdef CFG_STEREO
            for (int i = 0; i < 2; ++i)
#endif
            {
                ...
                const float r2 =
                    LA::SymmetricMatrix2x2f::MahalanobisDistance(W, _e);
    ...
    if (iKF != -1)
    {
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            const float r2 =
                LA::SymmetricMatrix2x2f::MahalanobisDistance(x.m_Wr, e.m_er);
