...
public:
    ...
    virtual bool Run(const bool visualize = true, const bool step = true,
        const int iFrm = -1);
    virtual int Start(
        const std::string viewFile = "", const bool pause = false);
    virtual void Stop(
        const std::string viewFile = "", const bool pause = false);
...
protected:
    ...
    enum DrawTimeLineType
    {
        DRAW_TLN_NONE,
        DRAW_TLN_FEATURE_MATCH,
        DRAW_TLN_PRIOR
    };
    enum DrawCameraTypeKF
    {
        DRAW_CAM_KF_LBA,
        DRAW_CAM_KF_GBA,
    ...
    enum DrawBackgroundColor
    {
        DRAW_BG_BLACK,
        DRAW_BG_WHITE,
        DRAW_BG_COLORS
    };
    enum DrawProfileType
    {
        DRAW_PRF_ACCELERATION,
    ...
    public:
        ...
        inline bool operator==(const FeatureIndex& iFtr) const
        {
            return m_ix == iFtr.m_ix && m_iz == iFtr.m_iz && m_ic == iFtr.m_ic;
        }
    ...
    public:
        ...
        public:
            ...
            inline bool operator==(const Source& ix) const
            {
                return m_iKF == ix.m_iKF && m_ix == ix.m_ix;
            }
            inline bool operator!=(const Source& ix) const
            {
                return m_iKF != ix.m_iKF || m_ix != ix.m_ix;
            }
            inline void Set(const int iKF, const int ix)
            {
                m_iKF = iKF;
                m_ix = ix;
            }
            inline bool Equal(const int iKF, const int ix) const
            {
                return m_iKF == iKF && m_ix == ix;
            }
        ...
        {
        public:
            inline bool operator==(const Measurement& iz) const
            {
                return m_iFrm == iz.m_iFrm && m_iKF == iz.m_iKF &&
                       m_iLF == iz.m_iLF && m_iz == iz.m_iz;
            }
            inline bool operator!=(const Measurement& iz) const
            {
                return m_iFrm != iz.m_iFrm || m_iKF != iz.m_iKF ||
            ...
            inline void SetKF(const int iFrm, const int iKF, const int iz)
            {
                m_iFrm = iFrm;
                m_iKF = iKF;
                m_iLF = -1;
            ...
            inline void SetLF(const int iFrm, const int iLF, const int iz)
            {
                m_iFrm = iFrm;
                m_iKF = -1;
                m_iLF = iLF;
            ...
            inline bool EqualKF(const int iKF, const int iz) const
            {
                return m_iKF == iKF && m_iz == iz;
            }
            inline bool EqualLF(const int iLF, const int iz) const
            {
                return m_iLF == iLF && m_iz == iz;
            }
            ...
            {
                UT_ASSERT(Invalid() || m_iKF != -1 && m_iLF == -1 ||
                          m_iKF == -1 && m_iLF != -1);
            }
        ...
        {
        public:
            inline bool operator==(const Current& ic) const
            {
                return m_iFrm == ic.m_iFrm && m_iz == ic.m_iz;
            }
            inline bool operator!=(const Current& ic) const
            {
                return m_iFrm != ic.m_iFrm || m_iz != ic.m_iz;
            }
            inline void operator=(const Measurement& iz)
            {
                m_iFrm = iz.m_iFrm;
                m_iz = iz.m_iz;
            }
            inline void Set(const int iFrm, const int iz)
            {
                m_iFrm = iFrm;
                m_iz = iz;
            }
            inline bool Equal(const int iFrm, const int iz) const
            {
                return m_iFrm == iFrm && m_iz == iz;
            }
...
protected:
    ...
    virtual Depth::InverseGaussian GetFrameDepthKF(
        const int iKF, const int type = -1);
    virtual Depth::InverseGaussian GetFrameDepthLF(
        const int iLF, const int type = -1);
    virtual Depth::InverseGaussian GetFeatureDepth(
        const int iKF, const int ix, const int type = -1);
    ...
    virtual bool DragActiveFrame(
        const CVD::ImageRef& from, const CVD::ImageRef& to);
    virtual void ActivateFeature(
        const FeatureIndex::Source& ix, const int iz = -1);
    virtual bool SearchActiveFeature(const CVD::ImageRef& x);
    virtual void SearchActiveFeature(
        const Point2D& x, const int iKF = -1, int ix = -1, const int iz = -1);
    ...
    virtual void PrintFeatureTrack(const int iFrm, const int iz,
        const Intrinsic& K, const Point2D& z, const Point2D& zp,
        const LA::SymmetricMatrix2x2f& W, const std::string str = " ");
    virtual void PrintFeatureTrackKF(const int iKFx, const int ix,
        const int iKFz, const std::string str = " ");
    virtual void PrintFeatureTrackLF(const int iKFx, const int ix,
        const int iLFz, const std::string str = " ");
#ifdef CFG_STEREO
    ...
    virtual int CountFeatureMatchesMeasurement(
    ...
    virtual void DrawString(const std::string str, const int t, const bool r,
        LA::AlignedVector3f& v, LA::AlignedMatrix3x3f& S, int* rowStart = NULL);
    virtual void DrawString(const CameraPrior::Pose& Zp,
        const LA::AlignedMatrixXf& S, const LA::AlignedVectorXf& x,
        int* rowStart = NULL);
    virtual void DrawTimeLine();
    virtual int DrawTimeLine(std::vector<float>* alphas,
        const float alphaMin = 0.0f, const float alphaMax = 1.0f,
        const bool localFrm = true);
    virtual void DrawTimeLine(const CameraPrior::Pose& Zp,
        const LA::AlignedMatrixXf& S, std::vector<float>* alphas,
        const float alphaMin = 0.0f, const float alphaMax = 1.0f);

    virtual void Draw2DImage();
    virtual void Draw2DFeatures();
    virtual void Draw2DFeature(
        const FeatureIndex::Source& ix, const int iz = -1);
    virtual void Draw2DProjections();
    virtual void Draw2DProjection(
        const int iKF, const int ix, const int iz = -1);
    virtual void Get2DProjectionRange(
    ...
    virtual void Draw3DCameras(const std::vector<float>& alphas,
        const float alphaMin = 0.0f, const int iKFr = -1);
    virtual void Draw3DPoints();
    virtual void Draw3DWorldAxis();
    virtual void Draw3DDepthPlane();
    virtual void Draw3DDepthPlane(
        const Rigid3D& C, const float d, Point3D* Xs = NULL);
    ...
    virtual bool OnMouseDraging(
        const CVD::ImageRef& from, const CVD::ImageRef& to, const int button);
    virtual bool OnMouseDoubleClicked(
        const CVD::ImageRef& where, const int button);
    virtual void OnResize(const CVD::ImageRef& size);
    virtual void SaveB(FILE* fp);
    virtual void LoadB(FILE* fp);
    virtual void SaveScreen(
        const std::string fileName, const bool sizeImg = true);
...
protected:
    ...
    int m_iLF, m_iFrm, m_iKFActive, m_iLFActive, m_iLFActiveLast, m_iFrmActive,
        m_iFrmActiveLast;
#ifdef CFG_STEREO
    ...
    double m_projMatrix[16], m_projMatrixPano[16], m_modelMatrix[16],
        m_modelMatrixScale[16];
    int m_viewport[4];
    ...
    Keyboard::Switch m_keyDrawCamTypeKF, m_keyDrawCamTypeLF, m_keyDrawDepType,
        m_keyDrawDepTypeCMP;
    Keyboard::Binary m_keyDrawString;
    Keyboard::Switch m_keyDrawTlnType;
    Keyboard::Scalar m_keyDrawTlnMaxFtrMatches, m_keyDrawTlnPriorVarPos,
        m_keyDrawTlnPriorVarRot;
