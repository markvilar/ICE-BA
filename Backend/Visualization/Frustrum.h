...
public:
    inline void SetShape(const float z, const float xzRatio,
        const float yzRatio, const float size, const float axisLen)
    {
    ...
    inline void SetAxisLength(const float len)
    {
        m_len.vdup_all_lane(m_Xc.z() * len);
    }
    inline void SetPose(const Rigid3D& T, const bool axis)
    {
        ...
        // 4 plane vertexes' world coordinate are: X[R0] + Y[R1] + Z[R2] + Cc,
        // X[R0] - Y[R1] + Z[R2] + Cc,
        //                                       - X[R0] + Y[R1] + Z[R2] + Cc, -
        //                                       X[R0] - Y[R1] + Z[R2] + Cc,
