...
template <class HANDLER> class EventHandler : public CVD::GLWindow::EventHandler
{
...
public:
    ...
    typedef void (HANDLER::*CallBackMouseDown)(
        const CVD::ImageRef& where, const int button);
    typedef void (HANDLER::*CallBackMouseUp)(
        const CVD::ImageRef& where, const int button);
    typedef bool (HANDLER::*CallBackMouseDraging)(
        const CVD::ImageRef& from, const CVD::ImageRef& to, const int button);
    typedef bool (HANDLER::*CallBackMouseDoubleClick)(
        const CVD::ImageRef& where, const int button);
    typedef void (HANDLER::*CallBackResize)(const CVD::ImageRef& size);

public:
    inline EventHandler()
        : m_quit(false), m_keyDown(false), m_mouseDown(false), m_handler(NULL),
          m_keyDownCB(NULL), m_keyUpCB(NULL), m_mouseMoveCB(NULL),
          m_mouseDownCB(NULL), m_mouseUpCB(NULL), m_mouseDragingCB(NULL),
          m_mouseDoubleClickCB(NULL), m_resizeCB(NULL)
    {
    }
    ...
    virtual void on_mouse_move(
        CVD::GLWindow& win, CVD::ImageRef where, int state)
    {
    ...
    virtual void on_mouse_down(
        CVD::GLWindow& win, CVD::ImageRef where, int state, int button)
    {
        m_mouseDown = true;
        m_button = button;
    ...
    inline void Initialize(HANDLER* handler, CallBackKeyDown keyDownCB,
        CallBackKeyUp keyUpCB, CallBackMouseMove mouseMoveCB,
        CallBackMouseDown mouseDownCB, CallBackMouseUp mouseUpCB,
        CallBackMouseDraging mouseDragingCB,
