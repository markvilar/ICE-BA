...
void Viewer::Initialize(const int w, const int h)
{
    ...
    m_handler.Initialize(this,
        ...
        &Viewer::OnMouseDown,
        &Viewer::OnMouseUp,
        &Viewer::OnMouseDraging,
        &Viewer::OnMouseDoubleClicked,
        &Viewer::OnResize);
}
...
void Viewer::DrawStringTopLeft(
    const int row, void* font, const char* format, ...)
{
#ifdef CFG_DEBUG
    UT_ASSERT(row >= 1);
#endif
...
//{
//  glRasterPos2i(VW_STRING_BORDER_X, m_pWnd->size().y - VW_STRING_BORDER_Y -
//  VW_STRING_HEIGHT * row); char str[UT_STRING_WIDTH_MAX];
//  UT_GET_STRING(format, str);
//  DrawString(VW_STRING_FONT, str);
//}
