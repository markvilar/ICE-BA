...
//#ifdef CFG_DEBUG
  ...
  if (key == '+' || key == '-') {
    ...
    case VW_KEY_XD_SCREEN:
        ...
        {
            bool resize = m_pWnd->size().x != m_K.m_K.w() ||
                          m_pWnd->size().y != m_K.m_K.h();
            if (resize)
            {
    ...
    // return true; case VW_KEY_XD_ACTIVATE_LAST_FRAME:
    // ActivateFrame(m_iFrmActive - 1);  return true;
    ...
    case VW_KEY_XD_DRAW_CAMERA_TYPE_LAST:
        if (activeKF)
        {
            m_keyDrawCamTypeKF.Press(
                key == VW_KEY_XD_DRAW_CAMERA_TYPE_NEXT, false);
    ...
    case VW_KEY_XD_INPUT_ACTIVE_FEATURE:
    {
        ...
        if (iKF != m_GBA->m_KFs.end() && *iKF == iFrm)
        {
            ...
            const FRM::Frame& F = *(activeKF ? GetKeyFrame(m_iKFActive)
                                             : GetLocalFrame(m_iLFActive));
    ...
    if (m_keyDrawViewType == DRAW_VIEW_2D || m_keyDrawViewType == DRAW_VIEW_3D)
    {
        ...
        case VW_KEY_XD_DRAW_AXIS_LENGTH_DECREASE:
            ...
            {
                m_keyDrawAxisLen.Press(
                    key == VW_KEY_XD_DRAW_AXIS_LENGTH_INCREASE);
        ...
        case VW_KEY_XD_DRAW_DEPTH_VARIANCE_DECREASE:
            m_keyDrawDepVar.Press(
                key == VW_KEY_XD_DRAW_DEPTH_VARIANCE_INCREASE);
            return true;
                ...
                (m_keyDrawViewType == DRAW_VIEW_2D &&
                    ...
                    m_keyDrawViewType == DRAW_VIEW_3D &&
                        (m_keyDrawErrType3D == DRAW_ERR_ALL ||
                            m_keyDrawErrType3D == DRAW_ERR_COVARIANCE)))
            {
                m_keyDrawCovScale.Press(
                    key == VW_KEY_XD_DRAW_COVARIANCE_SCALE_INCREASE);
                return true;
            }
            break;
    ...
    else if (m_keyDrawViewType == DRAW_VIEW_PROFILE)
    {
        ...
        case VM_KEY_PROFILE_LEVEL_3_LAST:
        {
            ...
            while (m_keyDrawPrfType.Press(
                       key == VM_KEY_PROFILE_LEVEL_3_NEXT, false) &&
...
void ViewerIBA::OnMouseMove(const CVD::ImageRef& where)
{
    ...
    // m_projMatrix, m_viewport, &Xx, &Xy, &Xz); UT::Print("\r%e %e %e\t\t\t",
    // Xx, Xy, Xz);
}
...
void ViewerIBA::OnMouseDown(const CVD::ImageRef& where, const int button)
{
    ...
    else if (m_keyDrawViewType == DRAW_VIEW_3D)
    {
        ...
        else if (button == VW_MOUSE_BUTTON_RIGHT)
        {
            ...
            UnProjectWindowPoint(
                where, m_whereMouseDown3DStart, m_arcball.GetCenterZ());
        }
    }
}
    ...
    else if (m_keyDrawViewType == DRAW_VIEW_3D)
    {
            ...
            button == VW_MOUSE_BUTTON_MIDDLE_DOWN)
        {
            ...
            const float factor = button == VW_MOUSE_BUTTON_MIDDLE_DOWN
                                     ...
                                     : -m_factorWinToTranslationZ;
            m_translation.xyzr() =
                m_arcball.GetRotation().r_20_21_22_x() * factor +
                m_translation.xyzr();
        }
...
bool ViewerIBA::OnMouseDoubleClicked(
    const CVD::ImageRef& where, const int button)
{
    ...
    {
        const Point2D x = Point2D(
            where.x * m_factorWinToImg.x(), where.y * m_factorWinToImg.y());
...
void ViewerIBA::OnResize(const CVD::ImageRef& size)
{
    ...
    m_viewport[2] = size.x;
    m_viewport[3] = size.y;
    m_factorImgToWin.Set(
        float(size.x) / m_K.m_K.w(), float(size.y) / m_K.m_K.h());
    m_factorWinToImg.Set(
        1.0f / m_factorImgToWin.x(), 1.0f / m_factorImgToWin.y());
