        ...
        const Rigid3D* Ts = NULL, const Rotation3D* Ru = NULL)
    {
        ...
        while (fgets(line, UT_STRING_WIDTH_MAX, fp))
        {
            ...
            else if (n2 == 7 || n2 == 8 || n2 == 17)
            {
                ...
                {
                    q.xyzw().vset_all_lane(
                        float(_v[4]), float(_v[5]), float(_v[6]), float(_v[3]));
                }
                if (flag & CT_FLAG_INVERSE)
                {
                    q.Inverse();
...
//#ifdef CFG_DEBUG
    ...
    inline int Search(
        const float t, Camera& Ci, float& ti, const float dtMax = 0.0f) const
    {
    ...
    static inline void TransformPose(
        const Rigid3D& T, AlignedVector<Camera>& Cs)
    {
        const int N = Cs.Size();
        for (int i = 0; i < N; ++i)
        {
    ...
    static inline void TransformBias(const Rotation3D& R,
        AlignedVector<Camera>& Cs, LA::AlignedVector3f& ba,
    ...
    static inline void AlignToOrigin(
        AlignedVector<Camera>& Cs, const bool g = true, const bool m = true)
    {
        if (Cs.Empty())
        {
            return;
