    ...
    const LA::SymmetricMatrix2x2f* Wx2)
{
  ...
  if (Wx2) {
    ...
    UT::Print(
        "e = %f", f * sqrtf((d->GetProjected(t12, x1) - x2).SquaredLength()));
#endif
#endif
    ...
    for (int iIter = 0; iIter < DEPTH_TRI_MAX_ITERATIONS; ++iIter)
    {
        ...
        const float xGN = x;
        const float F =
            Wx2 ? LA::SymmetricMatrix2x2f::MahalanobisDistance(*Wx2, e)
                : e.SquaredLength();
        ...
        for (int iIterDL = 0; iIterDL < DEPTH_TRI_DL_MAX_ITERATIONS; ++iIterDL)
        {
            ...
            const float dFa =
                F -
                (Wx2 ? LA::SymmetricMatrix2x2f::MahalanobisDistance(*Wx2, ea)
                     : ea.SquaredLength());
            const float dFp =
                F -
                (Wx2 ? LA::SymmetricMatrix2x2f::MahalanobisDistance(*Wx2, ep)
                     : ea.SquaredLength());
        ...
        if (iIter == 0)
        {
            ...
            UT::Print("%se = %f\n",
                ...
                f * sqrtf((InverseGaussian(0.0f).GetProjected(t12, x1) - x2)
                              .SquaredLength()));
        }
...
bool Triangulate(const float w, const int N, const Measurement* zs,
    InverseGaussian* d, AlignedVector<float>* work, const bool initialized,
    float* eAvg)
{
    if (N == 0)
    {
    ...
    work->Resize(
        (J.BindSize(Nx2) * 3 + (DEPTH_TRI_ROBUST ? _w.BindSize(N) : 0)) /
        sizeof(float));
...
//#if 1
      UT::Print("%d %f %f\n", i, a, b);
        ...
        {
            F +=
                LA::SymmetricMatrix2x2f::MahalanobisDistance(zs[i].m_W, eis[i]);
        }
        const float dBkp = d->u();
        bool update = true, converge = false;
        for (int iIterDL = 0; iIterDL < DEPTH_TRI_DL_MAX_ITERATIONS; ++iIterDL)
        ...
        if (iIter == 0)
        {
            ...
            UT::Print("%se = %f  z = %f\n",
                std::string(str.size(), ' ').c_str(),
                f * ComputeError(N, zs, d0),
                1.0f / d0.u());
        }
        UT::Print("%se = %f  z = %f  x = %f\n",
            str.c_str(),
            f * ComputeError(N, zs, *d),
            1.0f / d->u(),
            x);
#endif
    }
