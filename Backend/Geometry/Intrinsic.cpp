...
//#if 1
    if (UT::Debugging()) {
      UT::Print("%d %e %e\n", iIter, xn->x(), xn->y());
        ...
        {
            const float dr2I =
                1.0f / (ds[7] * r6 + ds[6] * r4 + ds[5] * r2 + 1.0f);
            dr *= dr2I;
...
//#ifdef CFG_DEBUG
    ...
    A.Set(J.m00(), J.m01(), J.m11());
        ...
        if (!UM)
        {
            ...
            update = true;
            converge = false;
            for (int iIterDL = 0; iIterDL < FTR_UNDIST_DL_MAX_ITERATIONS;
                 ++iIterDL)
            {
                ...
                *xn += dx;
                ...
                {
                    delta2 = std::max(
                        delta2, FTR_UNDIST_DL_RADIUS_FACTOR_INCREASE * dx2);
        ...
        {
            UT::PrintSeparator();
            m_k.GetNormalizedToImage(xd).Print(
                std::string(str.size(), ' ') + "x = ", false, true);
        }
