...
public:
    ...
    inline void MakeIdentity() { xyzw().vset_all_lane(1.0f, 0.0f, 0.0f, 0.0f); }

    inline void FromVectors(
        ...
        const std::string str = "") const
    {
        ...
        // if (UT::AssertEqual(SIMD::Dot012(kth1.xyzw(), kth2.xyzw()), 1.0f) &&
        // UT::AssertEqual(kth1.w(), kth2.w()))
        const float d = kth1.xyzw().vdot012(kth2.xyzw());
        // if (UT::AssertEqual(UT_DOT_TO_ANGLE(d), 0.0f) &&
        // UT::AssertEqual(kth1.w(), kth2.w()))
        const float eps = 0.1f;
        // const float eps = 0.01f;
        if (UT::AssertZero(UT_DOT_TO_ANGLE(d),
                verbose,
                str + ".dot",
...
public:
    ...
    inline void Slerp(
        const float w1, const Quaternion& q1, const Quaternion& q2)
    {
        x() = q1.Dot(q2);
        if (x() > 0.0f)
        {
    ...
    inline void Slerp(const Quaternion& q1, const Quaternion& q2,
    ...
    inline bool AssertEqual(const Quaternion& q, const int verbose = 1,
        const std::string str = "", const float eps = 0.001745329252f) const
    {
        ...
        // return q1.LA::AlignedVector4f::AssertEqual(q2, verbose, str, epsAbs,
        // epsRel); const float eps = 0.1f * UT_FACTOR_DEG_TO_RAD;
    ...
    static inline void AB(
        const Quaternion& A, const Quaternion& B, Quaternion& AB)
    {
        xp128f t;
        t = A.xyzw() * B.x();
        AB.x() = t[3];
    ...
    static inline void AIB(
        const Quaternion& A, const Quaternion& B, Quaternion& AIB)
    {
        xp128f t;
        t = A.xyzw() * B.x();
        AIB.x() = -t[3];
...
public:
    ...
    static inline void GetTranspose(
        const LA::AlignedVector3f& v, LA::AlignedMatrix3x3f& M)
    {
        M.m00() = 0.0f;
        M.m01() = v.z();
        M.m02() = -v.y();
    ...
    static inline void Ab(const LA::AlignedVector3f& a,
    ...
    static inline void AddAbTo(const LA::AlignedVector3f& a,
        const LA::AlignedVector3f& b, LA::AlignedVector3f& Ab)
    {
    ...
    static inline void AddATbTo(const LA::AlignedVector3f& a,
        const LA::AlignedVector3f& b, LA::AlignedVector3f& ATb)
    {
    ...
    static inline void AB(const LA::AlignedVector3f& a,
        const LA::AlignedMatrix3x3f& B, LA::AlignedMatrix3x3f& AB)
    {
    ...
    static inline void ATB(const LA::AlignedVector3f& a,
        const LA::AlignedMatrix3x3f& B, LA::AlignedMatrix3x3f& AB)
    {
        const float ax = a.x(), ay = a.y(), az = a.z();
    ...
    static inline void AB(const LA::AlignedMatrix3x3f& A,
        const LA::AlignedVector3f& b, LA::AlignedMatrix3x3f& AB)
    {
    ...
    static inline void ATB(const LA::AlignedMatrix3x3f& A,
        const LA::AlignedVector3f& b, LA::AlignedMatrix2x3f& AB)
    {
    ...
    static inline void ATBT(const LA::AlignedMatrix3x3f& A,
        const LA::AlignedVector3f& b, LA::AlignedMatrix2x3f& AB)
    {
    ...
    static inline void ABT(const LA::AlignedMatrix3x3f& A,
        const LA::AlignedVector3f& b, LA::SymmetricMatrix3x3f& AB)
    {
        AB.m00() = A.m02() * b.y() - A.m01() * b.z();
        AB.m01() = A.m00() * b.z() - A.m02() * b.x();
        AB.m02() = A.m01() * b.x() - A.m00() * b.y();
    ...
    static inline void ABTToUpper(const LA::AlignedMatrix3x3f& A,
        const LA::AlignedVector3f& b, LA::AlignedMatrix3x3f& AB)
    {
        AB.m00() = A.m02() * b.y() - A.m01() * b.z();
        AB.m01() = A.m00() * b.z() - A.m02() * b.x();
    ...
    static inline void AddABTTo(const LA::AlignedMatrix3x3f& A,
        const LA::AlignedVector3f& b, LA::SymmetricMatrix3x3f& AB)
    {
    ...
    static inline void AddABTToUpper(const LA::AlignedMatrix3x3f& A,
        const LA::AlignedVector3f& b, LA::AlignedMatrix3x3f& AB)
    {
        AB.m00() = A.m02() * b.y() - A.m01() * b.z() + AB.m00();
        AB.m01() = A.m00() * b.z() - A.m02() * b.x() + AB.m01();
    ...
    static inline void AddABTTo03(const LA::AlignedMatrix3x3f& A,
        const LA::AlignedVector3f& b, LA::SymmetricMatrix6x6f& AB)
    {
...
public:
    ...
    inline Rotation3D(const float R[3][3])
    {
        Set(R);
        MakeOrthogonal();
    }
    ...
    inline void SetRodrigues(const LA::AlignedVector3f& w, const float eps)
    {
        ...
        const float t1 = UT_SINF(th) / th, t2 = (1.0f - UT_COSF(th)) / th2,
                    t3 = 1.0f - t2 * th2;
    ...
    static inline void GetRodriguesJacobian(const LA::AlignedVector3f& w,
        LA::AlignedMatrix3x3f& Jr, const float eps)
    {
        const LA::AlignedVector3f w2(w.xyzr() * w.xyzr());
        const float th2 = w2.Sum(), th = sqrtf(th2);
        if (th < eps)
    ...
    static inline LA::AlignedMatrix3x3f GetRodriguesJacobian(
        const LA::AlignedVector3f& w, const float eps)
    {
    ...
    static inline void GetRodriguesJacobianInverse(const LA::AlignedVector3f& w,
        LA::AlignedMatrix3x3f& JrI, const float eps)
    {
        const LA::AlignedVector3f w2(w.xyzr() * w.xyzr());
        const float th2 = w2.Sum(), th = sqrtf(th2);
        if (th < eps)
        ...
        const float t2 =
                        th2I - (1.0f + UT_COSF(th)) / (2.0f * th * UT_SINF(th)),
                    t3 = 1.0f - t2 * th2;
        const LA::AlignedVector3f t1w(w.xyzr() * t1);
        const LA::AlignedVector3f t2w2(w2.xyzr() * t2);
        const float t2wx = t2 * w.x();
    ...
    static inline void GetRodriguesJacobianInverseXY(
        const LA::AlignedVector3f& w, LA::AlignedMatrix2x3f& JrI,
        const float eps)
    {
        const float th2 = w.SquaredLength(), th = sqrtf(th2);
        if (th < eps)
    ...
    static inline LA::AlignedMatrix3x3f GetRodriguesJacobianInverse(
        const LA::AlignedVector3f& w, const float eps)
    {
    ...
    inline void SetAxisAngle(const AxisAngle& kth)
    {
        const float cth = UT_COSF(kth.w()), sth2 = 1.0f - cth * cth,
                    sth1 = sqrtf(sth2);
    ...
    inline void GetAxisAngle(AxisAngle& kth) const
    {
        const float tr = Trace(),
                    cth = (tr - 1.0f) * 0.5f /*, th = UT_ACOSF(cth)*/;
    ...
    static inline float GetAngle(const Rotation3D& R1, const Rotation3D& R2)
    {
        ...
        const float tr = (R1.r_00_01_02_x() * R2.r_00_01_02_x() +
                          ...
                          R1.r_20_21_22_x() * R2.r_20_21_22_x())
                             .vsum_012();
    ...
    inline void SetQuaternion(const Quaternion& q)
    {
        ...
        r_00_01_02_x() += r_00_01_02_x();
        r00() = -r00() + 1.0f;
        r_10_11_12_x() += r_10_11_12_x();
        r11() = -r11() + 1.0f;
        r_20_21_22_x() += r_20_21_22_x();
        r22() = -r22() + 1.0f;
    }
    ...
    inline void GetTranspose(Rotation3D& RT) const
    {
        LA::AlignedMatrix3x3f::GetTranspose(RT);
    }
    inline Rotation3D GetTranspose() const
    {
        Rotation3D RT;
        GetTranspose(RT);
        return RT;
    ...
    inline void GetGravity(LA::AlignedVector3f& g) const
    {
        GetColumn2(g);
        g.MakeMinus();
    }
    ...
    inline void Apply(
        const LA::AlignedVector3f& X, LA::AlignedVector3f& RX) const
    {
    ...
    inline void Apply(
        const Point2D& x, Point2D& Rx, LA::AlignedMatrix2x2f& Jx) const
    {
    ...
    inline Point2D GetApplied(const Point2D& x) const
    {
        Point2D Rx;
        Apply(x, Rx);
        return Rx;
    ...
    inline void ApplyInversely(
        const LA::AlignedVector3f& X, LA::AlignedVector3f& RTX) const
    {
        // TODO (yanghongtian) : computation order
        RTX.xyzr() = r_00_01_02_x() * X.x() + r_10_11_12_x() * X.y() +
                     r_20_21_22_x() * X.z();
    }
    inline LA::AlignedVector3f GetAppliedInversely(
        const LA::AlignedVector3f& X) const
    {
    ...
    {
        // TODO (yanghongtian) : computation order
        RTX.xyzw() = r_00_01_02_x() * X.x() + r_10_11_12_x() * X.y() +
                     r_20_21_22_x() * X.z();
        RTX.w() = 1.0f;
    }
        ...
        const std::string str = "", const float eps = 0.001745329252f) const
    {
        ...
        if ((eps < 0.0f && UT::VectorAssertEqual(
                               &r00(), &R.r00(), 12, verbose, str, eps)) ||
    ...
    inline bool AssertOrthogonalRows(const int verbose = 1,
        const std::string str = "", const float eps = 0.001745329252f) const
    {
        // const float eps = 0.1f * UT_FACTOR_DEG_TO_RAD;
        const float d00 = r_00_01_02_x().vdot012(r_00_01_02_x());
        if (!UT::AssertEqual(
                UT_DOT_TO_ANGLE(d00), 0.0f, verbose, str + ".rxTrx", eps))
        {
            return false;
        }
        const float d01 = r_00_01_02_x().vdot012(r_10_11_12_x());
        if (!UT::AssertEqual(UT_DOT_TO_ANGLE(d01) - UT_PI_2,
                0.0f,
                verbose,
                str + ".rxTry",
                eps))
        ...
        if (!UT::AssertEqual(UT_DOT_TO_ANGLE(d02) - UT_PI_2,
                0.0f,
                verbose,
                str + ".rxTrz",
                eps))
        ...
        if (!UT::AssertEqual(
                UT_DOT_TO_ANGLE(d11), 0.0f, verbose, str + ".ryTry", eps))
        {
            return false;
        }
        const float d12 = r_10_11_12_x().vdot012(r_20_21_22_x());
        if (!UT::AssertEqual(UT_DOT_TO_ANGLE(d12) - UT_PI_2,
                0.0f,
                verbose,
                str + ".ryTrz",
                eps))
        ...
        if (!UT::AssertEqual(
                UT_DOT_TO_ANGLE(d22), 0.0f, verbose, str + ".rzTrz", eps))
        {
            return false;
        }
        ...
        if (!UT::AssertEqual(
                UT_DOT_TO_ANGLE(dc0), 0.0f, verbose, str + ".rx", eps))
        {
            return false;
        }
        ...
        if (!UT::AssertEqual(
                UT_DOT_TO_ANGLE(dc1), 0.0f, verbose, str + ".ry", eps))
        {
            return false;
        }
        ...
        if (!UT::AssertEqual(
                UT_DOT_TO_ANGLE(dc2), 0.0f, verbose, str + ".rz", eps))
        {
            return false;
        }
        return true;
    ...
    static inline void AB(
        const Rotation3D& Ra, const Rotation3D& Rb, Rotation3D& Rab)
    {
        const Rotation3D RbT = Rb.GetTranspose();
        ABT(Ra, RbT, Rab);
    }
    static inline void ABT(
        const Rotation3D& Ra, const Rotation3D& Rb, Rotation3D& RaRbT)
    {
        LA::AlignedMatrix3x3f::ABT(Ra, Rb, RaRbT);
        RaRbT.MakeOrthogonal();
    }
    static inline void ATB(
        const Rotation3D& Ra, const Rotation3D& Rb, Rotation3D& RaTRb)
    {
...
public:
    ...
    {
        return AxisAngle(
            GetAxis().GetAlignedVector3f().GetNormalized(), GetAngle());
    }
    ...
    inline bool AssertEqual(
        const EigenAxisAngle& e_kth, const int verbose = 1) const
    {
...
public:
    ...
    inline void SetAxisAngle(const EigenAxisAngle& e_kth)
    {
        ...
        UT::AssertEqual(e_k.SquaredLength(), 1.0f);
#endif
        const float thh = e_kth.GetAngle() * 0.5f, sthh = sinf(thh),
                    cthh = cosf(thh);
        x() = e_k.x() * sthh;
        y() = e_k.y() * sthh;
        z() = e_k.z() * sthh;
    ...
    inline void GetAxisAngle(EigenAxisAngle& e_kth) const
    {
        ...
        e_kth.Set(EigenVector3f(e_k / sqrtf(e_k.squaredNorm())),
            UT_ACOSF(w()) * 2.0f);
    }
    ...
    inline bool AssertEqual(
        const EigenQuaternion& e_q, const int verbose = 1) const
    {
...
public:
    ...
    inline EigenSkewSymmetricMatrix(const Eigen::Matrix3f& e_M)
        : EigenMatrix3x3f(e_M)
    {
    }
...
public:
    ...
    inline EigenRotation3D GetTranspose() const
    {
        return EigenRotation3D(transpose());
    }
    inline void SetRodrigues(const EigenVector3f& e_w, const float eps)
    {
#if 0
    ...
    static inline EigenMatrix3x3f GetRodriguesJacobian(
        const EigenVector3f& e_w, const float eps)
    {
        // return
        // EigenMatrix3x3f(Rotation3D::GetRodriguesJacobian(e_w.GetAlignedVector3f()));
    ...
    static inline EigenMatrix3x3f GetRodriguesJacobianInverse(
        const EigenVector3f& e_w, const float eps)
    {
        // return
        // EigenMatrix3x3f(Rotation3D::GetRodriguesJacobianInverse(e_w.GetAlignedVector3f()));
        const EigenSkewSymmetricMatrix e_S(e_w);
        const float th = sqrtf(e_w.SquaredLength());
        if (th < eps)
        {
        ...
        {
            return EigenMatrix3x3f(
                EigenMatrix3x3f::Identity() + 0.5f * e_S +
                (1.0f / (th * th) -
                    (1.0f + cosf(th)) / (2.0f * th * sinf(th))) *
                    e_S * e_S);
        }
    }
