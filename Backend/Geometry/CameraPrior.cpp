...
bool Pose::Marginalize(
    const int i, AlignedVector<float>* work, const float* eps)
{
    ...
    work->Resize((A.BindSize(Np, Np, true) + b.BindSize(Np) +
                     _work.BindSize(Np) + (eps ? _eps.BindSize(6) : 0)) /
                 sizeof(float));
    ...
    bool scc = true;
    scc = A.MarginalizeLDL(ipr, 3, b, &_work, eps ? _eps.Data() + 3 : NULL) &&
          scc;
    scc = A.MarginalizeLDL(ipp - 3, 3, b, &_work, eps ? _eps.Data() : NULL) &&
          scc;
#else
    const bool scc =
...
void Pose::SetPriorEquation(
    const Matrix::X& A, const Vector::X& b, const bool g)
{
    const int N = static_cast<int>(m_iKFs.size());
    ...
    if (g && _g)
    {
        ...
        for (int i = 0, _ipr = ipr, _ipp = ipp; i < N;
             ++i, _ipr += 3, _ipp += 3)
        {
...
void Pose::GetPriorEquation(
    Matrix::X* A, Vector::X* b, const bool symmetric, const bool g) const
{
    const int N = static_cast<int>(m_iKFs.size());
    const bool _g = m_Zps.Size() != N;
    ...
    const float* eps) const
{
    ...
    if (!GetPriorMeasurement(static_cast<Element::T>(w),
            &_S,
            x ? &_x : NULL,
            xTb ? &_xTb : NULL,
            eps ? _eps.Data() : NULL))
    ...
    if (x)
    {
        ...
        LA::Vector3f xc;
        for (int i = 0, ipr1 = ipr, ipp1 = ipp, ipp2 = Npg, ipr2 = ipp2 + 3;
             i < N;
    ...
    for (int i = 0, ipr1 = ipr, ipp1 = ipp, ipp2 = Npg, ipr2 = ipp2 + 3; i < N;
         ++i, ipr1 += 3, ipp1 += 3, ipp2 += 6, ipr2 += 6)
    {
        for (int j = i, jpr1 = ipr1, jpp1 = ipp1, jpp2 = ipp2, jpr2 = ipr2;
             j < N;
    ...
    const float* eps) const
{
    ...
    work->Resize(
        (S->BindSize(9, 9) + (x ? x->BindSize(9) : 0) + _S.BindSize(9, 9) +
            (x ? _x.BindSize(_Nx) : 0) + (eps ? _eps.BindSize(9) : 0)) /
        sizeof(float));
    ...
    if (eps)
    {
        const LA::Vector3f *ev = (LA::Vector3f*)eps, *eba = ev + 1,
                           *ebw = eba + 1;
        _eps.SetBlock(0, *ebw);
        _eps.SetBlock(3, *ev);
        _eps.SetBlock(6, *eba);
...
//#ifdef CFG_DEBUG
  ...
  if (x) {
    ...
    work->Resize(
        (S->BindSize(9, 9) + (x ? x->BindSize(Nx) : 0)) / sizeof(float));
    S->Bind(work->Data(), 9, 9);
    if (x)
    {
        x->Bind(S->BindNext(), Nx);
...
void Joint::GetPriorEquation(
    Matrix::X* A, Vector::X* b, const bool symmetric) const
{
    const int N = static_cast<int>(m_iKFs.size());
    const bool g = m_Zps.Size() != N;
    const int Npg = g ? 2 : 0;
    ...
    const float* eps) const
{
    ...
    const int Npr = N * 3, Npp = Npr, Npc = Npr + Npp, Npgc = Npg + Npc,
              Np = Npgc + 9;
    const int ipr = 0, ipp = Npr;
    const int ipbw1 = Npc, ipv1 = ipbw1 + 3, ipg1 = ipv1 + 3,
              ipba1 = ipg1 + Npg;
    ...
    AlignedVector<float>* work, const float* eps)
{
    ...
    const int ipcs[10] = {
        ipp1, ipr1, ipv1, ipba1, ipbw1, ipp2, ipr2, ipv2, ipba2, ipbw2};
    ...
    bool scc = true;
    scc =
        _A.MarginalizeLDL(ipr1, 3, _b, &_work, eps ? _eps.Data() + 3 : NULL) &&
        scc;
    scc =
        _A.MarginalizeLDL(ipp1 - 3, 3, _b, &_work, eps ? _eps.Data() : NULL) &&
        scc;
    scc = _A.MarginalizeLDL(
              ipbw1 - 6, 3, _b, &_work, eps ? _eps.Data() + 12 : NULL) &&
          scc;
    scc = _A.MarginalizeLDL(
              ipv1 - 9, 3, _b, &_work, eps ? _eps.Data() + 6 : NULL) &&
          scc;
    scc = _A.MarginalizeLDL(
              ipba1 - 12, 3, _b, &_work, eps ? _eps.Data() + 9 : NULL) &&
          scc;
    // UT::DebugStart();
    // UT::Print("%.10e\n", _A[ipr1 + 1][ipr1 + 1]);
    ////UT::Print("%.10e\n", _A[ipr2 + 1][ipr2 + 1]);
    ////const bool scc1 = _A.MarginalizeLDL(ipr1, 3, _b, &_work, eps ?
    ///_eps.Data() + 3 : NULL); /UT::Print("%.10e\n", _A[ipr2 - 2][ipr2 - 2]);
    // const bool scc11 = _A.MarginalizeLDL(ipr1, 1, _b, &_work, eps ?
    // _eps.Data() + 3 : NULL); UT::Print("%.10e\n", _A[ipr1][ipr1]);
    ////UT::Print("%.10e\n", _A[ipr2][ipr2]);
    // const bool scc12 = _A.MarginalizeLDL(ipr1, 1, _b, &_work, eps ?
    // _eps.Data() + 3 : NULL); UT::Print("%.10e\n", _A[ipr2 - 1][ipr2 - 1]);
    // const bool scc13 = _A.MarginalizeLDL(ipr1, 1, _b, &_work, eps ?
    // _eps.Data() + 3 : NULL); UT::Print("%.10e\n", _A[ipr2 - 2][ipr2 - 2]);
    ...
    Element::CM &Ac1m1 = Acm1[Nk], &dAc1m1 = m_Acm[Nx2];
    dAc1m1.Set(
        A.m_Ap1v1, A.m_Ap1ba1, A.m_Ap1bw1, A.m_Ar1v1, A.m_Ar1ba1, A.m_Ar1bw1);
    Ac1m1 += dAc1m1;
    Element::CC& Ac1c2 = m_Acc[Nk][N];
    ...
    Element::MC& Am1c2 = Amc[1];
    Am1c2.Set(
        A.m_Av1p2, A.m_Av1r2, A.m_Aba1p2, A.m_Aba1r2, A.m_Abw1p2, A.m_Abw1r2);
    Element::CM& Ac2m1 = m_Acm[Nx2];
    Am1c2.GetTranspose(Ac2m1);
    Element::MM& Am2m1 = Amm[0];
...
bool Joint::PropagateLF(const IMU::Delta::Factor::Auxiliary::RelativeLF& A,
    LA::AlignedVectorXf* x, AlignedVector<float>* work, const float* eps) const
{
    ...
    AlignedVector<float>* work, const float* eps)
{
    ...
    bool scc = true;
    scc =
        _A.MarginalizeLDL(ipbw1, 3, _b, &_work, eps ? _eps.Data() + 6 : NULL) &&
        scc;
    scc =
        _A.MarginalizeLDL(ipv1 - 3, 3, _b, &_work, eps ? _eps.Data() : NULL) &&
        scc;
    scc = _A.MarginalizeLDL(
              ipba1 - 6, 3, _b, &_work, eps ? _eps.Data() + 3 : NULL) &&
          scc;
    // UT::DebugStart();
    // UT::Print("%f\n", _A[ipr2][ipg]);
    // const bool scc1 = _A.MarginalizeLDL(ipbw1, 3, _b, &_work, eps ?
    // _eps.Data() + 6 : NULL); UT::Print("%f\n", _A[ipr2][ipg - 3]); const bool
    // scc2 = _A.MarginalizeLDL(ipv1 - 3, 3, _b, &_work, eps ? _eps.Data() :
    // NULL); UT::Print("%f\n", _A[ipr2][ipg - 6]); const bool scc3 =
    // _A.MarginalizeLDL(ipba1 - 6, 3, _b, &_work, eps ? _eps.Data() + 3 : NULL);
    // UT::Print("%f\n", _A[ipr2][ipg - 6]);
...
bool Joint::GetPriorPose(
    const int iKF, Pose* Zp, AlignedVector<float>* work, const float* eps) const
{
#if defined CFG_CAMERA_PRIOR_DOUBLE || defined CFG_CAMERA_PRIOR_REORDER
    Zp->m_iKFr = m_iKFr;
    Zp->m_iKFs = m_iKFs;
    ...
    work->Resize((A.BindSize(Np, Np, true) + b.BindSize(Np) +
                     _work.BindSize(Np) + (eps ? _eps.BindSize(15) : 0)) /
                 sizeof(float));
    A.Bind(work->Data(), Np, Np, true);
    b.Bind(A.BindNext(), Np);
    _work.Bind(b.BindNext(), Np);
...
#else
        scc =
            A.MarginalizeLDL(Np - 15, 15, b, &_work, eps ? _eps.Data() : NULL);
#endif
    }
    else
    {
...
#else
        scc = A.MarginalizeLDL(
            Np - 9, 9, b, &_work, eps ? _eps.Data() + 6 : NULL);
#endif
    }
...
bool Joint::GetPriorMotion(
    Motion* Zp, AlignedVector<float>* work, const float* eps) const
{
#if defined CFG_CAMERA_PRIOR_DOUBLE || defined CFG_CAMERA_PRIOR_REORDER
    Zp->m_v = m_v;
    Zp->m_ba = m_ba;
...
#endif
    work->Resize((A.BindSize(Np, Np, true) + b.BindSize(Np) +
                     _work.BindSize(Np) + (eps ? _eps.BindSize(Np) : 0)) /
                 sizeof(float));
    A.Bind(work->Data(), Np, Np, true);
    b.Bind(A.BindNext(), Np);
    _work.Bind(b.BindNext(), Np);
...
//#if 0
    ...
    scc = A.MarginalizeLDL(
              ipg - Npc, 2, b, &_work, eps ? _eps.Data() + ipg : NULL) &&
          scc;
#else
    // UT::DebugStart();
    int ip = 4;
    ...
    {
        scc1 =
            A.MarginalizeLDL(0, 1, b, &_work, eps ? _eps.Data() : NULL) && scc1;
        if (UT::Debugging())
        {
            if (ip > 0)
            {
    ...
    const AlignedVector<Rigid3D>& Cs, const float eps) const
{
    ...
    for (int i = 0, j = 2, k = 6; i < N; ++i, j += 6, k += 6)
    {
        ...
        const Rigid3D &T2 = iKF == INT_MAX ? Cs.Back() : Cs[iKF],
                      &T21 = m_Zps[i];
        ...
        const EigenVector3f e_er =
            ((Rotation3D(T21) * Rotation3D(T2)) / Rotation3D(T1))
                .GetRodrigues(eps);
        ...
        const EigenVector3f e_ep2 = EigenVector3f(
            e_ep1 + e_Jpr1 * e_dr1 + e_Jpp1 * e_dp1 + e_Jpp2 * e_dp2);
        ...
        UT::AssertReduction(
            e_er1, e_er2, 1, UT::String("er[%d --> %d]", m_iKFr, iKF));
        UT::AssertReduction(
            e_ep1, e_ep2, 1, UT::String("ep[%d --> %d]", m_iKFr, iKF));
#endif
    }
#if 1
    ErrorJacobian Je;
...
Pose::EigenFactor Pose::EigenGetFactor(
...
void Pose::EigenGetResidual(
    const AlignedVector<Rigid3D>& Cs, EigenVectorXf* e_r, const float eps) const
{
    const EigenErrorJacobian e_Je = EigenGetErrorJacobian(Cs, eps);
...
void Pose::EigenGetPriorMeasurement(
    const float w, EigenMatrixXf* e_S, EigenVectorXf* e_x) const
{
    EigenPrior e_Ap;
    e_Ap.Set(*this);
    ...
    *e_S =
        e_ldlt.solve(EigenMatrixX<Element::T>::Identity(N)).cast<float>() * w;
    if (e_x)
    {
...
void Motion::EigenGetPriorMeasurement(
    const float w, EigenMatrixXf* e_S, EigenVectorXf* e_x) const
{
    EigenPrior e_Ap;
    e_Ap.Set(*this);
...
template <typename TYPE> class DebugEigenMatrixX : public EigenMatrixX<TYPE>
{
public:
...
#ifdef CFG_DEBUG
    UT_ASSERT(
        m_A.GetRows() == 17 && m_A.GetColumns() == 17 && m_b.Size() == 17);
#endif
    m_A.PushZero(15, 15);
...
void Joint::EigenPrior::GetPriorPose(
    const int iKF, Pose::EigenPrior* e_Ap) const
{
    EigenMatrixX<Element::T> e_Ab;
    e_Ab.Set(EigenMatrixX<Element::T>(m_A.cast<Element::T>()),
        EigenVectorX<Element::T>(m_b.cast<Element::T>()));
...
void Joint::EigenGetPriorMeasurement(
    const float w, EigenMatrixXf* e_S, EigenVectorXf* e_x) const
{
    EigenPrior e_Ap;
    e_Ap.Set(*this);
