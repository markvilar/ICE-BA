...
public:
    ...
    public:
        ...
        inline void ImageToNormalized(
            const TYPE xx, const TYPE xy, Point2D& xn) const
        {
            xn.x() = (xx - m_cx) * m_fxI;
            xn.y() = (xy - m_cy) * m_fyI;
        }
        ...
        template <typename TYPE>
        inline void ImageToNormalized(
            const TYPE xx, const TYPE xy, float& xn, float& yn) const
        {
            xn = (xx - m_cx) * m_fxI;
            yn = (xy - m_cy) * m_fyI;
        }
        inline void ImageToNormalized(
            const Point2DCovariance& S, Point2DCovariance& Sn) const
        {
            Sn.sxx() = S.sxx() * m_fxxI;
            Sn.sxy() = S.sxy() * m_fxyI;
            Sn.syy() = S.syy() * m_fyyI;
        ...
        inline void ImageToNormalized(
            const float S[2][2], Point2DCovariance& Sn) const
        {
            Sn.sxx() = S[0][0] * m_fxxI;
            Sn.sxy() = S[0][1] * m_fxyI;
            Sn.syy() = S[1][1] * m_fyyI;
        ...
        inline void AssertConsistency(
            const bool fishEye, const bool radial6, const bool tangential) const
        {
            UT_ASSERT(m_fxI == 1.0f / m_fx && m_fyI == 1.0f / m_fy);
            UT_ASSERT(m_fxx == m_fx * m_fx && m_fxy == m_fx * m_fy &&
                      m_fyy == m_fy * m_fy);
            UT_ASSERT(m_fxxI == 1.0f / m_fxx && m_fxyI == 1.0f / m_fxy &&
                      m_fyyI == 1.0f / m_fyy);
            if (fishEye)
            {
            }
    ...
    public:
        ...
        )
        {
            ...
            const bool fishEye = K.FishEye(), radial6 = K.Radial6(),
                       tangential = K.Tangential();
            ...
            for (int yr = 0, i = 0; yr < h; ++yr)
            {
                ...
                for (int xr = 0; xr < w; ++xr, ++i)
                {
                    ...
                    if (fishEye)
                    {
                        const float r = sqrtf(r2), t1 = UT_ATANF(r),
                                    t2 = t1 * t1;
                        const float t4 = t2 * t2, t6 = t2 * t4, t8 = t4 * t4;
                        const float s =
                            t1 *
                            (k.m_ds[3] * t8 + k.m_ds[2] * t6 + k.m_ds[1] * t4 +
                                k.m_ds[0] * t2 + 1.0f) /
                            r;
                        xd.x() = s * x;
                        xd.y() = s * y;
                    ...
                    else
                    {
                        ...
                        dr = k.m_ds[4] * r6 + k.m_ds[1] * r4 + k.m_ds[0] * r2 +
                             1.0f;
                        if (radial6)
                            dr /= k.m_ds[7] * r6 + k.m_ds[6] * r4 +
                                  k.m_ds[5] * r2 + 1.0f;
                        xd.x() = dr * x;
                        xd.y() = dr * y;
                        if (tangential)
                        {
                            const float dx =
                                d2y * x + k.m_ds[3] * (r2 + x2 + x2);
                            const float dy =
                                k.m_ds[2] * (r2 + y2 + y2) + d3y * t.x;
                            xd.x() = dx + xd.x();
                            xd.y() = dy + xd.y();
                        }
                    }
                    ...
                    {
                        m_xs[i].Set(ix, iy);
                        UT::ImageInterpolateWeight(
                            xd.x() - ix, xd.y() - iy, m_ws[i]);
                    }
                }
            }
        }
...
public:
    ...
    // [NOTE] GCC extension treats xp128f as __vector(4) float and allows direct
    // indexing
    ...
    inline LA::AlignedMatrix3x3f operator*(const LA::AlignedMatrix3x3f& M) const
    {
        ...
        KM.m_00_01_02_r0() =
            Fx() * M.m_00_01_02_r0() + Cx() * M.m_20_21_22_r2();
        KM.m_10_11_12_r1() =
            Fy() * M.m_10_11_12_r1() + Cy() * M.m_20_21_22_r2();
    ...
    inline void Set(const int w, const int h, const float fx,
        const float fy = 0.0f, const float cx = 0.0f, const float cy = 0.0f,
        const float* ds = NULL, const bool fishEye = false,
        const float fxr = 0.0f, const float fyr = 0.0f, const float cxr = 0.0f,
        const float cyr = 0.0f, const Rotation3D* Rr = NULL)
    {
        m_w = w;
        m_h = h;
        ...
        m_k.Set(fx,
            ...
            cy == 0.0f ? _cyr : cy);
        m_radial6 =
            ds && (ds[5] != 0.0f || ds[6] != 0.0f || ds[7] != 0.0f) ? 1 : 0;
        m_tangential = ds && (ds[2] != 0.0f || ds[3] != 0.0f) ? 1 : 0;
        m_needUndist =
            m_radial6 != 0 || m_tangential != 0 ||
                    (ds && (ds[0] != 0.0f || ds[1] != 0.0f || ds[4] != 0.0f))
                ? 1
                : 0;
...
//#if 1
    ...
    inline void NormalizedToImageN(
        const AlignedVector<Point2D>& xns, AlignedVector<Point2D>& xs) const
    {
        const int N = int(xs.Size()), NF = N - (N & 1);
        xs.Resize(N);
    ...
    inline void NormalizedToImageN(
        const Point2D* xns, const int N, Point2D* xs) const
    {
        const int NF = N - (N & 1);
    ...
    inline void NormalizedToImage(
        const LA::SymmetricMatrix2x2f& Sn, LA::SymmetricMatrix2x2f& S) const
    {
        S.m00() = Sn.m00() * fxx();
        S.m01() = Sn.m01() * fxy();
        S.m11() = Sn.m11() * fyy();
    }
    inline LA::SymmetricMatrix2x2f GetNormalizedToImage(
        const LA::SymmetricMatrix2x2f& Sn) const
    {
        Point2DCovariance S;
    ...
    inline void NormalizedToImage(
        const LA::AlignedMatrix3x3f& Hn, LA::AlignedMatrix3x3f& H) const
    {
        NormalizedToImage(*this, *this, Hn, H);
    }
    inline LA::AlignedMatrix3x3f GetNormalizedToImage(
        const LA::AlignedMatrix3x3f& Hn) const
    {
    ...
    static inline void NormalizedToImage(const Intrinsic& K1,
        const Intrinsic& K2, const LA::AlignedMatrix3x3f& Hn,
        LA::AlignedMatrix3x3f& H)
    {
        LA::AlignedMatrix3x3f M;
        M.m_00_01_02_r0() =
            K2.Fx() * Hn.m_00_01_02_r0() + K2.Cx() * Hn.m_20_21_22_r2();
        M.m_10_11_12_r1() =
            K2.Fy() * Hn.m_10_11_12_r1() + K2.Cy() * Hn.m_20_21_22_r2();
        ...
        H.m_20_21_22_r2() =
            M.m_20_21_22_r2() -
            (M.m_00_01_02_r0() * K1.FxIcx() + M.m_10_11_12_r1() * K1.FyIcy());
        H.Transpose();
    }
    ...
    inline void ImageToNormalized2(xp128f& x) const { x = (x - m_c) * m_fI; }
    inline void ImageToNormalized2(const xp128f& x, xp128f& xn) const
    {
        xn = (x - m_c) * m_fI;
    }
    ...
    inline void ImageToNormalizedN(
        const AlignedVector<Point2D>& xs, AlignedVector<Point2D>& xns) const
    {
        const int N = int(xs.Size()), NF = N - (N & 1);
        xns.Resize(N);
    ...
    inline void ImageToNormalizedN(
        const Point2D* xs, const int N, Point2D* xns) const
    {
        const int NF = N - (N & 1);
    ...
    inline Point2DCovariance GetImageToNormalized(
        const Point2DCovariance& S) const
    {
    ...
    {
        m_k.AssertConsistency(FishEye(), Radial6(), Tangential());
    }
    ...
    public:
        ...
        inline Point2D Get(const Point2D& xd) const
        {
...
#ifdef CFG_DEBUG
            UT_ASSERT(
                ix1 >= 0 && ix2 < m_xns.w() && iy1 >= 0 && iy2 < m_xns.h());
#endif
            ...
            UT::ImageInterpolate<float, float>(
                xn11.x(), xn12.x(), xn21.x(), xn22.x(), w, xn.x());
            UT::ImageInterpolate<float, float>(
                xn11.y(), xn12.y(), xn21.y(), xn22.y(), w, xn.y());
            return xn;
        }
    ...
    inline void Distort(const Point2D& xn, Point2D* xd) const
    {
        ...
        if (m_fishEye)
        {
            ...
            const float s =
                t1 *
                (ds[3] * t8 + ds[2] * t6 + ds[1] * t4 + ds[0] * t2 + 1.0f) / r;
            xd->x() = s * x;
            xd->y() = s * y;
        }
        else
        {
            ...
            if (m_tangential)
            {
                const float xy = x * y;
                const float dx =
                    ds[2] * (xy + xy) + ds[3] * (x2 + x2 + x2 + y2);
                const float dy =
                    ds[2] * (x2 + y2 + y2 + y2) + ds[3] * (xy + xy);
                xd->x() = dx + xd->x();
                xd->y() = dy + xd->y();
            }
        }
    ...
    bool Undistort(const Point2D& xd, Point2D* xn,
        LA::AlignedMatrix2x2f* JT = NULL, UndistortionMap* UM = NULL,
        const bool initialized = false) const;
...
protected:
    ...
    union
    {
        struct
        {
            ubyte m_needRect, m_needUndist, m_fishEye, m_radial6, m_tangential;
        };
        int m_flag[2];
    };
    xp128f m_f, m_fI, m_c;
