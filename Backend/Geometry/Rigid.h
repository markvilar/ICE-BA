...
public:
    ...
    public:
        ...
        inline Row operator/(const Rigid3D& Tb) const
        {
            Row TaTbI;
            Rigid3D::ABI(*this, Tb, TaTbI);
            return TaTbI;
        }
    };
...
public:
    ...
    inline Rigid3D(const Rigid3D& T, const LA::AlignedVector3f* dp,
        const LA::AlignedVector3f* dr, const float eps)
    {
        Point3D p;
        T.GetPosition(p);
        if (dp)
    ...
    {
        r00_r01_r02_tx().vset_all_lane(
            float(T[0]), float(T[1]), float(T[2]), float(T[3]));
        r10_r11_r12_ty().vset_all_lane(
            float(T[4]), float(T[5]), float(T[6]), float(T[7]));
        r20_r21_r22_tz().vset_all_lane(
    ...
    inline void ScaleTranslation(const float s)
    {
        tx() *= s;
        ty() *= s;
        tz() *= s;
    ...
    inline void GetRotationTranspose(Rotation3D& RT) const
    {
        Rotation3D::GetTranspose(RT);
    }
    ...
    inline void GetPosition(xp128f& p) const
    {
        ...
        p = r_00_01_02_x() * (-tx()) + r_10_11_12_x() * (-ty()) +
            r_20_21_22_x() * (-tz());
    }
    ...
    inline void ApplyRotation(const xp128f& X, xp128f& RX) const
    {
        Rotation3D::Apply(X, RX);
    }
    inline void ApplyRotation(const Point2D& x, LA::Vector3f& Rx) const
    {
        Rotation3D::Apply(x, Rx);
    }
    inline void ApplyRotationInversely(
        const LA::AlignedVector3f& X, LA::AlignedVector3f& RTX) const
    {
        Rotation3D::ApplyInversely(X, RTX);
    }
    inline LA::AlignedVector3f GetAppliedRotation(
        const LA::AlignedVector3f& X) const
    {
        return Rotation3D::GetApplied(X);
    }
    inline LA::AlignedVector3f GetAppliedRotationInversely(
        const LA::AlignedVector3f& X) const
    {
    ...
    inline void ApplyInversely(
        const Point2D& x, const float z, Point3D& X) const
    {
        ...
        const float tEps = 0.0f) const
    {
        ...
        if (Rotation3D::AssertEqual(T, verbose, str + ".m_R", rEps) &&
            GetTranslation().AssertEqual(
                T.GetTranslation(), verbose, str + ".m_t", tEps))
        {
            return true;
        }
        else if (verbose)
    ...
    static inline void ABI(const Rigid3D& Ta, const Rigid3D& Tb, Rigid3D& TabI)
    {
        ...
        TabI.MakeOrthogonal();
        TabI.SetTranslation(
            Ta.GetTranslation() - TabI.GetAppliedRotation(Tb.GetTranslation()));
    }
    static inline void ABI(const Row& Ta, const Rigid3D& Tb, Row& TabI)
    {
        LA::AlignedMatrix3x3f::aBT(Ta, Tb, TabI);
        TabI.t() = Ta.t() -
                   (TabI.r0_r1_r2_t() * Tb.GetTranslation().v012r()).vsum_012();
    }
};
#endif // _RIGID_H_
