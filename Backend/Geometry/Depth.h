...
public:
    ...
    inline bool operator==(const InverseGaussian& d) const
    {
    ...
    inline void Set(const float u, const float s2)
    {
        this->u() = u;
        this->s2() = s2;
    }
    inline void Set(const InverseGaussian& d)
    {
        u() = d.u();
        s2() = d.s2();
    }
    ...
    {
        s2() = DEPTH_VARIANCE_WALK * dt * dt + s2();
    }
    ...
    static inline bool Valid(const float d)
    {
        return d > DEPTH_MIN && d < DEPTH_MAX;
    }
    ...
    inline void Project(
        const Rigid3D& T12, const Point2D& x1, LA::Vector2f& x2) const
    {
        Project(T12, x1, u(), x2);
    }
    inline void Project(const LA::AlignedVector3f& t12, const Point2D& x1,
        LA::Vector2f& x2) const
    {
        Project(t12, x1, u(), x2);
    }
    inline void Project(const LA::AlignedVector3f& t12, const LA::Vector3f& Rx1,
        LA::Vector2f& x2) const
    {
        Project(t12, Rx1, u(), x2);
    }
    static inline void Project(const LA::AlignedVector3f& t12,
        const Point2D& x1, const float d1, LA::Vector2f& x2)
    {
    ...
    static inline void Project(const LA::AlignedVector3f& t12,
        const LA::Vector3f& Rx1, const float d1, LA::Vector2f& x2)
    {
        const LA::AlignedVector3f t = t12 * d1;
    ...
    static inline void Project(
        const Rigid3D& T12, const Point2D& x1, const float d1, LA::Vector2f& x2)
    {
    ...
    inline void Project(const LA::AlignedVector3f& t12, const Point2D& x1,
        LA::Vector2f& x2, LA::Vector2f& Jx2) const
    {
    ...
    inline void Project(const Rigid3D& T12, const Point2D& x1, LA::Vector2f& x2,
        float& d12, float& d2, LA::Vector2f& Jx2,
        LA::AlignedVector3f& R12x1) const
    {
        const xp128f t = xp128f::get(x1.x(), x1.y(), 1.0f, u());
    ...
    inline bool Project(const Rigid3D& T12, const Point2D& x1,
        const BoundingBox2D& B2, Point2D& x2) const
    {
        const xp128f t = xp128f::get(x1.x(), x1.y(), 1.0f, u());
    ...
    inline bool Project(const Rigid3D& T12, const Point2D& x1,
        const BoundingBox2D& B2, Point2D& x2, LA::Vector2f& Jx2) const
    {
        const xp128f t = xp128f::get(x1.x(), x1.y(), 1.0f, u());
        const float z12 = (T12.r20_r21_r22_tz() * t).vsum_all();
        if (z12 < FLT_EPSILON)
    ...
    inline bool Project(const Rigid3D& T12, const Point2D& x1,
        const BoundingBox2D& B2, Point2D& x2, float& d2,
        LA::Vector2f& Jx2) const
    {
        const xp128f t = xp128f::get(x1.x(), x1.y(), 1.0f, u());
        const float z12 = (T12.r20_r21_r22_tz() * t).vsum_all();
    ...
    inline Point2D GetProjected(
        const LA::AlignedVector3f& t12, const Point2D& x1) const
    {
        Point2D x2;
        Project(t12, x1, x2);
        return x2;
    ...
    inline Point2D GetProjected(
        const LA::AlignedVector3f& t12, const LA::Vector3f& Rx1) const
    {
        Point2D x2;
        Project(t12, Rx1, x2);
        return x2;
    ...
    inline bool ProjectD(
        const Rigid3D::Row& T12z, const Point2D& x1, InverseGaussian& d2) const
    {
        const xp128f t1 =
            T12z.r0_r1_r2_t() * xp128f::get(x1.x(), x1.y(), 1.0f, u());
    ...
    static inline void ProjectD(const Rigid3D& T21, const Point2D& x2,
        const float d2, float& d1, LA::Vector2f& Jx2, float& Jd2,
        LA::AlignedVector6f& Jpr21)
    {
    ...
    inline float GetProjectedD(
        const Rigid3D::Row& T12z, const Point2D& x1) const
    {
        const xp128f t = xp128f::get(x1.x(), x1.y(), 1.0f, u());
...
public:
    ...
    inline void Initialize()
    {
        InverseGaussian::Initialize();
        a() = b() = 10.0f;
    }
    inline void Initialize(const float u)
    {
        InverseGaussian::Initialize(u);
        a() = b() = 10.0f;
    }
    inline void Initialize(const float u, const float s2)
    {
        InverseGaussian::Set(u, s2);
        a() = b() = 10.0f;
    }
    inline void Initialize(const InverseGaussian& d)
    {
        InverseGaussian::Set(d);
        a() = b() = 10.0f;
    }
    inline bool Update(const InverseGaussian& d)
    {
        ...
        const float SabI = 1.0f / (a() + b()), Ss2I = 1.0f / Ss2,
                    dz = d.u() - u(), dz2 = dz * dz;
        const float C1 =
            a() * SabI * exp(-dz * dz * 0.5f * Ss2I) * sqrtf(UT_1_2PI * Ss2I);
...
public:
    ...
    inline void GetReduction(const Factor& A, const float d, const float xd,
...
public:
    inline Measurement() {}
    inline Measurement(const LA::AlignedVector3f& t, const LA::Vector3f& Rx,
        const Point2D& z, const LA::SymmetricMatrix2x2f& W)
    {
        Set(t, Rx, z, W);
    }
    inline void Set(const LA::AlignedVector3f& t, const LA::Vector3f& Rx,
        const Point2D& z, const LA::SymmetricMatrix2x2f& W)
    {
        m_t = &t;
        m_Rx = Rx;
