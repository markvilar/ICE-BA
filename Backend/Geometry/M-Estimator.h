...
template <int PARAM_DOF>
inline float ChiSquareDistance(const float p, const float w = 1.0f);
template <> inline float ChiSquareDistance<3>(const float p, const float w)
{
#ifdef CFG_DEBUG
    UT_ASSERT(p == 0.9f || p == 0.99f || p == 0.999f || p == 0.9999f ||
              p == 0.99999f || p == 1.0f);
#endif
    float X2;
    if (p == 0.9f)
...
template <>
inline void Weight<FUNCTION_NONE>(
    const AlignedVector<float>& r2s, AlignedVector<float>& ws)
{
...
template <>
inline void Weight<FUNCTION_HUBER>(
    const AlignedVector<float>& r2s, AlignedVector<float>& ws)
{
    const int N = r2s.Size();
    ws.Resize(N);
...
template <> inline float Variance<FUNCTION_HUBER>()
{
    return ME_VARIANCE_HUBER;
}
...
template <> inline float Variance<FUNCTION_TUKEY>()
{
    return ME_VARIANCE_TUKEY;
}
...
template <>
inline void Weight<FUNCTION_STUDENT_T_5>(
    const AlignedVector<float>& r2s, AlignedVector<float>& ws)
{
    WeightsStudentT<5>(r2s, ws);
}
template <>
inline void Weight<FUNCTION_CAUCHY>(
    const AlignedVector<float>& r2s, AlignedVector<float>& ws)
{
