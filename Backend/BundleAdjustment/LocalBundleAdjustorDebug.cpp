...
void LocalBundleAdjustor::DebugMarginalizeLocalFrame()
{
    ...
    const std::string str = UT::String("LBA [%d] <-- [%d]",
        m_LFs[m_ic2LF.back()].m_T.m_iFrm + 1,
        LF1.m_T.m_iFrm);
    if (LF1.m_T.m_iFrm == m_KFs[iKFr].m_T.m_iFrm)
    {
        ...
        else
        {
            ...
            ZpLF.Initialize(BA_WEIGHT_PRIOR_CAMERA_INITIAL,
                BA_VARIANCE_PRIOR_VELOCITY_FIRST,
                BA_VARIANCE_PRIOR_BIAS_ACCELERATION_FIRST,
                BA_VARIANCE_PRIOR_BIAS_GYROSCOPE_FIRST);
            m_ZpBkp.Initialize(BA_WEIGHT_PRIOR_CAMERA_INITIAL,
                iKFr,
                m_CsKF[iKFr],
                BA_VARIANCE_PRIOR_GRAVITY_FIRST,
                ZpLF,
                true);
            e_ApLF.Initialize(BA_WEIGHT_PRIOR_CAMERA_INITIAL,
                BA_VARIANCE_PRIOR_VELOCITY_FIRST,
                BA_VARIANCE_PRIOR_BIAS_ACCELERATION_FIRST,
                BA_VARIANCE_PRIOR_BIAS_GYROSCOPE_FIRST);
        }
        ...
        D.GetFactor(
            BA_WEIGHT_IMU, C1, C2, m_K.m_pu, &e, &J, &A, BA_ANGLE_EPSILON);
        D.EigenGetFactor(
            BA_WEIGHT_IMU, C1, C2, m_K.m_pu, &e_A, BA_ANGLE_EPSILON);
        e_A.AssertEqual(A);
        e_A.Set(A, e_A.m_F);
    ...
    else
    {
        ...
        for (int iZ = 0; iZ < NZ; ++iZ)
        {
            ...
            if (iKF == m_ZpBkp.m_iKFr)
            {
                ...
                for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz)
                {
                    ...
                    const int ix = z.m_ix;
                    const FTR::EigenFactor e_A =
                        FTR::EigenGetFactor<LBA_ME_FUNCTION>(BA_WEIGHT_FEATURE,
                            Trr,
                            KF.m_xs[ix],
                            ds[ix],
                            Tr1,
                    ...
                    const EigenVector6f mdczT =
                        EigenVector6f((e_A.m_adcz / e_A.m_add).transpose());
            ...
            else
            {
                const std::vector<int>::const_iterator it = std::lower_bound(
                    m_ZpBkp.m_iKFs.begin(), m_ZpBkp.m_iKFs.end(), iKF);
                ...
                if (it == m_ZpBkp.m_iKFs.end() || *it != iKF)
                {
                    ...
                    m_ZpBkp.Insert(BA_WEIGHT_PRIOR_CAMERA_INITIAL,
                        ik,
                        iKF,
                        Trk,
                        ...
                        &m_work);
                    e_Ap.Insert(BA_WEIGHT_PRIOR_CAMERA_INITIAL,
                        ik,
                        BA_VARIANCE_PRIOR_POSITION_NEW,
                        BA_VARIANCE_PRIOR_ROTATION_NEW);
                    ++Nk;
                ...
                for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz)
                {
                    ...
                    const EigenVector6f mdcxT =
                        EigenVector6f((e_A.m_adcx / e_A.m_add).transpose());
                    const EigenVector6f mdczT =
                        EigenVector6f((e_A.m_adcz / e_A.m_add).transpose());
        ...
        D.EigenGetFactor(
            BA_WEIGHT_IMU, C1, C2, m_K.m_pu, Tr, &e_A, BA_ANGLE_EPSILON);
        e_A.AssertEqual(A);
        e_A.Set(A, e_A.m_F);
...
void LocalBundleAdjustor::DebugGenerateTracks()
{
    ...
    for (int ic = 0; ic < Nc; ++ic)
    {
        ...
        for (int iZ = 0; iZ < NZ; ++iZ)
        {
            ...
            for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz)
            {
                Xs[LF.m_zs[iz].m_ix].m_zsLF.push_back(
                    Track::MeasurementLF(ic, iz));
            }
        }
    }
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        for (int iZ = 0; iZ < NZ; ++iZ)
        {
            ...
            for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz)
            {
                Xs[KF.m_zs[iz].m_ix].m_zsKF.push_back(
                    Track::MeasurementKF(iKF, iz));
            }
        }
    }
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            else
            {
                ...
                {
                    const int ic = X.m_zsLF[i].m_ic,
                              ic2 = std::min(ic + STL, Nc);
                    m_idxsTmp1.resize(0);
                    ...
                    if (X.m_STsLF.empty() ||
                        X.m_STsLF.back().back() < m_idxsTmp1.back())
                    {
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            for (int ist = 0, iST = KF.m_ix2ST[ix]; ist < Nst; ++ist, ++iST)
            {
                ...
                UT_ASSERT(_ST.m_icMin == X.m_zsLF[ST.front()].m_ic &&
                          _ST.m_icMax == X.m_zsLF[ST.back()].m_ic);
                const int NzST = static_cast<int>(ST.size());
                for (int i = 0; i < NzST; ++i)
                {
                    const Track::MeasurementLF& z = X.m_zsLF[ST[i]];
                    const LocalFrame::SlidingTrack& STz =
                        m_LFs[m_ic2LF[z.m_ic]].m_STs[z.m_iz];
        ...
        for (int iZ = 0; iZ < NZ; ++iZ)
        {
            ...
            for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz)
            {
                ...
                const int i = static_cast<int>(
                    std::lower_bound(X.m_zsLF.begin(), X.m_zsLF.end(), ic) -
                    X.m_zsLF.begin());
                ...
                {
                    const std::vector<int>& ST = X.m_STsLF[ist];
                    const std::vector<int>::const_iterator j =
                        std::lower_bound(ST.begin(), ST.end(), i);
...
void LocalBundleAdjustor::DebugUpdateFactors()
{
  ...
  if (m_debug >= 4) {
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            X.m_Sadd.AssertEqual(
                KF.m_Axs[ix].m_Sadd, 1, _str + " Sadd", epsAbs, epsRel);
            ...
            for (int ist = 0, iST = KF.m_ix2ST[ix]; ist < Nst; ++ist, ++iST)
            {
                X.m_SaddsST[ist].AssertEqual(KF.m_AxsST[iST].m_Sadd,
                    1,
  ...
  if (m_debug >= 4) {
    ...
    for (int ic = 0; ic < Nc; ++ic)
    {
        ...
        const auto e_SAcu = Camera::Factor::Unitary::CC::Get(
            e_SAccs[icu].GetSymmetricMatrix6x6f(), e_Sbcs[ic].GetVector6f());
        const Camera::Factor::Unitary::CC& SAcu = m_SAcusLF[m_ic2LF[ic]];
        e_SAcu.AssertEqual(
            SAcu, 1, str + UT::String(" SAcc[%d][%d]", ic, ic), epsAbs, epsRel);
        ...
        {
            const int _iLF = LF.m_iLFsMatch[ik],
                      _ic = (_iLF + Nc - m_ic2LF[0]) % Nc;
            UT_ASSERT(m_ic2LF[_ic] == _iLF);
            const int icb = e_I[ic][_ic];
            if (icb != -1)
            {
                const auto e_Acb = Camera::Factor::Binary::CC::Get(
    ...
    for (int ic = 0; ic < Nc; ++ic)
    {
        ...
        e_SAcms[ic] = SAcm;
        e_Sbms[ic].AssertEqual(
            SAcm.m_Au.m_Amm.m_b, 1, str + UT::String(" Sbm[%d]", ic));
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            const std::string _str =
                str + UT::String(" iKF = %d ix = %d", iKF, ix);
            const int NzLF = static_cast<int>(X.m_zsLF.size()),
                      NzKF = static_cast<int>(X.m_zsKF.size());
        ...
        if (m_iIter == 2 && z.m_ic == 21 && z.m_iz == 7) {
          UT::DebugStart();
...
#endif
                A = FTR::EigenGetFactor<LBA_ME_FUNCTION>(BA_WEIGHT_FEATURE,
                    C,
                    x,
                    d,
                    m_CsLF[iLF].m_T,
            ...
            for (int i = 0, j = NzLF; i < NzKF; ++i, ++j)
            {
                ...
                A = FTR::EigenGetFactor<LBA_ME_FUNCTION>(
                ...
                A.AssertEqual(_KF.m_Azs[z.m_iz],
                    1,
            ...
            for (int ist = 0; ist < Nst; ++ist)
            {
                ...
                for (int i = 0; i < NzST; ++i)
                {
                    ...
                    const FTR::EigenFactor::DD e_addST =
                        FTR::EigenFactor::DD::Get(AST.m_add, AST.m_bd);
//#ifdef CFG_DEBUG
#if 0
          if (iKF == 0 && ix == 0) {
            if (i == 0) {
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            if (xr)
            {
                const FTR::EigenFactor::Stereo e_Ard =
                    FTR::EigenGetFactor<LBA_ME_FUNCTION>(
                        BA_WEIGHT_FEATURE_KEY_FRAME, m_K.m_br, ds[ix], x);
...
void LocalBundleAdjustor::DebugUpdateFactorsPriorCameraMotion()
{
    ...
    A.AssertEqual(m_ApLF,
        1,
        UT::String(
            "LBA [%d] iIter %d Am", m_LFs[m_ic2LF.back()].m_T.m_iFrm, m_iIter));
    A.Set(m_ApLF);
...
void LocalBundleAdjustor::DebugUpdateFactorsIMU()
{
    ...
    for (int ic1 = 0, ic2 = 1, icp1 = 0, imp1 = pc, icp2 = pcm, imp2 = pcm + pc;
         ic2 < nLFs;
...
#endif
        m_DsLF[iLF2].EigenGetFactor(BA_WEIGHT_IMU,
            m_CsLF[iLF1],
            m_CsLF[iLF2],
            m_K.m_pu,
            &A,
...
#endif
        A.AssertEqual(m_AdsLF[iLF2],
            m_SAcmsLF[iLF2].m_Ab,
            1,
            str + UT::String(" Ad[%d][%d]", ic1, ic2));
        A.Set(m_AdsLF[iLF2], m_SAcmsLF[iLF2].m_Ab);
        Camera::EigenFactor::Unitary &SA11 = e_SAcms[ic1].m_Au,
                                     &SA22 = e_SAcms[ic2].m_Au;
...
void LocalBundleAdjustor::DebugUpdateFactorsFixOrigin()
{
    ...
    Camera::Fix::Origin::EigenFactor e_A =
        m_Zo.EigenGetFactor(m_CsLF[iLF].m_T, BA_ANGLE_EPSILON);
    const int iFrm = m_LFs[m_ic2LF.back()].m_T.m_iFrm;
    const std::string str = UT::String("LBA [%d] iIter %d", iFrm, m_iIter);
...
{
    const float w =
        UT::Inverse(BA_VARIANCE_FIX_POSITION_Z, BA_WEIGHT_FIX_POSITION_Z);
    const int Nc = static_cast<int>(m_LFs.size());
    for (int ic = 0; ic < Nc; ++ic)
    {
...
void LocalBundleAdjustor::DebugUpdateFactorsFixMotion()
{
    ...
    for (int ic = 0, imp = pc; ic < nLFs; ++ic, imp += pcm)
    {
        ...
        // e_F = _wv * C.m_v.SquaredLength() + _wba * C.m_ba.SquaredLength() +
        // _wbw * C.m_bw.SquaredLength() + e_F;
#if 0
    if (m_debug >= 4) {
...
void LocalBundleAdjustor::DebugUpdateSchurComplement()
{
    ...
    // const float add = UT::Inverse(BA_VARIANCE_REGULARIZATION_DEPTH,
    // BA_WEIGHT_FEATURE);
    const int nKFs = static_cast<int>(m_KFs.size());
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
...
//#ifdef CFG_DEBUG
  ...
  if (m_debug >= 4) {
    ...
    {
        const int icu = e_I[ic][ic];
        const auto e_SMcu = Camera::Factor::Unitary::CC::Get(
            e_SMccs[icu].GetSymmetricMatrix6x6f(), e_Smcs[ic].GetVector6f());
        const Camera::Factor::Unitary::CC& SMcu = m_SMcusLF[m_ic2LF[ic]];
        e_SMcu.AssertEqual(
            SMcu, 1, str + UT::String(" SMcc[%d][%d]", ic, ic), epsAbs, epsRel);
        ...
        for (int ik = 0; ik < Nk; ++ik)
        {
            ...
            const auto e_SMcb = Camera::Factor::Binary::CC::Get(
                e_SMccs[icb].GetAlignedMatrix6x6f());
...
void LocalBundleAdjustor::DebugSolveSchurComplement()
{
    ...
    EigenVector6f e_Sbc;
    EigenVector9f e_Sbm;
    const float e2Max = ME::ChiSquareDistance<3>(
...
//#ifdef CFG_DEBUG
      ...
      if (ic1 == 1) {
        ...
        if (ic0 >= 0)
        {
            ...
            // e_Sbc = EigenVector6f(SAcmb.m_Acc.transpose() * e_xcs[ic0] +
            // e_Sbc);
...
#endif
        const EigenVector6f e_rc =
            EigenVector6f((e_Sbcs[ic1] - e_Smcs[ic1]) - e_Sbc);
        ...
        e_Sbm = -e_Sbm;
        const EigenMatrix9x9f e_Mm =
            m_Mms[ic1].GetAlignedMatrix9x9f().GetMatrix9x9f();
        ...
        if (e2Max == 0.0f)
        {
            UT_ASSERT(UT::AssertZero(e_e2v) && UT::AssertZero(e_e2ba) &&
                      UT::AssertZero(e_e2bw));
        }
        else
        {
...
#ifdef CFG_DEBUG
static inline void PrintBackSubstitution(const int iFrm,
    const LA::Vector6f& mdc, const LA::Vector6f& x1, const float xd1,
    const float Sxd1, const LA::Vector6f& x2, const float xd2, const float Sxd2)
{
    UT::PrintSeparator();
    UT::Print("[%d]\n", iFrm);
    mdc.Print("mdc = ", false, true);
    x1.Print("xc1 = ", false, false);
    UT::Print(" --> %f\n", mdc.Dot(x1));
    x2.Print("xc2 = ", false, false);
    UT::Print(" --> %f\n", mdc.Dot(x2));
    const LA::Vector6f dx = x1 - x2;
    dx.Print("dxc = ", false, false);
    UT::Print(" --> %f\n", mdc.Dot(dx));
    const LA::Vector3f dp = dx.Get012(), dr = dx.Get345();
    const float dp2 = dp.SquaredLength(), dr2 = dr.SquaredLength();
    UT_ASSERT(dp2 <= BA_BACK_SUBSTITUTE_POSITION &&
              dr2 <= BA_BACK_SUBSTITUTE_ROTATION);
    UT::Print("  --> %f %f --> %f %f\n",
        sqrtf(dp2),
        sqrtf(dr2) * UT_FACTOR_RAD_TO_DEG,
        mdc.Get012().Dot(dp),
        mdc.Get345().Dot(dr));
    UT::Print(" xd = %f - %f = %f\n", xd1, xd2, xd1 - xd2);
    UT::Print("Sxd = %f - %f = %f\n", Sxd1, Sxd2, Sxd1 - Sxd2);
...
void LocalBundleAdjustor::DebugSolveBackSubstitution()
{
  ...
  for (int iKF = 0; iKF < nKFs; ++iKF) {
    ...
    for (int iKF = 0, iX = 0; iKF < nKFs; ++iKF)
    {
        ...
        float* xds = NULL;
        const ubyte* uds = m_uds.data() + m_iKF2d[iKF];
        const bool exist = UT::VectorExistFlag<ubyte>(
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            const Track& X = Xs[ix];
            const float Sadd = X.m_Sadd.m_a /* + add*/,
                        mdd = Sadd > epsd ? 1.0f / Sadd : 0.0f;
            ...
            UT::AssertEqual(Sxd,
                xds ? xds[ix] : 0.0f,
                1,
                str + UT::String(" iKF %d ix %d xd", iKF, ix),
                epsAbs,
                epsRel);
#ifdef CFG_DEBUG
            if (UT::Debugging())
...
void LocalBundleAdjustor::DebugSolveGradientDescent()
{
    ...
    for (int ic = 0; ic < Nc; ++ic)
    {
        ...
        e_Agcs[ic] =
            EigenVector6f(e_A.m_Acm * e_gm + e_SAccs[e_I[ic][ic]] * e_gc);
        e_Agms[ic] =
            EigenVector9f(e_A.m_Amm * e_gm + e_A.m_Acm.transpose() * e_gc);
    }
    for (int ic1 = 0, ic2 = 1; ic2 < Nc; ic1 = ic2++)
    {
        ...
        e_Agm1 = EigenVector9f(
            e_A.m_Amm.block<9, 3>(0, 0) * e_gm2.block<3, 1>(0, 0) + e_Agm1);
//#ifdef CFG_DEBUG
#if 0
    if (m_iIter == 1 && ic1 == 10) {
      e_Agm1.Print();
...
//#ifdef CFG_DEBUG
    ...
    if (m_iIter == 1 && ic1 == 10) {
      e_Agm1.Print();
        ...
        e_Agm1 = EigenVector9f(
            e_A.m_Amm.block<9, 6>(0, 3) * e_gm2.block<6, 1>(3, 0) + e_Agm1);
...
//#ifdef CFG_DEBUG
    ...
    UT::Print(
        "\r[%d] Assert Agd Start\t\t\t", m_LFs[m_ic2LF.back()].m_T.m_iFrm);
#endif
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        const int iX = m_iKF2X[iKF];
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            UT::AssertEqual(gds1[ix],
                gds2[ix],
                1,
                str + UT::String(" iKF %d ix %d gd", iKF, ix));
            gds1[ix] = gds2[ix];
            ...
            {
                const Track::MeasurementLF& z = X.m_zsLF[i];
                const EigenVector6f e_adcz =
                    EigenVector6f(z.m_adcz.transpose());
                e_Agd = e_adcz.dot(e_gcs[z.m_ic]) + e_Agd;
                e_Agcs[z.m_ic] = EigenVector6f(e_adcz * e_gd + e_Agcs[z.m_ic]);
            }
            UT::AssertEqual(e_Agd,
                Agds2[ix],
                1,
                str + UT::String(" iKF %d ix %d Agd", iKF, ix));
            e_Agd = Agds2[ix];
        }
    }
#ifdef LBA_DEBUG_PRINT_STEP
...
#ifdef LBA_DEBUG_PRINT_STEP
    UT::Print(
        "\r[%d] Assert Agc Start\t\t\t", m_LFs[m_ic2LF.back()].m_T.m_iFrm);
#endif
    const LA::Vector6f* Agcs = (LA::Vector6f*)m_Ags.Data();
    const LA::Vector9f* Agms = (LA::Vector9f*)(Agcs + Nc);
    for (int ic = 0; ic < Nc; ++ic)
    {
        e_Agcs[ic].GetVector6f().AssertEqual(
            Agcs[ic], 1, str + UT::String(" Agc[%d]", ic));
        e_Agcs[ic] = Agcs[ic];
    }
#ifdef LBA_DEBUG_PRINT_STEP
...
#ifdef LBA_DEBUG_PRINT_STEP
    UT::Print(
        "\r[%d] Assert Agm Start\t\t\t", m_LFs[m_ic2LF.back()].m_T.m_iFrm);
#endif
    for (int ic = 0; ic < Nc; ++ic)
    {
        e_Agms[ic].GetVector9f().AssertEqual(
            Agms[ic], 1, str + UT::String(" Agm[%d]", ic));
        e_Agms[ic] = Agms[ic];
    }
#ifdef LBA_DEBUG_PRINT_STEP
...
#ifdef LBA_DEBUG_PRINT_STEP
    UT::Print(
        "\r[%d] Assert gTg Start\t\t\t", m_LFs[m_ic2LF.back()].m_T.m_iFrm);
#endif
...
#ifdef LBA_DEBUG_PRINT_STEP
    UT::Print(
        "\r[%d] Assert m_gTAg Start\t\t\t", m_LFs[m_ic2LF.back()].m_T.m_iFrm);
    // UT::Print("\r[%d] %e %e %e %e %e\t\t\t",
    // m_LFs[m_ic2LF.back()].m_T.m_iFrm,
    //  gTAgc, gTAgm, gTAgk, gTAgd, gTAg);
    UT::Print(
        "\r[%d] %e %e\t\t\t", m_LFs[m_ic2LF.back()].m_T.m_iFrm, gTgd, gTAgd);
#endif
    UT::AssertEqual(gTAg, m_gTAg, 1, str + " gTAg");
#ifdef LBA_DEBUG_PRINT_STEP
    UT::Print(
        "\r[%d] Assert m_gTAg Stop\t\t\t", m_LFs[m_ic2LF.back()].m_T.m_iFrm);
#endif
}
...
void LocalBundleAdjustor::DebugComputeReduction()
{
    ...
    UT::AssertEqual(
        e_dFp, m_dFp, 1, str + UT::String(" iIterDL %d dFp", m_iIterDL), eps);
}
...
void LocalBundleAdjustor::DebugComputeReductionFeature()
{
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        const Depth::InverseGaussian* ds =
            iX == -1 ? m_ds.data() + id : m_dsBkp.data() + iX;
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            for (int i = 0; i < NzLF; ++i)
            {
                ...
                const LocalFrame& LF = m_LFs[iLF];
                const float F =
                    FTR::EigenGetCost<LBA_ME_FUNCTION>(BA_WEIGHT_FEATURE,
                        C,
                        x,
                        d,
                        m_CsLFBkp[iLF].m_T,
        ...
        if (m_iIter == 2 && z.m_ic == 12 && dFpsLF[z.m_ic] != 0.0f) {
          UT::Print("iKF = %d, ix = %d, iz = %d, dFp = %f\n", iKF, ix, z.m_iz, dFpsLF[z.m_ic]);
            ...
            for (int i = 0, j = NzLF; i < NzKF; ++i, ++j)
            {
                ...
                const float F = FTR::EigenGetCost<LBA_ME_FUNCTION>(
                    BA_WEIGHT_FEATURE_KEY_FRAME,
                    C,
                    x,
                    d,
...
void LocalBundleAdjustor::DebugComputeReductionPriorDepth()
{
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            {
                const float F = FTR::EigenGetCost<LBA_ME_FUNCTION>(
                    BA_WEIGHT_FEATURE_KEY_FRAME, m_K.m_br, ds[ix], x, xds[ix]);
...
void LocalBundleAdjustor::DebugComputeReductionPriorCameraMotion()
{
    ...
    const float F = m_ZpLF.EigenGetCost(BA_WEIGHT_PRIOR_CAMERA_MOTION,
        m_CsLFBkp[m_ic2LF.front()],
        e_xr,
        e_xms[0]);
...
void LocalBundleAdjustor::DebugComputeReductionIMU()
{
    ...
    {
        const int iLF1 = m_ic2LF[ic1], iLF2 = m_ic2LF[ic2];
        const float F = m_DsLF[iLF2].EigenGetCost(BA_WEIGHT_IMU,
            m_CsLFBkp[iLF1],
            m_CsLFBkp[iLF2],
            m_K.m_pu,
            e_xcs[ic1],
...
void LocalBundleAdjustor::DebugComputeReductionFixOrigin()
{
    ...
    const float F =
        m_Zo.EigenGetCost(m_CsLFBkp[iLF].m_T, e_xcs[0], BA_ANGLE_EPSILON);
...
void LocalBundleAdjustor::DebugComputeReductionFixMotion()
{
    ...
    for (int ic = 0; ic < Nc; ++ic)
    {
        ...
        const EigenVector3f ev =
            EigenVector3f(EigenVector3f(C.m_v) + e_xm.block<3, 1>(0, 0));
        const EigenVector3f eba =
            EigenVector3f(EigenVector3f(C.m_ba) + e_xm.block<3, 1>(3, 0));
        const EigenVector3f ebw =
            EigenVector3f(EigenVector3f(C.m_bw) + e_xm.block<3, 1>(6, 0));
