...
void GlobalBundleAdjustor::DebugUpdateFactors()
{
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        {
            Track& X = Xs[ix];
            X.m_Sadx.AssertEqual(KF.m_Axs[ix].m_Sadx,
                1,
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        const auto e_SAcu = Camera::Factor::Unitary::CC::Get(
            e_SAccs[icu].GetSymmetricMatrix6x6f(), e_Sbcs[iKF].GetVector6f());
        const Camera::Factor::Unitary::CC& SAcu = m_SAcus[iKF];
        e_SAcu.AssertEqual(SAcu,
            1,
        ...
        AlignedVector<Camera::Factor::Binary::CC> SAcbs(
            (Camera::Factor::Binary::CC*)m_work.Data(), Nkp, false);
        SAcbs.MakeZero();
    ...
    for (int im = 0; im < Nm; ++im)
    {
        ...
        e_SAcmsLM[im] = SAcm;
        e_Sbms[im].AssertEqual(
            SAcm.m_Au.m_Amm.m_b, 1, str + UT::String(" Sbm[%d]", im));
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            for (int i = 0; i < Nz; ++i)
            {
                ...
                FTR::EigenFactor A =
                    FTR::EigenGetFactor<GBA_ME_FUNCTION>(BA_WEIGHT_FEATURE,
                        C,
                        x,
                        d,
                        m_Cs[z.m_iKF],
                ...
                // A.AssertEqual(_KF.m_Lzs[iz], _KF.m_Azs1[iz], _KF.m_Azs2[iz],
                // 1,
                //              _str + UT::String(" --> iKF %d iz %d Az",
                //              z.m_iKF, z.m_iz));
...
void GlobalBundleAdjustor::DebugUpdateFactorsPriorCameraPose()
{
    ...
    for (int iZ = 0; iZ < NZ; ++iZ)
    {
        ...
        CameraPrior::Pose::EigenFactor A =
            Z.EigenGetFactor(m_Aps[iZ].m_w, m_Cs, BA_ANGLE_EPSILON);
...
void GlobalBundleAdjustor::DebugUpdateFactorsPriorCameraMotion()
{
    ...
    A.AssertEqual(m_ApLM,
        1,
        UT::String("GBA [%d] iIter %d Am", m_KFs.back().m_T.m_iFrm, m_iIter));
    A.Set(m_ApLM);
...
void GlobalBundleAdjustor::DebugUpdateFactorsPriorDepth()
{
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            if (xr)
            {
                FTR::EigenFactor::Stereo e_Ard =
                    FTR::EigenGetFactor<GBA_ME_FUNCTION>(
                        BA_WEIGHT_FEATURE, m_K.m_br, ds[ix], x);
...
#endif
        m_DsLM[im2].EigenGetFactor(BA_WEIGHT_IMU,
            m_CsLM[im1],
            m_CsLM[im2],
            m_K.m_pu,
            &A,
            BA_ANGLE_EPSILON);
        A.AssertEqual(m_AdsLM[im2],
            m_SAcmsLM[im2].m_Ab,
            1,
            str + UT::String(" Ad[%d][%d]", im1, im2));
        A.Set(m_AdsLM[im2], m_SAcmsLM[im2].m_Ab);
        Camera::EigenFactor::Unitary &SA11 = e_SAcmsLM[im1].m_Au,
                                     &SA22 = e_SAcmsLM[im2].m_Au;
...
void GlobalBundleAdjustor::DebugUpdateFactorsFixOrigin()
{
    ...
    Camera::Fix::Origin::EigenFactor e_A =
        m_Zo.EigenGetFactor(m_Cs[iKF], BA_ANGLE_EPSILON);
    const int iFrm = m_KFs.back().m_T.m_iFrm;
    const std::string str = UT::String("GBA [%d] iIter %d", iFrm, m_iIter);
...
void GlobalBundleAdjustor::DebugUpdateFactorsFixMotion()
{
    ...
    for (int ic1 = Nc - m_CsLM.Size(), ic2 = ic1 + 1, im = 0; ic1 < Nc;
         ic1 = ic2++, ++im)
    {
...
void GlobalBundleAdjustor::DebugUpdateSchurComplement()
{
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            for (int i1 = 0; i1 < Nz; ++i1)
            {
                ...
                const EigenVector6f mdczT =
                    EigenVector6f(z1.m_adcz.transpose() * mdd);
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        const auto e_SMcu = Camera::Factor::Unitary::CC::Get(
            e_SMccs[icu].GetSymmetricMatrix6x6f(), e_Smcs[iKF].GetVector6f());
        const Camera::Factor::Unitary::CC& SMcu = m_SMcus[iKF];
        e_SMcu.AssertEqual(SMcu,
            1,
...
void GlobalBundleAdjustor::DebugSolveSchurComplement()
{
    ...
    for (int ic1 = 0, im1 = Nm - Nc; ic1 < Nc; ++ic1, ++im1)
    {
        ...
        if (im1 >= 0)
        {
            ...
            if (im2 < Nm)
            {
                const Camera::EigenFactor::Binary& e_SAcmb =
                    e_SAcmsLM[im2].m_Ab;
        ...
        const EigenMatrix9x9f e_Mm =
            m_MmsLM[im1].GetAlignedMatrix9x9f().GetMatrix9x9f();
...
void GlobalBundleAdjustor::DebugSolveBackSubstitution()
{
    ...
    const float *Sxds = m_Sxds.Data() + m_iKF2d[iKF];
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            const float Sadd = X.m_Sadx.m_add.m_a /* + add*/;
            const float mdd = Sadd > epsd ? 1.0f / Sadd : 0.0f;
            float xd = (X.m_Sadx.m_add.m_b + X.m_Sadx.m_adc.dot(xc)) * mdd,
                  Sxd = -xd;
#if 0
...
void GlobalBundleAdjustor::DebugSolveGradientDescent()
{
    ...
    for (int ic = Nc - Nm, im = 0; ic < Nc; ++ic, ++im)
    {
        ...
        e_Agms[im] =
            EigenVector9f(e_A.m_Amm * e_gm + e_A.m_Acm.transpose() * e_gcs[ic]);
    }
...
//#ifdef CFG_DEBUG
        ...
        if (m_iIter == 1 && ic == 0)
          UT::Print("%d: %e * %e + %e = %e\n", ix, e_gd, e_Sadx.m_adc(0, 0), e_Agc(0, 0), e_Sadx.m_adc(0, 0) * e_gd + e_Agc(0, 0));
                ...
                UT::AssertEqual(e_gd,
                    gds[ix],
                    1,
                    str + UT::String(" ic %d ix %d gd", ic, ix));
                UT::AssertEqual(e_Agd,
                    Agds[ix],
                    1,
                    str + UT::String(" ic %d ix %d Agd", ic, ix));
                e_gd = gds[ix];
                e_Agd = Agds[ix];
            }
            else
    ...
    for (int im = 0; im < Nm; ++im)
    {
        e_Agms[im].GetVector9f().AssertEqual(
            Agms[im], 1, str + UT::String(" Agm[%d]", im));
        e_Agms[im] = Agms[im];
    }
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            for (int i = 0; i < Nz; ++i)
            {
                ...
                const float F =
                    FTR::EigenGetCost<GBA_ME_FUNCTION>(BA_WEIGHT_FEATURE,
                        C,
                        x,
                        d,
                        m_CsBkp[_iKF],
...
void GlobalBundleAdjustor::DebugComputeReductionPriorCameraPose()
{
    ...
    for (int iZ = 0; iZ < NZ; ++iZ)
    {
        const float F = m_Zps[iZ].EigenGetCost(
            m_Aps[iZ].m_w, m_CsBkp, e_xcs, BA_ANGLE_EPSILON);
...
void GlobalBundleAdjustor::DebugComputeReductionPriorCameraMotion()
{
    ...
    const float F = m_ZpLM.EigenGetCost(
        BA_WEIGHT_PRIOR_CAMERA_MOTION, m_CsLMBkp[im], e_xr, e_xms[im]);
...
void GlobalBundleAdjustor::DebugComputeReductionPriorDepth()
{
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            if (x.m_xr.Valid())
            {
                const float F = FTR::EigenGetCost<GBA_ME_FUNCTION>(
                    BA_WEIGHT_FEATURE, m_K.m_br, ds[ix], x, xds[ix]);
...
void GlobalBundleAdjustor::DebugComputeReductionIMU()
{
         ...
         ic1 = ic2++, im1 = im2++)
    {
        ...
        const float F = m_DsLM[im2].EigenGetCost(BA_WEIGHT_IMU,
            m_CsLMBkp[im1],
            m_CsLMBkp[im2],
            m_K.m_pu,
            e_xcs[ic1],
...
void GlobalBundleAdjustor::DebugComputeReductionFixOrigin()
{
    ...
    const float F =
        m_Zo.EigenGetCost(m_CsBkp[iKF], e_xcs[iKF], BA_ANGLE_EPSILON);
...
void GlobalBundleAdjustor::DebugComputeReductionFixMotion()
{
    ...
    for (int ic = Nc - m_CsLM.Size(), im = 0; ic < Nc; ++ic, ++im)
    {
        ...
        else if (KF.m_us.Empty() &&
                 (ic + 1 == Nc || m_KFs[ic + 1].m_us.Empty()))
        {
            _wv = wv1;
            _wba = wba1;
            _wbw = wbw1;
