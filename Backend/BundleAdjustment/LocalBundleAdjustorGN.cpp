...
void LocalBundleAdjustor::UpdateFactors()
{
    ...
    const ubyte ucFlag =
        LBA_FLAG_FRAME_PUSH_TRACK | LBA_FLAG_FRAME_UPDATE_DEPTH;
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            else
            {
                const int iST1 = KF.m_ix2ST[ix], iST2 = KF.m_ix2ST[ix + 1];
                for (int iST = iST2 - 1;
                     iST >= iST1 && (KF.m_usST[iST] & LBA_FLAG_TRACK_PUSH);
                     --iST)
                {
                    KF.m_AxsST[iST].m_Sadd.m_a = add;
                }
...
void LocalBundleAdjustor::UpdateFactorsFeaturePriorDepth()
{
    ...
    if (m_verbose >= 3)
    {
        const int NzLF = CountMeasurementsFeatureLF(),
                  NZLF = CountMeasurementsFrameLF();
        const int NzKF = CountMeasurementsFeatureKF(),
                  NZKF = CountMeasurementsFrameKF();
        ...
        if (g_SNZLFST > 0 || g_SNzLFST > 0)
        {
            UT::Print("            + %4d / %4d = %.2f%% (%d / %2d = %.2f%%)\n",
                g_SNzLFST,
                NzLFST,
                UT::Percentage(g_SNzLFST, NzLFST),
                g_SNZLFST,
                NZLFST,
                UT::Percentage(g_SNZLFST, NZLFST));
        }
        ...
        UT::Print("  Prior Depth  = %d / %d = %.2f%% + %d / %d = %.2f%%\n",
            g_SNd,
            Nd,
            UT::Percentage(g_SNd, Nd),
            g_SNdST,
            NdST,
            UT::Percentage(g_SNdST, NdST));
    }
#endif
}
...
void LocalBundleAdjustor::UpdateFactorsFeatureLF()
{
    ...
    const ubyte udFlag =
        LBA_FLAG_TRACK_PUSH | LBA_FLAG_TRACK_POP | LBA_FLAG_TRACK_UPDATE_DEPTH;
    const int nLFs = static_cast<int>(m_LFs.size());
    for (int iLF = 0; iLF < nLFs; ++iLF)
    {
        ...
        for (int iZ = 0; iZ < NZ; ++iZ)
        {
            ...
            const bool ucx = (m_ucsKF[iKF] & LBA_FLAG_FRAME_UPDATE_CAMERA) != 0,
                       ucr = ucx || ucz;
            ...
            const bool pushFrm =
                (m_ucsKF[iKF] & LBA_FLAG_FRAME_PUSH_TRACK) != 0;
            *Tr = C / m_CsKF[iKF];
#ifdef CFG_STEREO
            Tr[1] = Tr[0];
...
//#ifdef CFG_DEBUG
        ...
        if (iLF == m_ic2LF[12]) {
          UT::Print("%d: %.10e\n", iz, SAczz.m_b.v4());
                ...
                const bool pushST =
                    pushFrm && (KF.m_usST[iST2 - 1] & LBA_FLAG_TRACK_PUSH);
                if (ucr || ud)
                {
                    if (!ud)
                    {
              ...
              UT::Print(" [%d] %d: [%d] %e + %e = %e\n", m_LFs[m_ic2LF.back()].m_T.m_iFrm, m_iIter, LF.m_T.m_iFrm,
                        ...
                        if (pushST)
                        {
                                 ...
                                 --iST)
                            {
                                KF.m_AxsST[iST].m_Sadd += AST.m_add;
                                KF.m_usST[iST] |=
                                    LBA_FLAG_TRACK_UPDATE_INFORMATION;
                            }
...
//#ifdef CFG_DEBUG
            ...
            UT::Print("%d: %e + %e = %e\n", iz, A.m_Aczz.m_A.m00(), SAczz.m_A.m00(), A.m_Aczz.m_A.m00() + SAczz.m_A.m00());
                    ...
                    {
                        Camera::Factor::Unitary::CC::AmB(
                            A.m_Aczz, dAczz, dAczz);
                        SAczz += dAczz;
                    }
                    // dF = A.m_F - dF;
                    // m_F = dF + m_F;
...
void LocalBundleAdjustor::UpdateFactorsFeatureKF()
{
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        for (int iZ = 0; iZ < NZ; ++iZ)
        {
            ...
            const bool ucx =
                           (m_ucsKF[_iKF] & LBA_FLAG_FRAME_UPDATE_CAMERA) != 0,
                       ucr = ucx || ucz;
            ...
            *Tr = C / m_CsKF[_iKF];
            ...
            for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz)
            {
                ...
                // dF = A.m_F;
                FTR::GetFactor<LBA_ME_FUNCTION>(BA_WEIGHT_FEATURE_KEY_FRAME,
                    Tr,
                    _KF.m_xs[ix],
                    _ds[ix],
                    C,
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            if (_ud)
            {
...
#endif
                {
                    ...
                    // dF = a.m_F;
                    zp.GetFactor<LBA_ME_FUNCTION>(
                        BA_WEIGHT_PRIOR_DEPTH, ds[ix].u(), A);
                    dadd = A;
                }
...
void LocalBundleAdjustor::UpdateFactorsIMU()
{
    ...
    for (int ic1 = 0, ic2 = 1; ic2 < nLFs; ic1 = ic2++)
    {
        ...
        // dF = A.m_F;
        m_DsLF[iLF2].GetFactor(BA_WEIGHT_IMU,
            m_CsLF[iLF1],
            m_CsLF[iLF2],
            m_K.m_pu,
            &A,
        ...
        {
            Camera::Factor::Unitary::CM::AmB(
                A.m_A11.m_Acm, dAcm1.m_Acm, dAcm1.m_Acm);
            Camera::Factor::Unitary::MM::AmB(
                A.m_A11.m_Amm, dAcm1.m_Amm, dAcm1.m_Amm);
            SAcm1.m_Au += dAcm1;
        }
        if (uc2)
        {
        ...
        {
            Camera::Factor::Unitary::CM::AmB(
                A.m_A22.m_Acm, dAcm2.m_Acm, dAcm2.m_Acm);
            Camera::Factor::Unitary::MM::AmB(
                A.m_A22.m_Amm, dAcm2.m_Amm, dAcm2.m_Amm);
            SAcm2.m_Au += dAcm2;
        }
        // dF = A.m_F - dF;
        // m_F = dF + m_F;
...
void LocalBundleAdjustor::UpdateFactorsFixMotion()
{
    ...
    for (int iLF = 0; iLF < nLFs; ++iLF)
    {
        ...
        SA.m_A.IncreaseDiagonal012(zv[i].w());
        SA.m_b.Increase(0, A.m_Av.m_b);
        SA.m_A.IncreaseDiagonal345(zba[i].w());
        SA.m_b.Increase(3, A.m_Aba.m_b);
        SA.m_A.IncreaseDiagonal678(zbw[i].w());
        SA.m_b.Increase(6, A.m_Abw.m_b);
    }
#ifdef CFG_VERBOSE
    if (m_verbose >= 3)
    {
        UT::Print("  Fix Motion = (%d %d %d) / %d = (%.2f%% %.2f%% %.2f%%)\n",
            ...
            SNbw,
            nLFs,
            UT::Percentage(SNv, nLFs),
            UT::Percentage(SNba, nLFs),
            UT::Percentage(SNbw, nLFs));
    }
#endif
}
...
void LocalBundleAdjustor::UpdateSchurComplement()
{
    ...
    for (int iLF = 0; iLF < nLFs; ++iLF)
    {
        ...
        else
        {
            ...
            {
                if (!(m_ucsLF[LF.m_iLFsMatch[ik]] &
                        LBA_FLAG_FRAME_UPDATE_CAMERA))
                {
                    continue;
                }
                LF.m_Zm.m_SMczms[ik].MakeZero();
                const int izm = LF.m_Zm.m_ik2zm[ik],
                          Nzm = LF.m_Zm.m_ik2zm[ik + 1] - izm;
            ...
            for (int iI = 0; iI < NI; ++iI)
            {
                ...
                for (int i = i1; i < i2; ++i)
                {
                    if (uds[LF.m_zs[LF.m_Zm.m_izms[i].m_iz1].m_ix] &
                        LBA_FLAG_FRAME_UPDATE_DEPTH)
                    {
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        const ubyte* uds = m_uds.data() + m_iKF2d[iKF];
        const int iX = static_cast<int>(iX2d.size()),
                  iXST = static_cast<int>(iXST2dST.size());
        KeyFrame& KF = m_KFs[iKF];
        const int Nx = static_cast<int>(KF.m_xs.size());
        iKF2X[iKF] = iX;
        iKF2XST[iKF] = iXST;
        iX2d.resize(iX + Nx, -1);
        iXST2dST.resize(iXST + static_cast<int>(KF.m_STs.size()), -1);
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            ubyte update = LBA_FLAG_MARGINALIZATION_DEFAULT,
                  nonZero = LBA_FLAG_MARGINALIZATION_DEFAULT;
            for (int iST = iST1; iST < iST2; ++iST)
            {
    ...
    // SIMD::Add(N, UT::Inverse(BA_VARIANCE_REGULARIZATION_DEPTH,
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            for (int iST = iST1; iST < iST2; ++iST)
            {
                ...
                {
#ifdef CFG_DEBUG
                    UT_ASSERT((KF.m_usST[iST] &
                                  LBA_FLAG_TRACK_UPDATE_INFORMATION_ZERO) == 0);
#endif
                    if (!ud)
                    {
...
//#ifdef CFG_DEBUG
    ...
    for (int iLF = 0; iLF < nLFs; ++iLF)
    {
        ...
        for (int iZ = 0; iZ < NZ; ++iZ)
        {
            ...
            const int* ixST2dST = iXST2dST.data() + iKF2XST[iKF];
            const ubyte* uds = m_uds.data() + m_iKF2d[iKF];
            // const bool ucx = (m_ucsKF[iKF] & LBA_FLAG_FRAME_UPDATE_CAMERA) !=
            // 0, ucr = ucx || ucz;
            const KeyFrame& KF = m_KFs[iKF];
            for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz)
            {
                ...
                if (id >= 0)
                {
                    FTR::Factor::FixSource::Marginalize(
                        _mdds[id], LF.m_Azs1[iz], &LF.m_Mzs1[iz]);
                }
#ifdef CFG_DEBUG
                else if (id == -2)
                {
              ...
              if (idST >= 0) {
                ...
                UT::Print(", -mdd1 = %e\n", KF.m_MxsST[iST].m_mdd.m_a);
                    ...
                    if (LF.m_ms[iz] & LBA_FLAG_MARGINALIZATION_UPDATE)
                    {
                        if (nonZero2)
                        {
                            ...
                            {
                                LF.m_ms[iz] |=
                                    LBA_FLAG_MARGINALIZATION_NON_ZERO;
                            }
                            if (LF.m_Nsts[iz] == 1)
                            {
                ...
                if (LF.m_ms[iz] & LBA_FLAG_MARGINALIZATION_NON_ZERO)
                {
                    if (!ucz && nonZero1)
                    {
                        dMczz = M.m_Mczz;
                        FTR::Factor::FixSource::Marginalize(
                            SmddST, LF.m_AzsST[iz].m_adcA, &SmdczST, &M);
                        Camera::Factor::Unitary::CC::AmB(
                            M.m_Mczz, dMczz, dMczz);
                        SMczz += dMczz;
                    }
                    else
                    {
...
//#if 1
        ...
        for (int iI = 0; iI < NI; ++iI)
        {
            ...
            const bool _ucz =
                           (m_ucsLF[_iLF] & LBA_FLAG_FRAME_UPDATE_CAMERA) != 0,
                       uczm = ucz || _ucz;
            ...
            for (int i = i1; i < i2; ++i)
            {
                ...
                if (LF.m_Zm.m_ms[i] & LBA_FLAG_MARGINALIZATION_NON_ZERO)
                {
                    ...
                    {
                        dMczm = Mczm;
                        FTR::Factor::FixSource::Marginalize(SmddST,
                            LF.m_AzsST[izm.m_iz1],
                            _LF.m_AzsST[izm.m_iz2],
                            &SmdczST,
                            &Mczm);
...
//#ifdef CFG_DEBUG
    ...
    if (m_verbose >= 3)
    {
        ...
        UT::PrintSeparator();
        UT::Print(
            "*%2d: [LocalBundleAdjustor::UpdateSchurComplement]\n", m_iIter);
        UT::Print("  Track    = %5d / %5d = %.2f%% (%d / %d = %.2f%%)\n",
            Nd,
            _Nd,
            UT::Percentage(Nd, _Nd),
            SNX,
            nKFs,
            UT::Percentage(SNX, nKFs));
        UT::Print("  TrackST  = %5d / %5d = %.2f%%\n",
            NdST,
            NST,
            UT::Percentage(NdST, NST));
        UT::Print("  Schur    = %5d / %5d = %.2f%% (%d / %d = %.2f%%)\n",
            SNs1,
            SNs2,
            UT::Percentage(SNs1, SNs2),
            SNS,
            NSLLF,
            UT::Percentage(SNS, NSLLF));
    }
#endif
}
...
//#ifdef CFG_INCREMENTAL_PCG
  ...
  m_xmsLF.MakeZero();
    ...
    if (LBA_EMBEDDED_MOTION_ITERATION)
    {
        ...
        const IMU::Delta::ES ESd1 =
            ComputeErrorStatisticIMU(m_xcsP.Data(), xms, false);
        CameraPrior::Motion::ES ESm1 =
            ComputeErrorStatisticPriorCameraMotion(m_xcsP.Data(), xms);
        ...
        //}
        const IMU::Delta::ES ESd2 =
            ComputeErrorStatisticIMU(m_xcsP.Data(), xms, false);
        CameraPrior::Motion::ES ESm2 =
            ComputeErrorStatisticPriorCameraMotion(m_xcsP.Data(), xms);
        const float F2 = ESd2.Total() + ESm2.Total();
        // UT_ASSERT(F2 <= F1);
        UT::AssertReduction(
            F1, F2, 1, UT::String("[%d] %d", m_LFs[m_ic2LF.back()].m_T.m_iFrm));
        // UT::Print("%e --> %e\n", F1, F2);
#endif
    }
//#ifdef CFG_INCREMENTAL_PCG
...
bool LocalBundleAdjustor::SolveSchurComplementPCG()
{
    ...
    const int pc = 6, pm = 9;
    const int pcm = pc + pm;
    const int Nc = int(m_LFs.size()), Ncp = Nc * pc, Nmp = Nc * pm,
              N = Ncp + Nmp;
    m_Acus.Resize(Nc);
    m_Amus.Resize(Nc);
    m_bs.Resize(N);
    ...
    m_ic2b.resize(Nc);
    const float ar =
        UT::Inverse(BA_VARIANCE_REGULARIZATION_ROTATION, BA_WEIGHT_FEATURE);
    const float ap =
        UT::Inverse(BA_VARIANCE_REGULARIZATION_POSITION, BA_WEIGHT_FEATURE);
    const float av =
        UT::Inverse(BA_VARIANCE_REGULARIZATION_VELOCITY, BA_WEIGHT_FEATURE);
    const float aba = UT::Inverse(
        BA_VARIANCE_REGULARIZATION_BIAS_ACCELERATION, BA_WEIGHT_FEATURE);
    const float abw = UT::Inverse(
        BA_VARIANCE_REGULARIZATION_BIAS_GYROSCOPE, BA_WEIGHT_FEATURE);
...
#ifdef CFG_DEBUG
        UT_ASSERT(Nk >= 1 &&
                  Nk == std::min(Nc - ic, LBA_MAX_SLIDING_TRACK_LENGTH) - 1);
#endif
...
//#ifdef CFG_DEBUG
  ...
  m_drs.AssertEqual(UT::String("%sAp.txt", dir.c_str()), 2, "", -1.0f, -1.0f);
    ...
    {
#endif // _MSC_VER
        const float e2MaxConv[2] = {
            ME::ChiSquareDistance<3>(
                BA_PCG_MIN_CONVERGE_PROBABILITY, BA_WEIGHT_FEATURE),
            ME::ChiSquareDistance<3>(
                BA_PCG_MAX_CONVERGE_PROBABILITY, BA_WEIGHT_FEATURE)};
        Se2Min = Se2;
        e2MaxMin = e2Max;
#ifdef CFG_VERBOSE
        if (m_verbose >= 3)
        {
            UT::PrintSeparator();
            UT::Print(
                "*%2d: [LocalBundleAdjustor::SolveSchurComplement]\n", m_iIter);
...
void LocalBundleAdjustor::SolveSchurComplementGT(
    const AlignedVector<Camera>& CsLF, LA::AlignedVectorXf* xs,
    const bool motion)
{
    if (!m_CsGT)
    {
...
#ifdef LBA_DEBUG_GROUND_TRUTH_STATE_ERROR
                dbw += LA::AlignedVector3f::GetRandom(
                    dbwMax * UT_FACTOR_DEG_TO_RAD);
#endif
                xm.Set678(dbw);
            }
        }
...
bool LocalBundleAdjustor::SolveSchurComplementLast()
{
    ...
    // const float _eps[pmcm] = {epsv, epsv, epsv, epsba, epsba, epsba, epsbw,
    // epsbw, epsbw,
    //                          epsp, epsp, epsp, epsr, epsr, epsr,
    //                          epsv, epsv, epsv, epsba, epsba, epsba, epsbw,
    //                          epsbw, epsbw};
    const float _eps[pcm] = {epsp,
        epsp,
        epsp,
        epsr,
        epsr,
    ...
    b.GetBlock(0, x);
    x.Get(xc2);
    m_xp2s[ic2] = x.SquaredLength();
    b.GetBlock(3, x);
    x.Get(xc2 + 3);
    m_xr2s[ic2] = x.SquaredLength();
    b.GetBlock(6, x);
    x.Get(xm2);
    m_xv2s[ic2] = x.SquaredLength();
    b.GetBlock(9, x);
    x.Get(xm2 + 3);
    m_xba2s[ic2] = x.SquaredLength();
    b.GetBlock(12, x);
    x.Get(xm2 + 6);
    m_xbw2s[ic2] = x.SquaredLength();
    return true;
}
...
void LocalBundleAdjustor::PrepareConditioner()
{
    ...
    const int Nb =
        std::min(LBA_PCG_CONDITIONER_BAND, LBA_MAX_SLIDING_TRACK_LENGTH);
    const int Nc = static_cast<int>(m_LFs.size());
    if (Nb <= 1)
    {
        m_Mcs.Resize(Nc);
    ...
    m_Mcm.Resize(Nc, 2);
    m_McmT.Resize(Nc, 2);
    m_Mmc.Resize(Nc, Nb - 1);
    m_MmcT.Resize(Nc, Nb - 1);
    m_Mmm.Resize(Nc, 2);
    m_MmmT.Resize(Nc, 2);
    for (int ic = 0; ic < Nc; ++ic)
    {
    ...
    {
        e_A.block<pc, pc>(icp, icp) =
            EigenMatrix6x6f(m_Acus[ic]).cast<double>();
        const LA::AlignedMatrix6x6f* Acbs = m_Acbs.Data() + m_ic2b[ic] - 1;
        const int Nbc = ic + Nb > Nc ? Nc - ic : Nb;
        for (int ib = 1, _ic = ic + 1; ib < Nbc; ++ib, ++_ic)
        {
            e_A.block<pc, pc>(icp, _ic * pcm) =
                EigenMatrix6x6f(Acbs[ib]).cast<double>();
        }
        const int iLF = m_ic2LF[ic];
        const Camera::EigenFactor e_Acm = m_SAcmsLF[iLF];
        e_A.block<pm, pm>(imp, imp) =
            EigenMatrix9x9f(m_Amus[ic]).cast<double>();
        e_A.block<pc, pm>(icp, imp) = e_Acm.m_Au.m_Acm.cast<double>();
        if (ic > 0)
        {
    ...
    m_work.Resize((AccsT.BindSize(Nb) + AcmsT.BindSize(2) + AmcsT.BindSize(Nb) +
                      AmmsT.BindSize(2)) /
                  sizeof(float));
    ...
    for (int ic = 0; ic < Nc; ++ic)
    {
        ...
        if (Mcc.InverseLDL(epsc))
        {
            ...
            for (int ib = 1; ib < Nbcc; ++ib)
            {
                ...
                LA::AlignedMatrix6x6f::AddABTToUpper(
                    MccT, AccsT[ib], _Mccs[ib]);
#ifdef LBA_DEBUG_EIGEN_PCG
                ...
                if (ib == 1)
                {
                    LA::AlignedMatrix9x6f::AddABTTo(
                        MccT, AcmsT[ib], m_Mcm[_ic][0]);
                }
            }
            for (int ib = 0; ib < Nbcm; ++ib)
            {
        ...
        if (Mmm.InverseLDL(epsm))
        {
            ...
            for (int ib = 0; ib < Nbmc; ++ib)
            {
                ...
                LA::AlignedMatrix6x6f* _Mccs = m_Mcc[_ic] - ib;
                LA::AlignedMatrix6x9f::AddABTToUpper(
                    MmcT, AmcsT[ib], _Mccs[ib]);
#ifdef LBA_DEBUG_EIGEN_PCG
                _Mccs[ib].SetLowerFromUpper();
#endif
...
#ifdef LBA_DEBUG_EIGEN_PCG
        for (int ic1 = ic, icp1 = icp, imp1 = imp; ic1 < Nc;
             ++ic1, icp1 += pcm, imp1 += pcm)
        {
            const int Nbcc1 = ic1 + Nb > Nc ? Nc - ic1 : Nb;
            const int Nbcm1 = ic1 + 1 == Nc ? 1 : 2;
            const int Nbmc1 = Nbcc1 - 1;
            const int Nbmm1 = Nbcm1;
            for (int ib = 0, ic2 = ic1, icp2 = icp1; ib < Nbcc1;
                 ++ib, ++ic2, icp2 += pcm)
            {
                const EigenMatrix6x6f e_Mcc =
                    e_M.block<pc, pc>(icp1, icp2).cast<float>();
                e_Mcc.AssertEqual(
                    m_Mcc[ic1][ib], 1, UT::String("Mcc[%d][%d]", ic1, ic2));
                e_M.block<pc, pc>(icp1, icp2) =
                    EigenMatrix6x6f(m_Mcc[ic1][ib]).cast<double>();
            }
            for (int ib = 0, ic2 = ic1, imp2 = imp1; ib < Nbcm1;
                 ++ib, ++ic2, imp2 += pcm)
            {
                const EigenMatrix6x9f e_Mcm =
                    e_M.block<pc, pm>(icp1, imp2).cast<float>();
                e_Mcm.AssertEqual(
                    m_Mcm[ic1][ib], 1, UT::String("Mcm[%d][%d]", ic1, ic2));
                e_M.block<pc, pm>(icp1, imp2) =
                    EigenMatrix6x9f(m_Mcm[ic1][ib]).cast<double>();
            }
            for (int ib = 0, ic2 = ic1 + 1, icp2 = icp1 + pcm; ib < Nbmc1;
                 ++ib, ++ic2, icp2 += pcm)
            {
                const EigenMatrix9x6f e_Mmc =
                    e_M.block<pm, pc>(imp1, icp2).cast<float>();
                e_Mmc.AssertEqual(
                    m_Mmc[ic1][ib], 1, UT::String("Mmc[%d][%d]", ic1, ic2));
                e_M.block<pm, pc>(imp1, icp2) =
                    EigenMatrix9x6f(m_Mmc[ic1][ib]).cast<double>();
            }
            for (int ib = 0, ic2 = ic1, imp2 = imp1; ib < Nbmm1;
                 ++ib, ++ic2, imp2 += pcm)
            {
                const EigenMatrix9x9f e_Mmm =
                    e_M.block<pm, pm>(imp1, imp2).cast<float>();
                e_Mmm.AssertEqual(
                    m_Mmm[ic1][ib], 1, UT::String("Mmm[%d][%d]", ic1, ic2));
                e_M.block<pm, pm>(imp1, imp2) =
                    EigenMatrix9x9f(m_Mmm[ic1][ib]).cast<double>();
            }
        }
#endif
    }
    ...
    const EigenMatrixXd e_I1 = EigenMatrixXd(e_A * e_AI),
                        e_I2 = EigenMatrixXd(e_AI * e_A);
    ...
    for (int ic1 = 0, icp1 = 0; ic1 < Nc; ++ic1, icp1 += pcm)
    {
        for (int ic2 = 0, icp2 = 0; ic2 < Nc; ++ic2, icp2 += pcm)
        {
            ...
            {
                EigenMatrix15x15f(
                    e_I1.block<pcm, pcm>(icp1, icp2).cast<float>())
                    .AssertEqual(e_I, 1, str1);
                EigenMatrix15x15f(
                    e_I2.block<pcm, pcm>(icp1, icp2).cast<float>())
                    .AssertEqual(e_I, 1, str2);
            }
            else
            {
                EigenMatrix15x15f(
    ...
    e_r.Resize(N);
    const LA::Vector6f *rcs = (LA::Vector6f*)m_rs.Data(),
                       *zcs = (LA::Vector6f*)m_zs.Data();
    const LA::Vector9f *rms = (LA::Vector9f*)(rcs + Nc),
                       *zms = (LA::Vector9f*)(zcs + Nc);
    for (int ic = 0, icp = 0, imp = pc; ic < Nc; ++ic, icp += pcm, imp += pcm)
    {
    ...
    for (int ic = 0, icp = 0, imp = pc; ic < Nc; ++ic, icp += pcm, imp += pcm)
    {
        const EigenVector6f e_zc1 =
            EigenVector6f(e_z1.block<pc, 1>(icp, 0).cast<float>());
        const EigenVector6f e_zc2 = EigenVector6f(zcs[ic]);
        e_zc1.AssertEqual(e_zc2, 1, UT::String("zc[%d]", ic));
        const EigenVector9f e_zm1 =
            EigenVector9f(e_z1.block<pm, 1>(imp, 0).cast<float>());
...
void LocalBundleAdjustor::ApplyM(
    const LA::AlignedVectorXf& xs, LA::AlignedVectorXf* Mxs)
{
    const int Nb =
        std::min(LBA_PCG_CONDITIONER_BAND, LBA_MAX_SLIDING_TRACK_LENGTH);
    ...
    for (int ic = 0; ic < Nc; ++ic)
    {
        ...
        for (int ib = 0; ib < Nbmc; ++ib)
        {
            LA::AlignedMatrix6x9f::AddAbTo<float>(
                MmcsT[ib], bm, bcs[ic + ib + 1]);
        }
...
void LocalBundleAdjustor::ApplyA(
    const LA::AlignedVectorXf& xs, LA::AlignedVectorXf* Axs)
{
...
void LocalBundleAdjustor::ApplyAcm(const LA::ProductVector6f* xcs,
    const LA::Vector9f* xms, LA::Vector6f* Axcs, LA::Vector9f* Axms,
    const bool Acc, const LA::AlignedMatrix9x9f* Amus)
{
    ...
    for (int ic = 0, r = 0; ic < Nc; ++ic, r = 1 - r)
    {
        ...
        {
            const float a =
                i < 6 ? SAcm.m_Ab.m_Amm.m_Ababa : SAcm.m_Ab.m_Amm.m_Abwbw;
...
LocalBundleAdjustor::Residual LocalBundleAdjustor::ComputeResidual(
    const LA::AlignedVectorXf& xs, const bool minus)
{
    Residual R;
    ApplyA(xs, &m_rs);
    if (minus)
...
void LocalBundleAdjustor::SolveBackSubstitution()
{
    ...
    for (int ic = 0; ic < Nc; ++ic)
    {
        ...
        for (int iZ = 0; iZ < NZ; ++iZ)
        {
            ...
            {
                uds[LF.m_zs[iz].m_ix] |=
                    LBA_FLAG_TRACK_UPDATE_BACK_SUBSTITUTION;
            }
        }
    }
...
#ifdef CFG_GROUND_TRUTH
void LocalBundleAdjustor::SolveBackSubstitutionGT(
    const std::vector<Depth::InverseGaussian>& ds, LA::AlignedVectorXf* xs)
{
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        const Depth::InverseGaussian* _ds =
            iX == -1 ? m_ds.data() + id : ds.data() + iX;
...
//#if 1
  ...
  for (int ic = 0; ic < Nc; ++ic) {
    ...
    m_work.Resize(A.BindSize(Nmr, Nmc) + b.BindSize(Nmr) + x.BindSize(Nc) +
                  ai.BindSize(Nmc));
...
void LocalBundleAdjustor::EmbeddedPointIteration(
    const AlignedVector<Camera>& CsLF, const AlignedVector<Rigid3D>& CsKF,
    const std::vector<ubyte>& ucsKF, const std::vector<ubyte>& uds,
...
//#ifdef CFG_DEBUG
#if 0
  {
    ...
    {
        const FRM::Frame* F =
            ic < nKFs ? (FRM::Frame*)&m_KFs[ic] : &m_LFs[ic - nKFs];
#ifdef CFG_STEREO
        if (ic < nKFs)
        {
...
//#ifdef CFG_DEBUG
    ...
    for (int ic = 0; ic < Nc; ++ic)
    {
        ...
        for (int iZ = 0; iZ < NZ; ++iZ)
        {
            ...
            for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz)
            {
                ...
                if (z.m_z.Valid())
                {
                    // z.m_W.GetScaled(KF ? KF->m_Azs[iz].m_wx :
                    // LF->m_Lzs[iz].m_wx, W);
                ...
                if (z.m_zr.Valid())
                {
                    // z.m_Wr.GetScaled(KF ? KF->m_Azs[iz].m_wxr :
                    // LF->m_Lzs[iz].m_wxr, W);
...
#else
                // z.m_W.GetScaled(KF ? KF->m_Azs[iz].m_wx : LF->m_Lzs[iz].m_wx,
                // W);
                const LA::SymmetricMatrix2x2f& W = z.m_W;
                const int i = ++Nzs[id];
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            if (!Depth::Triangulate(BA_WEIGHT_FEATURE,
                    Nzs[id],
                    m_zds.data() + id2z[id],
                    &d,
                    &m_work,
