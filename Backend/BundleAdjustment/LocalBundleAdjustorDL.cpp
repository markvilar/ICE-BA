...
void LocalBundleAdjustor::ComputeReductionFeatureLF()
{
    ...
    for (int ic = 0; ic < nLFs; ++ic)
    {
        ...
        const LA::ProductVector6f* xc =
            (m_ucsLF[iLF] & LBA_FLAG_FRAME_UPDATE_CAMERA) ? &m_xcsP[ic] : NULL;
        dFa = dFp = 0.0f;
        const int NZ = int(LF.m_Zs.size());
        for (int iZ = 0; iZ < NZ; ++iZ)
        {
            ...
            *Tr = C / m_CsKF[iKF];
            ...
            for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz)
            {
                ...
                const int ix = z.m_ix;
                const float* xd = xds && (uds[ix] & LBA_FLAG_TRACK_UPDATE_DEPTH)
                                      ? &xds[ix]
                                      : NULL;
                if (!xc && !xd)
                {
                ...
                FTR::GetReduction(
                    LF.m_Lzs[iz], Tr, KF.m_xs[ix], ds[ix], z, xc, xd, Ra, Rp);
                dFa = Ra.m_dF + dFa;
                dFp = Rp.m_dF + dFp;
//#ifdef CFG_DEBUG
#if 0
        ...
        for (int iZ = 0; iZ < NZ; ++iZ)
        {
            ...
            *Tr = C / m_CsKF[_iKF];
            ...
            for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz)
            {
                ...
                FTR::GetReduction(
                    KF.m_Azs[iz], Tr, _KF.m_xs[ix], ds[ix], z, xds[ix], Ra, Rp);
//#ifdef CFG_DEBUG
#if 0
        if (iKF == 43) {
...
void LocalBundleAdjustor::ComputeReductionPriorCameraMotion()
{
    ...
    m_ZpLF.GetReduction(BA_WEIGHT_PRIOR_CAMERA_MOTION,
        m_ApLF,
        m_CsLF[iLF],
        _xr,
        _xv,
...
void LocalBundleAdjustor::ComputeReductionIMU()
{
    ...
    for (int ic1 = 0, ic2 = 1; ic2 < Nc; ic1 = ic2++)
    {
        ...
        if (ic1 == 0)
        {
            ...
            _xp[r1] =
                uc ? &(xp[r1] = LA::AlignedVector3f(&xcs[ic1].v0())) : NULL;
            _xr[r1] =
                uc ? &(xr[r1] = LA::AlignedVector3f(&xcs[ic1].v3())) : NULL;
...
#endif
        m_DsLF[iLF2].GetReduction(BA_WEIGHT_IMU,
            m_AdsLF[iLF2],
            m_CsLF[iLF1],
            m_CsLF[iLF2],
            m_K.m_pu,
      ...
      if (axd > 40.0f) {
        ...
        UT::DebugStop();
            ...
            d.s2() = DEPTH_VARIANCE_EPSILON +
                     KF.m_Mxs[ix].m_mdd.m_a * BA_WEIGHT_FEATURE;
            // if (!d.Valid()) {
            //  d = dsBkp[ix];
            //  continue;
            //}
...
bool LocalBundleAdjustor::UpdateStatesDecide()
{
    ...
    for (int ic1 = 0, ic2 = 1; ic2 < nLFs; ic1 = ic2++)
    {
        ...
        IMU::PreIntegrate(m_LFs[iLF2].m_us,
            ...
            D.m_u2.Valid() ? &D.m_u2 : NULL,
            BA_ANGLE_EPSILON);
#ifdef LBA_DEBUG_GROUND_TRUTH_MEASUREMENT
        if (!m_CsLFGT.Empty())
        {
            D.DebugSetMeasurement(
                m_CsLFGT[iLF1], m_CsLFGT[iLF2], m_K.m_pu, BA_ANGLE_EPSILON);
        }
#endif
        // if (UT::Debugging()) {
        //  UT::DebugStop();
...
void LocalBundleAdjustor::ConvertCameraUpdates(
...
void LocalBundleAdjustor::ConvertMotionUpdates(const float* xms,
    ...
    LA::AlignedVectorXf* xbw2s)
{
    ...
    xba2s->Resize(Nc);
    xbw2s->Resize(Nc);
    float *_xv2s = xv2s->Data(), *_xba2s = xba2s->Data(),
          *_xbw2s = xbw2s->Data();
    for (int ic = 0; ic < Nc; ++ic)
    {
...
void LocalBundleAdjustor::ConvertCameraMotionResiduals(
    const LA::AlignedVectorXf& rs, const LA::AlignedVectorXf& zs, float* Se2,
    float* e2Max)
{
    const int N = rs.Size();
#ifdef CFG_DEBUG
...
void LocalBundleAdjustor::ConvertDepthUpdates(
    const float* xs, LA::AlignedVectorXf* xds)
{
    int i = 0;
...
void LocalBundleAdjustor::PushDepthUpdates(
...
float LocalBundleAdjustor::AverageDepths(
    const Depth::InverseGaussian* ds, const int N)
{
