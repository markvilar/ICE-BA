...
void GlobalBundleAdjustor::Initialize(IBA::Solver* solver, const int serial,
...
void GlobalBundleAdjustor::PushKeyFrame(const GlobalMap::InputKeyFrame& IKF,
    ...
    const std::vector<Depth::InverseGaussian>& dzs, const float d)
{
    ...
    m_IKFs1.push_back(InputKeyFrame(IKF, us, dzs, d));
...
void GlobalBundleAdjustor::PushDeleteMapPoints(
...
void GlobalBundleAdjustor::PushUpdateCameras(
    const int iFrm, const std::vector<GlobalMap::InputCamera>& Cs)
{
...
void GlobalBundleAdjustor::PushCameraPriorPose(
    const int iFrm, const CameraPrior::Pose& IZp)
{
...
void GlobalBundleAdjustor::PushCameraPriorMotion(
    const int iFrm, const int iKF, const CameraPrior::Motion& IZp)
{
...
//#ifdef CFG_DEBUG
#if 0
  {
    ...
    else if (m_verbose >= 2)
    {
        ...
        UT::Print(
            "  Frame = %d\tMeasurement = %d + %d\tSchur = %d\tTrack = %d\n",
            m_Cs.Size(),
            CountMeasurementsFeature(),
            CountMeasurementsPriorCameraPose(),
            CountSchurComplements(),
            m_ds.size());
#ifdef CFG_GROUND_TRUTH
        if (!m_CsKFGT.Empty() && m_dsGT)
        {
            UT::PrintSeparator();
            PrintErrorStatistic(
                "*GT: ", m_CsKFGT, m_CsLMGT, *m_dsGT, m_DsLMGT, true);
        }
#endif
    }
#endif
...
//#if 0
    ...
    if (m_verbose >= 2)
    {
        ...
        viewer->ActivateFrame(
            viewer->GetKeyFrames() + viewer->GetLocalFrames() - 1);
...
//#ifdef CFG_GROUND_TRUTH
    ...
    if (m_history >= 1)
    {
        ...
        if (m_history >= 2)
        {
            ...
            hist.m_ESb =
                ComputeErrorStatistic(m_CsBkp, m_CsLMBkp, m_dsBkp, m_DsLM);
            hist.m_ESp = ComputeErrorStatistic(
                m_CsBkp, m_CsLMBkp, m_dsBkp, m_DsLM, m_xsDL, false);
#ifdef CFG_GROUND_TRUTH
            if (m_CsGT)
            {
                ...
                if (m_dsGT)
                {
                    ...
                    {
                        EmbeddedPointIteration(
                            m_CsKFGT, m_ucsGT, m_udsGT, m_dsGT);
                    }
                    SolveBackSubstitutionGT(m_dsBkp, &m_xsGT);
                    hist.m_ESaGT = ComputeErrorStatistic(
                        m_CsKFGT, m_CsLMGT, *m_dsGT, m_DsLMGT);
                    hist.m_ESpGT = ComputeErrorStatistic(
                        m_CsBkp, m_CsLMBkp, m_dsBkp, m_DsLM, m_xsGT, false);
                }
            }
#endif
...
#ifdef GBA_DEBUG_PRINT
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            else
            {
                ...
                const xp128f sm =
                    xp128f::get(1.0f / (CountSchurComplements() - nKFs));
                for (int iKF = 0; iKF < nKFs; ++iKF)
                {
                    C = m_Cs[iKF] * C;
...
void GlobalBundleAdjustor::SetCallback(
...
bool GlobalBundleAdjustor::SaveCameras(
    const std::string fileName, const bool poseOnly)
{
    ...
    const int Nc1 = static_cast<int>(m_CsDel.size()), Nc2 = m_Cs.Size(),
              Nc = Nc1 + Nc2;
...
bool GlobalBundleAdjustor::SaveResiduals(
...
void GlobalBundleAdjustor::ComputeErrorIMU(
    float* er, float* ep, float* ev, float* eba, float* ebw)
{
    *er = *ep = *ev = *eba = *ebw = 0.0f;
         ...
         ic1 = ic2++, im1 = im2++)
    {
        ...
        const IMU::Delta::Error e = m_DsLM[im2].GetError(
            m_CsLM[im1], m_CsLM[im2], m_K.m_pu, BA_ANGLE_EPSILON);
...
float GlobalBundleAdjustor::ComputeRMSE()
{
    ...
    if (!m_CsKFGT.Empty())
    {
        for (int ic = 0; ic < Nc2; ++ic)
        {
            const Point3D p1 = m_Cs[ic].GetPosition(),
                          p2 = m_CsKFGT[ic].GetPosition();
            Se2 += (p1 - p2).SquaredLength();
        }
    }
#endif
...
void GlobalBundleAdjustor::UpdateData()
{
    ...
    if (m_verbose >= 2)
    {
        const int Nc = static_cast<int>(m_KFs.size()),
                  Nd = static_cast<int>(m_ds.size());
        const int Ncu =
            UT::VectorCountFlag<ubyte>(m_Ucs, GM_FLAG_FRAME_UPDATE_CAMERA);
        // const int Ndu = UT::VectorCountFlag<ubyte>(m_Uds,
        // GM_FLAG_TRACK_UPDATE_DEPTH);
        UT::PrintSeparator();
...
void GlobalBundleAdjustor::PushKeyFrame(const InputKeyFrame& IKF)
{
...
#endif
    const ubyte udFlag1 =
        GBA_FLAG_TRACK_UPDATE_DEPTH | GBA_FLAG_TRACK_UPDATE_INFORMATION_ZERO;
    ...
    if (im > 0 && !KF.m_us.Empty() && (GBA_PROPAGATE_CAMERA || !v1 || !v2))
    {
        ...
        IMU::PreIntegrate(KF.m_us,
            ...
            &m_work,
            false,
            _KF.m_us.Empty() ? NULL : &_KF.m_us.Back(),
            NULL,
            BA_ANGLE_EPSILON);
    ...
    else
    {
        ...
        if (_im > 0 && !_KF.m_us.Empty())
        {
            ...
            IMU::PreIntegrate(_KF.m_us,
                m_KFs[_iKF - 1].m_T.m_t,
                _t,
                m_CsLM[_im - 1],
                &_D,
                &m_work,
                true,
                _D.m_u1.Valid() ? &_D.m_u1 : NULL,
                &KF.m_us.Front(),
                BA_ANGLE_EPSILON);
    ...
    if (m_CsGT)
    {
        ...
        else
        {
            ...
            m_DsLMGT[im].DebugSetMeasurement(
                _C, m_CsLMGT[im], m_K.m_pu, BA_ANGLE_EPSILON);
#endif
            if (_im > 0 && !_KF.m_us.Empty())
            {
                ...
                IMU::PreIntegrate(_KF.m_us,
                    ...
                    BA_ANGLE_EPSILON);
#ifdef GBA_DEBUG_GROUND_TRUTH_MEASUREMENT
                m_DsLM[_im].DebugSetMeasurement(
                    m_CsLMGT[_im - 1], _C, m_K.m_pu, BA_ANGLE_EPSILON);
    ...
    for (int iZ = 0; iZ < NZ; ++iZ)
    {
        ...
        for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz)
        {
            ...
            if (IKF.m_dzs[iz].Valid() &&
                fabs(IKF.m_dzs[iz].u() - ds[ix].u()) >= BA_UPDATE_DEPTH)
            {
                ds[ix] = IKF.m_dzs[iz];
    ...
    for (int iX1 = 0, iX2 = 0; iX1 < NX; iX1 = iX2)
    {
        ...
        if (m_history >= 3)
        {
            m_ucsGT[_iKF] |= GBA_FLAG_FRAME_UPDATE_DEPTH;
            m_udsGT.insert(
                m_udsGT.begin() + id, Nx, GBA_FLAG_TRACK_UPDATE_DEPTH);
        }
#endif
        ...
        for (int ik2 = 0; ik2 < Nk; ++ik2)
        {
            ...
            if (Nk22 > Nk21)
            {
                const std::vector<int>::iterator _ik = std::lower_bound(
                    _KF.m_iKFsMatch.begin() + _KF.m_Zm.m_SMczms.Size(),
                    _KF.m_iKFsMatch.end(),
                    iKF2);
#ifdef CFG_DEBUG
            ...
            for (int ik1 = 0; ik1 < ik2; ++ik1)
            {
                ...
                const std::vector<int>::iterator _jk2 =
                    std::lower_bound(KF2.m_iKFsMatch.begin() + jk2,
                        KF2.m_iKFsMatch.begin() + KF2.m_Zm.m_SMczms.Size(),
                        iKF1);
                ...
                if (_jk2 == KF2.m_iKFsMatch.end() || *_jk2 != iKF1)
                {
                    ...
                    const std::vector<int>::iterator jk1 = std::lower_bound(
                        KF1.m_iKFsMatch.begin() + KF1.m_Zm.m_SMczms.Size(),
                        KF1.m_iKFsMatch.end(),
                        iKF2);
#ifdef CFG_DEBUG
                ...
                KF2.m_Zm.InsertFeatureMeasurementMatches(
                    jk2, m_izmsTmp, &m_work);
//#ifdef CFG_DEBUG
#if 0
...
void GlobalBundleAdjustor::DeleteKeyFrame(const int iKF)
{
        ...
        m_KFs[iKF + 1].SearchMatchKeyFrame(iKF - 1) == -1)
    {
        ...
        const std::vector<int>::iterator ik =
            std::lower_bound(KF.m_iKFsMatch.begin() + KF.m_Zm.m_SMczms.Size(),
                KF.m_iKFsMatch.end(),
                iKF + 1);
#ifdef CFG_DEBUG
    ...
    for (int jKF2 = iKF + 1; jKF2 < nKFs; ++jKF2)
    {
        ...
        const std::vector<FRM::Measurement>::iterator iZ2 =
            std::lower_bound(KF2.m_Zs.begin(), KF2.m_Zs.end(), iKF);
        m_iKF2Z[jKF2] = iZ2;
        ...
        for (int ik = 0; ik < Nk; ++ik)
        {
            ...
            const bool z1 =
                jKF1 > iKF && iZ1 != KF1.m_Zs.end() && iZ1->m_iKF == iKF;
            if (!z1 && !z2)
            {
                continue;
            }
            ...
            if (z2)
            {
                ...
                const int j1 = static_cast<int>(
                    std::lower_bound(
                        izms.begin() + i1, izms.begin() + j2, iZ2->m_iz1) -
                    izms.begin());
                const int Nzm = j2 - j1;
                ...
                for (int j = j1; j < j2; ++j)
                {
                    ...
                    ms[j] = (uds[ix] & GBA_FLAG_TRACK_UPDATE_INFORMATION_ZERO)
                                ? 0
                                : 1;
                }
    ...
    for (int jKF = iKF + 1; jKF < nKFs; ++jKF)
    {
        ...
        const int _iZ = iZ != KF.m_Zs.end() && iZ->m_iKF == iKF
                            ? static_cast<int>(iZ - KF.m_Zs.begin())
                            : -1;
        if (_iZ != -1)
        {
            Camera::Factor::Unitary::CC &SAczz = m_SAcus[jKF],
                                        &SMczz = m_SMcus[jKF];
            for (int iz = iZ->m_iz1; iz < iZ->m_iz2; ++iz)
            {
                ...
                Aczz.MakeMinus();
                SAczz += Aczz;
                if (uds[KF.m_zs[iz].m_ix] &
                    GBA_FLAG_TRACK_UPDATE_INFORMATION_ZERO)
                {
                    continue;
                }
        ...
        if (im1 >= 0 && im2 < Nm)
        {
            ...
            else
            {
                IMU::PreIntegrate(KF2.m_us,
                    ...
                    &m_work,
                    false,
                    KF1.m_us.Empty() ? NULL : &KF1.m_us.Back(),
                    NULL,
                    BA_ANGLE_EPSILON);
            }
        }
        ...
        if (im1 >= 0 && im2 < Nm)
        {
            ...
            else
            {
                ...
                D.DebugSetMeasurement(
                    m_CsLMGT[im1], m_CsLMGT[im], m_K.m_pu, BA_ANGLE_EPSILON);
                m_DsLM[im].DebugSetMeasurement(
                    m_CsLMGT[im1], m_CsLMGT[im], m_K.m_pu, BA_ANGLE_EPSILON);
#endif
            }
        }
    ...
    if (im >= 0)
    {
        ...
        if (im1 >= 0 && im2 < Nm && m_DsLM[im].Valid())
        {
            ...
            IMU::Delta::Factor& A = m_AdsLM[im2];
            m_DsLM[im].GetFactor(BA_WEIGHT_IMU,
                m_CsLM[im1],
                m_CsLM[im],
                m_K.m_pu,
                &A,
    ...
    for (iZp = 0; iZp < NZp1; ++iZp)
    {
        ...
        const std::vector<int>::iterator i =
            std::lower_bound(Zp.m_iKFs.begin(), Zp.m_iKFs.end(), iKF);
        const int _i = static_cast<int>(i - Zp.m_iKFs.begin()) + 1;
        const bool del =
            Zp.m_iKFr == iKF || (i != Zp.m_iKFs.end() && *i == iKF);
        ...
        if (nonZero)
        {
            ...
            Zp.GetFactor(
                BA_WEIGHT_PRIOR_CAMERA_POSE, m_Cs, &Ap, &Up, BA_ANGLE_EPSILON);
        }
        for (int j = -1, _j = 0; j < N; ++j, ++_j)
        {
            ...
            if (Zp.Valid())
            {
                const int iKF1 = std::min(Zp.m_iKFr, jKF),
                          iKF2 = std::max(Zp.m_iKFr, jKF);
                KeyFrame& KF2 = m_KFs[iKF2];
            ...
            for (int i2 = i1 + 1, _i2 = i2 + 1; i2 < N; ++i2, ++_i2)
            {
                ...
                if (nonZero)
                {
                    Camera::Factor::Binary::CC::AmB(
                        Ap.m_A[_i1][_i2], m_ApTmp[_i1][_i2], dAb);
                }
                else
                {
...
//#ifdef CFG_DEBUG
  ...
  if (m_debug) {
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
...
#ifdef CFG_DEBUG
        UT_ASSERT((mcs[iKF] != 0) ==
                  UT::VectorExistFlag<ubyte>(mds.data() + id, Nx, 3));
#endif
        if (mcs[iKF])
        {
        ...
        for (int ik = 0; ik < Nk; ++ik)
        {
            KF.m_Zm.DeleteFeatureMeasurementMatches(ik,
                KF.m_Zm.m_ik2zm[ik],
                KF.m_Zm.m_ik2zm[ik + 1],
                izsList[KF.m_iKFsMatch[ik]],
                izs);
        }
    }
//#ifdef CFG_DEBUG
...
void GlobalBundleAdjustor::UpdateCameras(
...
void GlobalBundleAdjustor::PushCameraPriorPose(const CameraPrior::Pose& Zp)
{
    ...
    const int nKFs = static_cast<int>(m_KFs.size()),
              imr = Zp.m_iKFr - (nKFs - m_CsLM.Size());
    if (imr >= 0)
    {
    ...
    for (int i1 = -1; i1 < N; ++i1)
    {
        ...
        {
            const int iKF2 = Zp.m_iKFs[i2];
            const int _iKF1 = std::min(iKF1, iKF2),
                      _iKF2 = std::max(iKF1, iKF2);
...
void GlobalBundleAdjustor::PushFeatureMeasurementMatchesNext(
    ...
    const std::vector<int>& iX2z2, FRM::MeasurementMatch& Zm)
{
    m_izmsTmp.resize(0);
