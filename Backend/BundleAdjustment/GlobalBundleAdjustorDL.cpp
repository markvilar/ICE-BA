...
#endif
            LA::AlignedMatrix6x6f::AddAbTo(
                KF.m_SAcxzs[iZ], gc, (float*)&Agcs[_iKF]);
        ...
        for (int iZ = 0; iZ < NZ; ++iZ)
        {
            ...
            *Tr = C / m_Cs[_iKF];
            ...
            const LA::ProductVector6f* _xc =
                (m_ucs[_iKF] & GBA_FLAG_FRAME_UPDATE_CAMERA) ? &m_xcsP[_iKF]
                                                             : NULL;
            ...
            for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz)
            {
                ...
                const int ix = z.m_ix;
                const float* xd = xds && (uds[ix] & GBA_FLAG_TRACK_UPDATE_DEPTH)
                                      ? &xds[ix]
                                      : NULL;
                if (!ucr && !xd)
                {
...
void GlobalBundleAdjustor::ComputeReductionPriorCameraMotion()
{
    ...
    m_ZpLM.GetReduction(BA_WEIGHT_PRIOR_CAMERA_MOTION,
        m_ApLM,
        m_CsLM[im],
        _xr,
        _xv,
...
void GlobalBundleAdjustor::ComputeReductionIMU()
{
         ...
         ic1 = ic2++, im1 = im2++)
    {
        ...
        if (im1 == 0)
        {
            ...
            _xp[r1] =
                uc ? &(xp[r1] = LA::AlignedVector3f(&xcs[im1].v0())) : NULL;
            _xr[r1] =
                uc ? &(xr[r1] = LA::AlignedVector3f(&xcs[im1].v3())) : NULL;
        ...
        m_DsLM[im2].GetReduction(BA_WEIGHT_IMU,
            m_AdsLM[im2],
            m_CsLM[im1],
            m_CsLM[im2],
            m_K.m_pu,
...
void GlobalBundleAdjustor::ComputeReductionFixPositionZ()
{
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        z.GetReduction(A,
            m_CsBkp[iKF].GetPositionZ(),
            m_Cs[iKF].GetPositionZ(),
            xcs[iKF].v2(),
            Ra,
            Rp);
        m_dFa = Ra.m_dF + m_dFa;
        ...
        UT::DebugStart();
            ...
            if (!m_CsLMGT.Empty())
            {
                D.DebugSetMeasurement(
                    m_CsLMGT[im1], m_CsLMGT[im2], m_K.m_pu, BA_ANGLE_EPSILON);
            }
#endif
//#ifdef CFG_DEBUG
#if 0
    ...
    const LA::AlignedVectorX<PCG_TYPE>& zs, PCG_TYPE* Se2, PCG_TYPE* e2Max)
{
    ...
    *e2Max = 0;
    if (GBA_PCG_CONDITIONER_BAND <= 1)
    {
        ...
        const int Nc = m_Cs.Size();
        const LA::Vector6<PCG_TYPE>* e2cs =
            (LA::Vector6<PCG_TYPE>*)m_e2s.Data();
        for (int ic = 0; ic < Nc; ++ic)
        {
...
void GlobalBundleAdjustor::ConvertDepthUpdates(
...
void GlobalBundleAdjustor::PushDepthUpdates(
...
float GlobalBundleAdjustor::AverageDepths(
