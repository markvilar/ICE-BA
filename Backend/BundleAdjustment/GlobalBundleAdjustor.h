...
public:
    ...
    public:
        ...
        inline void PushFeatureMeasurements(const int iKF,
            const std::vector<FTR::Measurement>& zs, int* ik, int* iz,
            ...
            const int iKF, const std::vector<int>::iterator* ik = NULL)
        {
            ...
            else
            {
                _ik = ik ? *ik
                         : std::lower_bound(
                               m_iKFsMatch.begin(), m_iKFsMatch.end(), iKF);
    ...
    public:
        ...
        {
            return m_ESx.Total() + m_ESc.Total() + m_ESm.Total() +
...
protected:
    ...
    virtual void SolveSchurComplementGT(const AlignedVector<Rigid3D>& Cs,
        const AlignedVector<Camera>& CsLM, LA::AlignedVectorXf* xs,
    ...
    virtual Residual ComputeResidual(
        const LA::AlignedVectorX<PCG_TYPE>& xs, const bool minmus = false);
    ...
    virtual void EmbeddedPointIteration(const AlignedVector<Rigid3D>& Cs,
        const std::vector<ubyte>& ucs, const std::vector<ubyte>& uds,
    ...
    virtual IMU::Delta::ES ComputeErrorStatisticIMU(
        const AlignedVector<Camera>& CsLM,
        const AlignedVector<IMU::Delta>& DsLM, const LA::ProductVector6f* xcs,
        const LA::Vector9f* xms, const bool updateOnly = true);
    virtual Camera::Fix::Origin::ES ComputeErrorStatisticFixOrigin(
        const AlignedVector<Rigid3D>& Cs,
        const AlignedVector<LA::ProductVector6f>& xcs,
