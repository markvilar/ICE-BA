  ...
  return;
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        for (int iZ = 0; iZ < NZ; ++iZ)
        {
            ...
            UT_ASSERT(_iKF < iKF);
#endif
            const bool ucx = (m_ucs[_iKF] & GBA_FLAG_FRAME_UPDATE_CAMERA) != 0,
                       ucr = ucx || ucz;
            ...
            *Tr = C / m_Cs[_iKF];
            ...
            for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz)
            {
                ...
                // dF = A.m_F;
                FTR::GetFactor<GBA_ME_FUNCTION>(BA_WEIGHT_FEATURE,
                    Tr,
                    _KF.m_xs[ix],
                    _ds[ix],
                    C,
...
//#ifdef CFG_DEBUG
    ...
    if (m_verbose >= 3)
    {
        const int Nz = CountMeasurementsFeature(),
                  NZ = CountMeasurementsFrame();
        UT::Print("  Feature = %d / %d = %.2f%% (%d / %d = %.2f%%)\n",
            SNz,
            Nz,
            UT::Percentage(SNz, Nz),
            SNZ,
            NZ,
            UT::Percentage(SNZ, NZ));
    }
  ...
  exit(0);
    ...
    for (int iZ = 0; iZ < NZ; ++iZ)
    {
        ...
        if (GBA_PRIOR_CAMERA_POSE_ROBUST)
        {
            ...
            const float F =
                (Z.GetCost(1.0f, A.m_Je.m_e) - Z.m_xTb) / BA_WEIGHT_FEATURE;
            // const float F = (A.m_F / BA_WEIGHT_PRIOR_CAMERA_POSE - Z.m_xTb) /
            // BA_WEIGHT_FEATURE;
            const float w =
                BA_WEIGHT_PRIOR_CAMERA_POSE * ME::Weight<GBA_ME_FUNCTION>(F);
    ...
    m_ts[TM_TEST_5].Stop();
        ...
        for (int i = -1, _i = 0; i < N; ++i, ++_i)
        {
            ...
            const int iKF1 = std::min(Z.m_iKFr, iKF),
                      iKF2 = std::max(Z.m_iKFr, iKF);
        ...
        for (int i1 = 0, _i1 = 1; i1 < N; ++i1, ++_i1)
        {
            ...
            for (int i2 = i1 + 1, _i2 = i2 + 1; i2 < N; ++i2, ++_i2)
            {
...
#endif
                const bool uc2 =
                    (m_ucs[iKF2] & GBA_FLAG_FRAME_UPDATE_CAMERA) != 0;
                ...
                {
                    Camera::Factor::Binary::CC::AmB(
                        _Ab, m_ApTmp[_i1][_i2], dAb);
                    KF2.m_SAps[ip] += dAb;
                }
            }
        }
...
void GlobalBundleAdjustor::UpdateFactorsPriorCameraMotion()
{
    ...
    const bool _ucm =
        uc || (ucm & (GBA_FLAG_CAMERA_MOTION_UPDATE_VELOCITY |
                         GBA_FLAG_CAMERA_MOTION_UPDATE_BIAS_ACCELERATION |
        ...
        for (int ix = 0; ix < Nx; ++ix)
        {
            ...
            if (KF.m_xs[ix].m_xr.Valid())
            {
                ...
                FTR::GetFactor<GBA_ME_FUNCTION>(
                    BA_WEIGHT_FEATURE, m_K.m_br, ds[ix], KF.m_xs[ix], &A, &U);
                dadd = A.m_add;
            }
            else
#endif
...
void GlobalBundleAdjustor::UpdateFactorsIMU()
{
         ...
         ic1 = ic2++, im1 = im2++)
    {
        ...
        // dF = A.m_F;
        m_DsLM[im2].GetFactor(BA_WEIGHT_IMU,
            m_CsLM[im1],
            m_CsLM[im2],
            m_K.m_pu,
            &A,
...
void GlobalBundleAdjustor::UpdateFactorsFixMotion()
{
    ...
    const Camera::Fix::Zero zv[3] = {
        ...
        Camera::Fix::Zero(
            BA_WEIGHT_FIX_MOTION, BA_VARIANCE_FIX_VELOCITY_INVALID)};
    const Camera::Fix::Zero zba[3] = {Camera::Fix::Zero(BA_WEIGHT_FIX_MOTION,
                                          BA_VARIANCE_FIX_BIAS_ACCELERATION),
        Camera::Fix::Zero(
            BA_WEIGHT_FIX_MOTION, BA_VARIANCE_FIX_BIAS_ACCELERATION_INITIAL),
        Camera::Fix::Zero(
            BA_WEIGHT_FIX_MOTION, BA_VARIANCE_FIX_BIAS_ACCELERATION_INVALID)};
    const Camera::Fix::Zero zbw[3] = {
        ...
        Camera::Fix::Zero(
            BA_WEIGHT_FIX_MOTION, BA_VARIANCE_FIX_BIAS_GYROSCOPE_INVALID)};
    const ubyte ucmFlag = GBA_FLAG_CAMERA_MOTION_UPDATE_ROTATION |
                          GBA_FLAG_CAMERA_MOTION_UPDATE_POSITION |
                          GBA_FLAG_CAMERA_MOTION_UPDATE_VELOCITY |
                          GBA_FLAG_CAMERA_MOTION_UPDATE_BIAS_ACCELERATION |
    ...
    if (m_verbose >= 3)
    {
        ...
        UT::Print("  Fix Motion = (%d %d %d) / %d = (%.2f%% %.2f%% %.2f%%)\n",
            ...
            SNbw,
            Nm,
            UT::Percentage(SNv, Nm),
            UT::Percentage(SNba, Nm),
            UT::Percentage(SNbw, Nm));
    }
#endif
}
  ...
  return;
    ...
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        ...
        const int iX = static_cast<int>(iX2d.size()),
                  Nx = static_cast<int>(KF.m_xs.size());
        iKF2X[iKF] = iX;
        iX2d.resize(iX + Nx, -1);
            ...
            if (id >= 0)
            {
                if (uc || (uds[ix] & GBA_FLAG_TRACK_UPDATE_INFORMATION_ZERO))
                {
                    FTR::Factor::Full::Source::Marginalize(
                        _mdds[id], KF.m_Axs[ix].m_Sadx, &KF.m_Mxs1[ix], &M);
                    SMcxx += M.m_Mcxx;
                }
                else
                {
...
#ifdef CFG_DEBUG
                UT_ASSERT(
                    (uds[ix] & GBA_FLAG_TRACK_UPDATE_INFORMATION_ZERO) == 0);
#endif
                if (!uc)
                {
        ...
        for (int iZ = 0; iZ < NZ; ++iZ)
        {
            ...
            for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz)
            {
                ...
                if (id >= 0)
                {
                    ...
                    {
                        FTR::Factor::Full::Marginalize(_mdds[id],
                            _KF.m_Mxs1[ix],
                            KF.m_Azs1[iz],
                            &KF.m_Mzs1[iz],
                            &M,
...
#ifdef CFG_DEBUG
                    UT_ASSERT((_uds[ix] &
                                  GBA_FLAG_TRACK_UPDATE_INFORMATION_ZERO) == 0);
#endif
                    if (!ucr)
                    {
...
//#ifdef CFG_DEBUG
        ...
        for (int ik = 0; ik < Nk; ++ik)
        {
            ...
            ubyte& Sczm = Scbs[ik];
#endif
            const bool _ucz = (m_ucs[_iKF] & GBA_FLAG_FRAME_UPDATE_CAMERA) != 0,
                       uczm = _ucz || ucz;
                ...
                if (iadcz >= 0)
                {
                    if (uczm || m_marksTmp1[izm.m_iz2])
                    {
                        FTR::Factor::Full::Marginalize(
                            _KF.m_Mzs1[izm.m_iz1], m_adczsTmp[iadcz], Mczm);
                        SMczm += Mczm;
                    }
                    else
                    {
    ...
    if (m_verbose >= 3)
    {
        const int _Nd = static_cast<int>(m_ds.size()),
                  NS = CountSchurComplements();
        UT::PrintSeparator();
        UT::Print(
            "*%2d: [GlobalBundleAdjustor::UpdateSchurComplement]\n", m_iIter);
...
bool GlobalBundleAdjustor::SolveSchurComplementPCG()
{
    ...
    const int pc = 6, pm = 9;
    const int Nc = m_Cs.Size(), Nm = m_CsLM.Size(), Ncp = Nc * pc,
              Nmp = Nm * pm, N = Ncp + Nmp;
    m_Acus.Resize(Nc);
    m_bs.Resize(N);
    const float ar =
  ...
  m_drs.SaveB(UT::String("%sAp_%02d.txt", dir.c_str(), m_iIter));
    ...
    if (std::isfinite(alpha))
    {
        ...
        if (m_verbose >= 3)
        {
            ...
            ConvertMotionUpdates(
                m_dxs.Data() + Ncp, &m_xv2s, &m_xba2s, &m_xbw2s);
...
//#if 0
            ...
            if (m_verbose >= 3)
            {
                ...
                const PCG_TYPE eba = sqrtf(m_xba2s.Mean());
                const PCG_TYPE ebw =
                    sqrtf(m_xbw2s.Mean()) * UT_FACTOR_RAD_TO_DEG;
                fprintf(fp,
                    "%e %e %e %e %e %e %e %e\n",
                    R.m_r2,
                    R.m_F,
...
void GlobalBundleAdjustor::SolveSchurComplementGT(
...
void GlobalBundleAdjustor::PrepareConditioner()
{
    ...
    const int Nb = GBA_PCG_CONDITIONER_BAND, Nc = m_Cs.Size(),
              Nm = m_CsLM.Size();
    if (Nb <= 1)
    {
        m_Mcs.Resize(Nc);
...
//#if 1
  ...
  UT::DebugStop();
    ...
    m_MccT.Resize(Nc, Nb);
    m_McmLM.Resize(Nm, 2);
    m_McmTLM.Resize(Nm, 2);
    m_MmcLM.Resize(Nm, Nb - 1);
    m_MmcTLM.Resize(Nm, Nb - 1);
    m_MmmLM.Resize(Nm, 2);
    m_MmmTLM.Resize(Nm, 2);
    m_Mcc.MakeZero();
    for (int ic = 0; ic < Nc; ++ic)
    {
    ...
    for (int ic = 0, icp = 0; ic < Nc; ++ic, icp += pcm)
    {
        ...
        for (int ikp = 0; ikp < Nkp; ++ikp)
        {
            ...
            if (ic < _ic + Nb)
            {
                g_A.block<pc, pc>(_ic * pcm, icp) =
                    EigenMatrix6x6f(Acbs[ikp]).cast<double>();
            }
        }
    }
    ...
    {
        const Camera::EigenFactor e_Acm = m_SAcmsLM[im];
        g_A.block<pm, pm>(imp, imp) =
            EigenMatrix9x9f(m_AmusLM[im]).cast<double>();
    ...
    m_work.Resize((AccsT.BindSize(Nb) + AcmsTLM.BindSize(2) +
                      AmcsTLM.BindSize(Nb) + AmmsTLM.BindSize(2)) /
                  sizeof(float));
            ...
            for (int ib = 1; ib < Nbcc; ++ib)
            {
                ...
                if (ib == 1 && im >= 0)
                {
                    LA::AlignedMatrix9x6f::AddABTTo(
                        MccT, AcmsTLM[ib], m_McmLM[im + ib][0]);
                }
            }
            for (int ib = 0; ib < Nbcm; ++ib)
            {
...
//#ifdef CFG_DEBUG
      ...
      if (UT::Debugging()) {
        ...
        UT::DebugStop();
            ...
            for (int ib = 0; ib < Nbmc; ++ib)
            {
                ...
                LA::AlignedMatrix6x9f::AddABTToUpper(
                    MmcT, AmcsTLM[ib], _Mccs[ib]);
#ifdef GBA_DEBUG_EIGEN_PCG
                _Mccs[ib].SetLowerFromUpper();
#endif
                for (int jb = ib + 1; jb < Nbmc; ++jb)
                {
                    LA::AlignedMatrix6x9f::AddABTTo(
                        MmcT, AmcsTLM[jb], _Mccs[jb]);
                }
                if (ib == 0 && Nbmm == 2)
                {
                    LA::AlignedMatrix9x9f::AddABTTo(
                        MmcT, AmmsTLM[1], m_McmLM[im + ib + 1][0]);
                }
            }
            if (Nbmm == 2)
            {
             ...
             ++ic1, ++im1, icp1 += pcm, imp1 += pcm)
        {
            ...
            if (im1 >= 0)
            {
                for (int ib = 0, im2 = im1, imp2 = imp1; ib < Nbcm1;
                     ++ib, ++im2, imp2 += pcm)
                {
    ...
    for (int ic = 0, icp = 0; ic < Nc; ++ic, icp += pcm)
    {
        for (int jc = 0, jcp = 0; jc < Nc; ++jc, jcp += pcm)
        {
            ...
            const EigenMatrix15x15f e_A1ij(
                e_A1.block<pcm, pcm>(icp, jcp).cast<float>());
            const EigenMatrix15x15f e_A2ij(
                e_A2.block<pcm, pcm>(icp, jcp).cast<float>());
    ...
    const EigenMatrixXd e_I1 = EigenMatrixXd(g_A * e_AI),
                        e_I2 = EigenMatrixXd(e_AI * g_A);
    for (int ic = 0, icp = 0; ic < Nc; ++ic, icp += pcm)
    {
        for (int jc = 0, jcp = 0; jc < Nc; ++jc, jcp += pcm)
        {
            ...
            if (ic == jc)
            {
                EigenMatrix15x15f(e_I1.block<pcm, pcm>(icp, jcp).cast<float>())
                    .AssertEqual(e_I, 1, str1);
                EigenMatrix15x15f(e_I2.block<pcm, pcm>(icp, jcp).cast<float>())
    ...
    for (int ic = 0, im = Nm - Nc, icp = 0, imp = pc; ic < Nc;
         ++ic, ++im, icp += pcm, imp += pcm)
    {
    ...
    const EigenVectorXd e_Az1 = EigenVectorXd(g_A * g_z1),
                        e_e1 = EigenVectorXd(e_Az1 - e_r);
    const EigenVectorXd e_Az2 = EigenVectorXd(g_A * g_z2),
                        e_e2 = EigenVectorXd(e_Az2 - e_r);
    UT::Print("%e vs %e\n", e_e1.norm(), e_e2.norm());
#endif
}

void GlobalBundleAdjustor::ApplyM(
    const LA::AlignedVectorX<PCG_TYPE>& xs, LA::AlignedVectorX<PCG_TYPE>* Mxs)
{
    const int Nb = GBA_PCG_CONDITIONER_BAND, Nc = m_Cs.Size(),
              Nm = m_CsLM.Size();
    if (Nb <= 1)
    ...
    for (int ic = 0, im = Nm - Nc; ic < Nc; ++ic, ++im)
    {
        ...
        if (im >= 0)
        {
            ...
            for (int ib = 0; ib < Nbcm; ++ib)
            {
                LA::AlignedMatrix9x6f::AddAbTo<float>(
                    McmsTLM[ib], bc, bmsLM[im + ib]);
            }
        }
        ...
        if (im >= 0)
        {
            ...
            for (int ib = 0; ib < Nbmc; ++ib)
            {
                LA::AlignedMatrix6x9f::AddAbTo<float>(
                    MmcsTLM[ib], bm, bcs[im + ib + 1]);
            }
            const LA::AlignedMatrix9x9f* MmmsTLM = m_MmmTLM[im];
            if (Nbmm == 2)
            {
                LA::AlignedMatrix9x9f::AddAbTo<float>(
                    MmmsTLM[1], bm, bmsLM[im + 1]);
            }
...
#ifdef GBA_DEBUG_EIGEN_PCG
        const int icp = ic * pcm, imp = icp + pc;
        const EigenVector6f e_bci =
            EigenVector6f(e_b.block<pc, 1>(icp, 0).cast<float>());
        e_b.block<pm, 1>(imp, 0) +=
            g_M.block<pm, pc>(imp, icp) * e_bci.cast<double>();
        for (int _ic = ic + 1, _icp = icp + pcm; _ic < Nc; ++_ic, _icp += pcm)
        {
            e_b.block<pcm, 1>(_icp, 0) +=
                g_M.block<pcm, pc>(_icp, icp) * e_bci.cast<double>();
        }
        e_b.block<pc, 1>(icp, 0) =
            -g_M.block<pc, pc>(icp, icp) * e_bci.cast<double>();
        const EigenVector9f e_bmi =
            EigenVector9f(e_b.block<pm, 1>(imp, 0).cast<float>());
        for (int _ic = ic + 1, _icp = icp + pcm; _ic < Nc; ++_ic, _icp += pcm)
        {
            e_b.block<pcm, 1>(_icp, 0) +=
                g_M.block<pcm, pm>(_icp, imp) * e_bmi.cast<double>();
        }
        e_b.block<pm, 1>(imp, 0) =
            -g_M.block<pm, pm>(imp, imp) * e_bmi.cast<double>();
        for (int _ic = ic, _im = im, _icp = ic * pcm, _imp = _icp + pc;
             _ic < Nc;
             ++_ic, ++_im, _icp += pcm, _imp += pcm)
        {
            const EigenVector6f e_bci =
                EigenVector6f(e_b.block<pc, 1>(_icp, 0).cast<float>());
            ...
            {
                const EigenVector9f e_bmi =
                    EigenVector9f(e_b.block<pm, 1>(_imp, 0).cast<float>());
                e_bmi.AssertEqual(
                    bmsLM[_im], 1, UT::String("bm[%d][%d]", im, _im));
                e_b.block<pm, 1>(_imp, 0) =
                    EigenVector9f(bmsLM[_im]).cast<double>();
    ...
    for (int ic = Nc - 1, im = Nm - 1; ic >= 0; --ic, --im)
    {
        ...
        if (im >= 0)
        {
            ...
            for (int ib = 0; ib < Nbmc; ++ib)
            {
                LA::AlignedMatrix9x6f::AddAbTo(
                    MmcsLM[ib], m_bcs[ic + ib + 1], _bm);
            }
            if (Nbmm == 2)
            {
                LA::AlignedMatrix9x9f::AddAbTo(
                    m_MmmLM[im][1], m_bmsLM[im + 1], _bm);
            }
            m_bmsLM[im].Set(_bm);
        }
        ...
        {
            e_b.block<pm, 1>(imp, 0) +=
                g_M.block<pm, pcm>(imp, _icp) * e_b.block<pcm, 1>(_icp, 0);
        }
        e_b.block<pc, 1>(icp, 0) +=
            g_M.block<pc, pm>(icp, imp) * e_b.block<pm, 1>(imp, 0);
        for (int _ic = ic + 1, _icp = icp + pcm; _ic < Nc; ++_ic, _icp += pcm)
        {
            e_b.block<pc, 1>(icp, 0) +=
                g_M.block<pc, pcm>(icp, _icp) * e_b.block<pcm, 1>(_icp, 0);
        }
        const EigenVector6f e_bci =
            EigenVector6f(e_b.block<pc, 1>(icp, 0).cast<float>());
...
void GlobalBundleAdjustor::ApplyA(
    const LA::AlignedVectorX<PCG_TYPE>& xs, LA::AlignedVectorX<PCG_TYPE>* Axs)
{
    const LA::Vector6<PCG_TYPE>* xcs = (LA::Vector6<PCG_TYPE>*)xs.Data();
    ConvertCameraUpdates(xcs, &m_xcsP);
    Axs->Resize(xs.Size());
    ...
    for (int ic = 0; ic < Nc; ++ic)
    {
        ...
        {
            const int _ic = KF.m_ikp2KF[ikp];
            LA::AlignedMatrix6x6f::AddAbTo<PCG_TYPE>(
                Acbs[ikp], xc, (PCG_TYPE*)&Axcs[_ic]);
            LA::AlignedMatrix6x6f::AddAbTo<PCG_TYPE>(
                AcbTs[ikp], m_xcsP[_ic], Axc);
        }
    }
    // ConvertCameraUpdates(m_Axcs, Axcs);
    const int Nm = m_CsLM.Size();
    const int ic0 = Nc - Nm;
    ApplyAcm(m_xcsP.Data() + ic0,
        (LA::Vector9<PCG_TYPE>*)(xcs + Nc),
        Axcs + ic0,
        (LA::Vector9<PCG_TYPE>*)(Axcs + Nc),
        false,
        m_AmusLM.Size() == Nm ? m_AmusLM.Data() : NULL);
}

void GlobalBundleAdjustor::ApplyAcm(const LA::ProductVector6f* xcs,
...
void GlobalBundleAdjustor::ApplyAcm(const LA::ProductVector6f* xcs,
    const LA::Vector9d* xms, LA::Vector6d* Axcs, LA::Vector9d* Axms,
    const bool Acc, const LA::AlignedMatrix9x9f* Amus)
{
    LA::AlignedMatrix6x6f A66;
    LA::AlignedMatrix6x9f A69;
...
{
    const ubyte ucFlag =
        GBA_FLAG_FRAME_UPDATE_DELTA | GBA_FLAG_FRAME_UPDATE_TRACK_INFORMATION;
    const int nKFs = static_cast<int>(m_KFs.size());
    for (int iKF = 0; iKF < nKFs; ++iKF)
    {
        const KeyFrame& KF = m_KFs[iKF];
...
void GlobalBundleAdjustor::EmbeddedPointIteration(
    const AlignedVector<Rigid3D>& Cs, const std::vector<ubyte>& ucs,
    const std::vector<ubyte>& uds, std::vector<Depth::InverseGaussian>* ds)
{
