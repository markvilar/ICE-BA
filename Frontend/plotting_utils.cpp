...
PoseDrawer2D::PoseDrawer2D()
    : scale_(1.0), min_x_(-0.5), min_y_(-0.5), max_x_(0.5), max_y_(0.5)
{
}
void PoseDrawer2D::addPose(float x, float y, float alpha)
{
    ...
    scale_ =
        std::min(imageSize / (max_x_ - min_x_), imageSize / (max_x_ - min_y_));
}
...
cv::Point2f PoseDrawer2D::convertToImageCoordinates(
...
using cv::viz::Viz3d;
void viz3d_keyboard_cb(const cv::viz::KeyboardEvent& callback, void* cookie)
{
 ...
 */
PoseDrawer3D::PoseDrawer3D(float viz_cam_height, const cv::Matx33f& cam_K)
    : img3d_{cv::viz::WImage3D(
          cv::Mat(1, 1, CV_8UC1), cv::Size2f(0.16, 0.12))}, // placeholder
      viz3d_text_(cv::viz::WText("xPerception", cv::Point2i(10, 10), 40)),
      viz3d_view_mode_(TOP_VIEW_FOLLOW), key_pressed_(0),
      viz_cam_height_(viz_cam_height)
{
    ...
    viz_window_ptr_->setViewerPose(cv::viz::makeCameraPose(
        cv::Vec3f(0, 0, 3), cv::Vec3f(0, 0, 0), cv::Vec3f(0, -1, 0)));
    // Show grid in world 3d
    cv::viz::WGrid grid(
        cv::Vec2i(20, 20), cv::Vec2i(1, 1), cv::viz::Color::white());
    viz_window_ptr_->showWidget("world grid", grid);
    // Show
    // textvio/xp_ceres-solver/internal/xpceres/CMakeFiles/xpceres.dir/cxsparse.cc.o
...
void PoseDrawer3D::viz3d_once(const cv::Affine3f& W_T_D, const cv::Mat& img,
    const cv::Mat& rig_xyz_mat, const cv::Mat_<cv::Vec3f>& depth_img)
{
    ...
    const cv::Vec3f cam_xyz(
        W_T_D.translation()[0], W_T_D.translation()[1], W_T_D.translation()[2]);
    ...
    if (viz3d_view_mode_ != CAM_FOLLOW)
    {
        ...
        if (!rig_xyz_mat.empty())
        {
            ...
            {
                rig_xyz_affine3f[i + 1].translation(
                    rig_xyz_mat.at<cv::Vec3f>(i));
            }
    ...
    if (frame_rate_counter > 10)
    {
        const int ms = std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::steady_clock::now() - frame_rate_last_time)
                           .count();
        frame_rate_last_time = std::chrono::steady_clock::now();
        ...
        if (!depth_img.empty())
        {
            ...
            for (int x = 0; x < depth_img.cols; ++x)
            {
                for (int y = 0; y < depth_img.rows; ++y)
                {
                    ...
                    {
                        color(y, x)[2] = static_cast<uint8_t>(
                            255 * (far_cut - z_val) / far_cut);
                        color(y, x)[1] =
    ...
    if (viz3d_view_mode_ == TOP_VIEW_FOLLOW)
    {
        ...
        viz_window_ptr_->setViewerPose(cv::viz::makeCameraPose(
            cam_xyz * 0.5 + cv::Vec3f(0, 0, viz_cam_height_ + 0.5),
            cam_xyz * 0.5,
            cv::Vec3f(0, -1, 0)));
    }
    else if (viz3d_view_mode_ == CAM_FOLLOW)
...
uchar PoseDrawer3D::key_pressed() const { return key_pressed_; }
