...
inline int32x4_t compute_bilinear_interpolation(const int16x8_t& top_row,
    const int16x8_t& bottom_row, const int16x4_t& w00, const int16x4_t& w01,
    const int16x4_t& w10, const int16x4_t& w11, const int32x4_t& shift)
{
    ...
    for (y = 0; y < rows; ++y)
    {
        ...
        for (; x <= colsn - 8; x += 8)
        {
            __m128i s0 = _mm_loadu_si128(
                reinterpret_cast<const __m128i*>(trow0 + x - cn));
            __m128i s1 = _mm_loadu_si128(
                reinterpret_cast<const __m128i*>(trow0 + x + cn));
            __m128i s2 = _mm_loadu_si128(
                reinterpret_cast<const __m128i*>(trow1 + x - cn));
            __m128i s3 =
                _mm_load_si128(reinterpret_cast<const __m128i*>(trow1 + x));
            __m128i s4 = _mm_loadu_si128(
                reinterpret_cast<const __m128i*>(trow1 + x + cn));
        ...
        {
            int16x8_t q0 =
                vld1q_s16(static_cast<const int16_t*>(&trow0[x + cn]));
            int16x8_t q1 =
                vld1q_s16(static_cast<const int16_t*>(&trow0[x - cn]));
            int16x8_t q2 =
                vld1q_s16(static_cast<const int16_t*>(&trow1[x + cn]));
            int16x8_t q3 =
                vld1q_s16(static_cast<const int16_t*>(&trow1[x - cn]));
...
XPTrackerInvoker::XPTrackerInvoker(const Mat& _prevImg, const Mat& _prevDeriv,
    const Mat& _nextImg, std::vector<XPKeyPoint>* _prevPts,
    std::vector<Point2f>* _nextPts, std::vector<bool>* _status,
    std::vector<float>* _err, Size _winSize, TermCriteria _criteria, int _level,
    int _maxLevel, int _start_level, int _flags, float _minEigThreshold)
{
    prevImg = &_prevImg;
    ...
    cv::Mat* IWinBuf, cv::Mat* derivIWinBuf, float* const A_ptr) const
{
    ...
    const int32x4_t q12 =
        vdupq_n_s32(static_cast<int32_t>(-inter_param.w_bits));
#endif
    ...
    for (y = 0; y < winSize.height; y++)
    {
        ...
        const uchar* src_row1 =
            I.ptr<uchar>(y + iprevPt.y + 1) + iprevPt.x * cn;
             ...
             x += 4, dsrc_row0 += 4 * 2, dsrc_row1 += 4 * 2, dIptr += 4 * 2)
        {
            ...
            v00 = _mm_unpacklo_epi8(
                _mm_cvtsi32_si128(*reinterpret_cast<const int*>(src_row0 + x)),
                z);
            ...
            v10 = _mm_unpacklo_epi8(
                _mm_cvtsi32_si128(*reinterpret_cast<const int*>(src_row1 + x)),
                z);
            ...
            t0 = _mm_srai_epi32(
                _mm_add_epi32(t0, qdelta), inter_param.w_bits - 5);
            _mm_storel_epi64(
                reinterpret_cast<__m128i*>(Iptr + x), _mm_packs_epi32(t0, t0));

            v00 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(dsrc_row0));
            v01 = _mm_loadu_si128(
                reinterpret_cast<const __m128i*>(dsrc_row0 + cn2));
            v10 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(dsrc_row1));
            v11 = _mm_loadu_si128(
                reinterpret_cast<const __m128i*>(dsrc_row1 + cn2));
            ...
            t0 =
                _mm_srai_epi32(_mm_add_epi32(t0, qdelta_d), inter_param.w_bits);
            t1 =
                _mm_srai_epi32(_mm_add_epi32(t1, qdelta_d), inter_param.w_bits);
            ...
            t1 =
                _mm_srai_epi32(_mm_slli_epi32(v00, 16), 16); //  Ix0 Ix1 Ix2 Ix3
             ...
             x += 4, dsrc_row0 += 4 * 2, dsrc_row1 += 4 * 2, dIptr += 4 * 2)
        {
            ...
            int32x4_t q4 = vaddq_s32(
                vmull_s16(d0d1.val[0], d26), vmull_s16(d2d3.val[0], d27));
            int32x4_t q6 = vaddq_s32(
                vmull_s16(d0d1.val[1], d26), vmull_s16(d2d3.val[1], d27));

            int32x4_t q7 = vaddq_s32(
                vmull_s16(d4d5.val[0], d28), vmull_s16(d6d7.val[0], d29));
            int32x4_t q8 = vaddq_s32(
                vmull_s16(d4d5.val[1], d28), vmull_s16(d6d7.val[1], d29));
...
#endif
        for (; x < winSize.width;
             ++x, dsrc_row0 += 2, dsrc_row1 += 2, dIptr += 2)
        {
            ...
            int ixval = CV_DESCALE(dsrc_row0[0] * inter_param.iw00 +
                                       dsrc_row0[2] * inter_param.iw01 +
                                       dsrc_row1[0] * inter_param.iw10 +
                                       dsrc_row1[2] * inter_param.iw11,
                inter_param.w_bits);
            int iyval = CV_DESCALE(dsrc_row0[1] * inter_param.iw00 +
                                       dsrc_row0[3] * inter_param.iw01 +
                                       dsrc_row1[1] * inter_param.iw10 +
                                       dsrc_row1[3] * inter_param.iw11,
                inter_param.w_bits);
...
#elif __ARM_NEON__
    iA11 +=
        static_cast<float>(vgetq_lane_s32(nA11, 0) + vgetq_lane_s32(nA11, 1) +
                           vgetq_lane_s32(nA11, 2) + vgetq_lane_s32(nA11, 3));
    iA12 +=
        static_cast<float>(vgetq_lane_s32(nA12, 0) + vgetq_lane_s32(nA12, 1) +
                           vgetq_lane_s32(nA12, 2) + vgetq_lane_s32(nA12, 3));
    iA22 +=
        static_cast<float>(vgetq_lane_s32(nA22, 0) + vgetq_lane_s32(nA22, 1) +
...
{
    const Point2f halfWin(
        (winSize.width - 1) * 0.5f, (winSize.height - 1) * 0.5f);
    ...
    cv::Rect valid_region(half_diff.x,
        half_diff.y,
        Jcols - iter_cache_size.width,
        Jrows - iter_cache_size.height);
    const bool get_min_eigenvals = (flags & OPTFLOW_LK_GET_MIN_EIGENVALS) != 0;
    cv::Mat J_patch(iter_cache_size,
        CV_MAKETYPE(DataType<uchar>::depth, 1),
    ...
    for (int ptidx = range.start; ptidx < range.end; ++ptidx)
    {
        ...
        DBG_XP_PYRAMID(
            "ft_id: " << prevPts[ptidx].class_id << " cur level = " << level);
        ...
        if (XPTrackerInvoker::is_keypoint_not_in_valid_range(
                iprevPt, valid_region))
        {
        ...
        Mat derivIWinBuf(
            winSize, CV_MAKETYPE(derivDepth, cn2), kp_repo->xy_gradient);
        ...
        for (j = 0; j < max_iterations; ++j)
        {
            ...
            // this info will be used to decide whether update the cached
            // buffer.
            Point2i off = inextPt - init_pos;

            if (std::abs(off.x) > half_diff.x ||
                std::abs(off.y) > half_diff.y || j == 0)
            {
#ifndef __ARM_NEON__
                for (int m = 0; m < iter_cache_size.height + 1; ++m)
                {
                    uchar* dst = const_cast<uchar*>(J_patch.ptr<uchar>(m));
                    uchar* src = const_cast<uchar*>(
                        J.ptr<uchar>(m + inextPt.y - half_diff.y) + inextPt.x -
                        half_diff.x);
            ...
            {
                const uchar* Jptr0 =
                    J_patch.ptr<uchar>(y + half_diff.y + off.y) +
                    (half_diff.x + off.x);
                const uchar* Jptr1 =
                ...
                for (; x <= winSize.width - 8; x += 8, dIptr += 8 * 2)
                {
                    ...
                    __m128i v00 = _mm_unpacklo_epi8(
                        _mm_loadl_epi64(
                            reinterpret_cast<const __m128i*>(Jptr0 + x)),
                        z);
                    __m128i v01 = _mm_unpacklo_epi8(
                        _mm_loadl_epi64(
                            reinterpret_cast<const __m128i*>(Jptr0 + x + cn)),
                        z);
                    __m128i v10 = _mm_unpacklo_epi8(
                        _mm_loadl_epi64(
                            reinterpret_cast<const __m128i*>(Jptr1 + x)),
                        z);
                    __m128i v11 = _mm_unpacklo_epi8(
                        _mm_loadl_epi64(
                            reinterpret_cast<const __m128i*>(Jptr1 + x + cn)),
                        z);
                    ...
                    diff0 = _mm_unpacklo_epi16(
                        diff0, diff0); //  It0 It0 It1 It1 ...
                    //  Ix0 Iy0 Ix1 Iy1 ...
                    v00 = _mm_loadu_si128(
                        reinterpret_cast<const __m128i*>(dIptr));
                    v01 = _mm_loadu_si128(
                        reinterpret_cast<const __m128i*>(dIptr + 8));
                ...
                for (; x <= winSize.width - 4; x += 4, dIptr += 8)
                {
                    ...
                    int16x8_t q1 =
                        vreinterpretq_s16_u16(vmovl_u8(vld1_u8(&Jptr1[x])));
                    // do interpolation
                ...
                q3 = vld2_s16(dIptr);
                q2 = vsetq_lane_s32(
                    0, q2, 3); // IMPORTANT: set lane3 to 0 for win_size 7
                // accumulate
                nb1 = vmlaq_s32(nb1, q2, vmovl_s16(q3.val[0]));
                nb2 = vmlaq_s32(nb2, q2, vmovl_s16(q3.val[1]));
                x += 3;
                ...
                {
                    int diff =
                        CV_DESCALE(Jptr0[x] * iw00 + Jptr0[x + 1] * iw01 +
            ...
            {
                DBG_XP_PYRAMID("break delta < epsilon at itr = "
                               << j << " nextPt = " << nextPts[ptidx]);
                break;
            }
            ...
            (flags & OPTFLOW_LK_GET_MIN_EIGENVALS) == 0)
        {
            ...
            // TODO(yanghongtian) : supportint patch here, update
            // need_to_update_repo flag. for now, need_to_update_repo is always
            // true.
            ...
            err[ptidx] =
                errval * 1.f / (32 * winSize.width * cn * winSize.height);
        }
    }
}
...
void XPcalcOpticalFlowPyrLK(const std::vector<cv::Mat>& _prevPyramids,
    ...
    const cv::Size _win_size, int _max_level, int _start_level,
    TermCriteria _criteria, int _flags, double _minEigThreshold)
{
#ifdef _XP_OPTICAL_FLOW_DEBUG_MODE_
    ...
    for (level = _max_level; level >= _start_level; --level)
    {
        ...
        XPTrackerInvoker(_prevPyramids[level],
            derivI,
            _nextPyramids[level],
            _prevPts,
            _nextPts,
