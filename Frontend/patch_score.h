...
public:
    ...
    void computeScore(uint8_t* cur_patch, int stride, int* zmssd, int* ssd)
    {
        ...
        if (patch_size_ == 8)
        {
            ...
            xImageAsEightBytes =
                _mm_loadl_epi64((__m128i*)imagepointer); // NOLINT
    ...
    int computeSsdScoreSlow(uint8_t* cur_patch, int stride) const
    {
        ...
        for (int y = 0; y < patch_size_; ++y)
        {
            ...
            {
                const uint8_t cur_px = cur_patch_ptr[x];
                int32_t d = static_cast<int32_t>(cur_px) -
                            static_cast<int32_t>(ref_patch_[r]);
                sum_ssd += d * d;
            }
        }
        return sum_ssd;
    ...
    // to rounding issue in integer division
    int computeZmssdScoreSlow(uint8_t* cur_patch, int stride) const
    {
        ...
        for (int y = 0, r = 0; y < patch_size_; ++y)
        {
            ...
            {
                int32_t zmd =
                    static_cast<int32_t>(ref_patch_[r]) * patch_area_ - sumA -
