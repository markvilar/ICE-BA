...
// container. The caller should be responsible for checking non-empty before
// pop_to_back. std::deque has back()
template <typename T> void pop_to_back(std::deque<T>* container, T* elem)
{
...
template <typename T, typename Container = std::deque<T>> class shared_queue
{
public:
