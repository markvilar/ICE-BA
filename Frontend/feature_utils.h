 ...
 */
inline void fast_pyra_down_internal(
    const cv::Mat& img_in_smooth, cv::Mat* _img_in_small)
{
    ...
    for (int rows = img_in_small.rows, y = 0; y < rows; ++y)
    {
        ...
        {
            sum0 = ((static_cast<int>(*data0_ptr) +
                        static_cast<int>(*(data0_ptr + 1))) +
    ...
    const cv::Mat& img_in_smooth, cv::Mat* _img_in_small)
{
    ...
    for (int rows = img_in_small.rows, y = 0; y < rows; ++y)
    {
        ...
        {
            int sum = ((static_cast<int>(*data0_ptr) +
                           static_cast<int>(*(data0_ptr + 1))) +
                          (static_cast<int>(*data1_ptr) +
                              static_cast<int>(*(data1_ptr + 1)))) /
                      4;
 ...
 */
inline cv::Mat fast_pyra_down(
    const cv::Mat& img_in_smooth, uchar* const data = nullptr)
{
...
void build_pyramids(const cv::Mat& img, int max_level,
    std::vector<cv::Mat>* pyramids, uchar* const pyra_buf_ptr = NULL);
...
public:
    ...
    int get()
    {
        ...
        // wrap around
        if (id_ <= 0)
            id_ = 1; // 0 indicates an invalid id!
        return id_;
    }
...
// This class performs feature (re)detection + feature propagation with optical
// flow by only considering the master view
class FeatureTrackDetector
{
public:
    struct FeatureTrack
    {
        explicit FeatureTrack(const cv::Point2f pt)
            : length(1), isActive(true), point(pt)
        {
        }
        int length;
        bool isActive;
    ...
    FeatureTrackDetector(const int length_thres, const float drop_rate,
    ...
    bool detect(const cv::Mat& img_in_smooth, const cv::Mat_<uchar>& mask,
    ...
    inline size_t feature_tracks_number() const
    {
        return feature_tracks_map_.size();
    }
    int add_new_feature_track(
        const cv::Point2f pt); // Return the added feature track id
...
protected:
    // Once feature track exceeds this threshold, we break this feature track
    // randomly with drop_rate. [NOTE] Use a negative drop rate for no drop out
    ...
    // A very simple-minded data structure to bookkeep feature track ids and
    // lengths We will have at most request_feat_num active feature tracks
    std::map<int, FeatureTrack> feature_tracks_map_;

    // A mask to hold the mask of the union of input mask and optical flow feats
    // mask
...
public:
    ...
    std::vector<cv::Mat>& get_prev_img_pyramids() { return prev_img_pyramids_; }
    std::vector<cv::Mat>& get_curr_img_pyramids() { return curr_img_pyramids_; }
    enum
    {
        BUILD_TO_CURR = 0,
        BUILD_TO_PREV = 1
    ...
    void build_img_pyramids(
        const cv::Mat& img_in_smooth, int build_type = BUILD_TO_CURR)
    {
        if (build_type == BUILD_TO_CURR)
        {
            build_pyramids(img_in_smooth,
                kMaxPyraLevelOF,
                &curr_img_pyramids_,
                curr_pyramids_buffer_.get());
        }
        else
        {
            build_pyramids(img_in_smooth,
...
public:
    ...
    explicit SlaveImgFeatureDetector(
        int block_size = 8, // block matching size. Multiple of 4
    ...
    bool detect_features_on_slave_img(const cv::Mat& master_image,
        const cv::Mat& slave_image, const std::vector<cv::KeyPoint>& master_kps,
        const DuoCalibParam& duo_calib_param,
        std::vector<cv::KeyPoint>* slave_kps_ptr,
        cv::Mat* slave_orb_feat_ptr = nullptr, int max_pixel_val_diff = 15);

private:
    bool detect_features_on_slave_img_helper(const cv::Mat& master_image,
        const cv::Mat& slave_image, const DuoCalibParam& duo_calib_param,
        int max_pixel_val_diff, int master_x, int master_y,
        const cv::Mat_<float>& s_R_m, const cv::Mat_<float>& s_t_m,
        ...
        int* best_slave_x_ptr, int* best_slave_y_ptr,
        int* best_search_dist_id_ptr, int* second_best_search_dist_id_ptr);
...
std::vector<std::vector<cv::DMatch>> neon_orb_match(const cv::Mat& desc_query,
    const cv::Mat& matching_mask, const cv::Mat& orb_desc_training);
std::vector<std::vector<cv::DMatch>> neon_orb_match_nn(
    const cv::Mat& desc_query, const cv::Mat& matching_mask,
...
int neon_find_close_points(float query_u, float query_v, int training_num,
    float* training_u, float* training_v,
...
public:
    ...
    inline double Score(double u, double v) const override
    {
        ...
        return oneMinus_rv * (oneMinus_ru * _scores.at<float>(v_int, u_int) +
                                 ru * _scores.at<float>(v_int, u_int + 1)) +
               rv * (oneMinus_ru * _scores.at<float>(v_int + 1, u_int) +
...
// Subpixel feature alignment (1D & 2D version, borrowed from SVO
// implementation)
namespace align
{
bool align1D(const cv::Mat& cur_img,
    ...
    Eigen::Vector2f* cur_px_estimate, float* h_inv);

bool align2D(const cv::Mat& cur_img, uint8_t* ref_patch_with_border,
    uint8_t* ref_patch, const int n_iter, Eigen::Vector2f* cur_px_estimate,
    bool no_simd = false);
...
// Warp a patch from the reference view to the current view (borrowed from SVO
// implementation)
namespace warp
{
bool getWarpMatrixAffine(const vio::cameras::CameraBase& cam_ref,
    const vio::cameras::CameraBase& cam_cur, const Eigen::Vector2f& px_ref,
    const Eigen::Vector3f& f_ref, const float depth_ref,
    const Eigen::Matrix3f& R_cur_ref, const Eigen::Vector3f& t_cur_ref,
    const int level_ref, Eigen::Matrix2f* A_cur_ref);

int getBestSearchLevel(const Eigen::Matrix2f& A_cur_ref, const int max_level);

bool warpAffine(const Eigen::Matrix2f& A_cur_ref, const cv::Mat& img_ref,
    const Eigen::Vector2f& px_ref, const int level_ref, const int level_cur,
...
public:
    ...
    struct Options
    {
        bool align_1d; //!< in epipolar search: align patch 1D along epipolar
                       //!< line
        int max_iter; //!< number of iterations for aligning the feature patches
                      //!< in gauss newton
        float max_epi_length_optim;  //!< max length of epipolar line to skip
                                     //!< epipolar search and directly go to img
                                     //!< align  NOLINT
        size_t max_epi_search_steps; //!< max number of evaluations along
                                     //!< epipolar line
        bool subpix_refinement; //!< do gauss newton feature patch alignment
                                //!< after epipolar search
        bool epi_search_edgelet_filtering;
        float epi_search_edgelet_max_angle;
        Options()
            : align_1d(false), max_iter(10), max_epi_length_optim(2.0),
              max_epi_search_steps(1000), subpix_refinement(true),
    ...
    float epi_length_; //!< length of epipolar line segment in pixels (only used
                       //!< for epipolar search)
    ...
    // [NOTE] This function assumes that px_cur is already set to an estimate
    // that is within ~2-3 pixel of the final result!
    bool findMatchDirect(const vio::cameras::CameraBase& cam_ref,
        const vio::cameras::CameraBase& cam_cur, const Eigen::Vector2f& px_ref,
        const Eigen::Vector3f& f_ref, const Eigen::Matrix3f& R_cur_ref,
        const Eigen::Vector3f& t_cur_ref, const int level_ref,
        const float depth_ref, const std::vector<cv::Mat>& pyrs_ref,
        const std::vector<cv::Mat>& pyrs_cur, const bool edgelet_feature,
    ...
    bool findEpipolarMatchDirect(const vio::cameras::CameraBase& cam_ref,
        ...
        const Eigen::Vector3f& f_ref, const Eigen::Matrix3f& R_cur_ref,
        const Eigen::Vector3f& t_cur_ref, const int level_ref,
        const float d_estimate, const float d_min, const float d_max,
        const std::vector<cv::Mat>& pyrs_ref,
        const std::vector<cv::Mat>& pyrs_cur, const cv::Mat_<uchar>& mask_cur,
        const bool edgelet_feature, Eigen::Vector2f* px_cur, float* depth,
        int* level_cur, cv::Mat* dbg_cur = nullptr);
...
public:
    ImgFeaturePropagator(const Eigen::Matrix3f& cur_camK,
        ...
        const cv::Mat_<uchar>& cur_mask, int feat_det_pyramid_level,
    ...
    bool PropagateFeatures(const cv::Mat& cur_img,
        ...
        cv::Mat* cur_orb_features = nullptr, const bool draw_debug = false);
...
bool generate_cam_mask(const cv::Matx33f& K, const cv::Mat_<float>& dist_coeffs,
    const cv::Size& mask_size, cv::Mat_<uchar>* cam_mask, float* fov_deg);

bool detect_orb_features(const cv::Mat& img_in_raw, const cv::Mat_<uchar>& mask,
    int request_feat_num, int pyra_level, int fast_thresh,
...
// OF This function is a wrapper of the pyramid version below.
void propagate_with_optical_flow(const cv::Mat& img_in_smooth,
    const cv::Mat_<uchar>& mask, const cv::Mat& pre_image,
    const cv::Mat& pre_image_orb_feature,
    const std::vector<cv::KeyPoint>& pre_image_keypoints,
    FeatureTrackDetector* feat_tracker_detector,
    std::vector<cv::KeyPoint>* key_pnts_ptr,
    cv::Mat_<uchar>* mask_with_of_out_ptr, cv::Mat* orb_feat_OF_ptr = nullptr,
    const cv::Vec2f& init_pixel_shift = cv::Vec2f(0, 0),
    const cv::Matx33f* K_ptr = nullptr,
    const cv::Mat_<float>* dist_ptr = nullptr,
    const cv::Matx33f* old_R_new_ptr = nullptr,
...
void XpEnforceKeyPointUniformity(const cv::Mat& LUT, double radius, int imgrows,
    int imgcols, size_t maxNumKpt,
    std::vector<brisk::ScoreCalculator<float>::PointWithScore>&
        points); // NOLINT
...
inline cv::Mat fast_pyra_down_original(const cv::Mat& img_in_smooth)
{
    ...
    for (int y = 0; y < img_in_small.rows; y++)
    {
        for (int x = 0; x < img_in_small.cols; x++)
        {
            ...
            int sum = static_cast<int>(*(img_in_smooth.data + shift0)) +
            ...
            *(img_in_small.data + y * width_step_small + x) =
                static_cast<uchar>(sum / 4);
        }
    }
    return img_in_small;
}
